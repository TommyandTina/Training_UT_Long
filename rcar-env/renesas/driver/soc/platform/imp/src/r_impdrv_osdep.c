 ...
 * num of sub device for vsdp*_reg.
***********************************************************************************************************************/
#define IMPDRV_DSP_VDSP_DEV_SUB1    (2U)
/**********************************************************************************************************************/

/*******************************************************************************************************************//**
 * @def IMPDRV_WAITPM_TIMEOUT
 * timeout for R_OSAL_PmWaitForState.
***********************************************************************************************************************/
#define IMPDRV_WAITPM_TIMEOUT       (1000)              /**< timeout for R_OSAL_PmWaitForState. */
/*******************************************************************************************************************//**
 * @def IMPDRV_OSAL_DSP_DEV_NUM_MAX
 * Num of max sub device for OSAL DSP core.
***********************************************************************************************************************/
...
/*******************************************************************************************************************//**
 * @brief           Wait for State transition of Power management, at Power ON.
 * @syncasync       Synchronous
 * @reentrant       Non-Reentrant
 * @param[in,out]   p_device_handle     OSAL device handle
 * @return          Function result
...
***********************************************************************************************************************/
IMPDRV_STATIC
e_impdrv_errorcode_t impdrv_osdep_wait_for_req_state(
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        /* wait for reset state to change */
        lRetOsal = R_OSAL_PmWaitForState(p_device_handle->handle, OSAL_PM_STATE_READY, IMPDRV_WAITPM_TIMEOUT);
        if (OSAL_RETURN_OK != lRetOsal)
        {
            ercd = IMPDRV_EC_NG_PMSYSTEMERROR;
        }
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        /** Power on core (only vdsp**_00 ~ vdsp**_01) */
        if ((IMPDRV_EC_OK == ercd) && (sub_dev_num < IMPDRV_DSP_VDSP_DEV_SUB1))
        {
            ercd = impdrv_osdep_pow_on(p_device_handle);
            if (IMPDRV_EC_OK != ercd)
            {
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        {
            /** Request OSAL to register the interrupt handler */
            lRetOsal = R_OSAL_InterruptUnregisterIsr(p_device_handle->handle, irq_channel, 
                                                    int_handler);
            if (OSAL_RETURN_OK != lRetOsal)
            {
                ercd = IMPDRV_EC_NG_SYSTEMERROR;
...
/***********************************************************************************************************************
* Start of function impdrv_osdep_reset_core_dsp()
* CD_PD_02_05_0037
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        if (false != reset)
        {
            ...
            {
                /* wait for reset state to change */
                lRetOsal = R_OSAL_PmWaitForState(p_device_handle->handle, OSAL_PM_STATE_RESET, IMPDRV_WAITPM_TIMEOUT);
                if (OSAL_RETURN_OK != lRetOsal)
                {
                    ercd = IMPDRV_EC_NG_PMSYSTEMERROR;
                }
    ...
    /** To wait finish register writing */
    if ((IMPDRV_EC_OK == ercd) && (false == reset))
    {
...
/***********************************************************************************************************************
* End of function impdrv_osdep_reset_core_dsp()
***********************************************************************************************************************/
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        while ('\0' != type[index])
        {
            if ((IMPDRV_DEV_LIST_SIZE - 5U) <= index)
            {
                ercd = IMPDRV_EC_NG_PARAM;
                break;
            }
...
)
{
    ...
    else if (IMPDRV_OSAL_DSP_DEV_NUM_MAX <= sub_dev_num)
    {
        ercd = IMPDRV_EC_NG_SYSTEMERROR;
    }
    else
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        while ('\0' != type[index])
        {
            if ((IMPDRV_DEV_LIST_SIZE - 7U) <= index)
            {
                ercd = IMPDRV_EC_NG_PARAM;
                break;
            }
...
/***********************************************************************************************************************
* Start of function impdrv_osdep_wait_for_req_state()
* CD_PD_02_05_0041
* [Covers: UD_PD_UD02_05_0041]
***********************************************************************************************************************/
IMPDRV_STATIC
e_impdrv_errorcode_t impdrv_osdep_wait_for_req_state(
    const st_impdrv_device_handle_t   *const p_device_handle
)
{
    ...
    e_impdrv_errorcode_t        ercd = IMPDRV_EC_OK;
    e_osal_return_t             lRetOsal;
    e_osal_pm_state_t           lRequiredState = OSAL_PM_STATE_RESET;
    e_osal_pm_reset_state_t     lResetState = OSAL_PM_RESET_STATE_INVALID;
    ...
    /** Wait for State transition of Power management */
    if (IMPDRV_EC_OK == ercd)
    {
        /** Get the Current reset state setting */
        lRetOsal = R_OSAL_PmGetResetState(p_device_handle->handle, &lResetState);
        if (OSAL_RETURN_OK == lRetOsal)
        {
            if (OSAL_PM_RESET_STATE_INVALID == lResetState)
            {
                ercd = IMPDRV_EC_NG_PMSYSTEMERROR;
            }
            else
            {
                if (OSAL_PM_RESET_STATE_RELEASED == lResetState)
                {
                    lRequiredState = OSAL_PM_STATE_READY;
                }
            }
        }
        else if (OSAL_RETURN_UNSUPPORTED_OPERATION == lRetOsal)
        {
            lRequiredState = OSAL_PM_STATE_READY;
        }
        else
        ...
        if (IMPDRV_EC_OK == ercd)
        {
            lRetOsal = R_OSAL_PmWaitForState(p_device_handle->handle, lRequiredState, IMPDRV_WAITPM_TIMEOUT);
            if (OSAL_RETURN_OK != lRetOsal)
            {
                ercd = IMPDRV_EC_NG_PMSYSTEMERROR;
            }
...
/***********************************************************************************************************************
* End of function impdrv_osdep_wait_for_req_state()
***********************************************************************************************************************/
...
)
{
    ...
    {
        lRetOsal = R_OSAL_PmSetRequiredState(p_device_handle->handle, OSAL_PM_REQUIRED_STATE_REQUIRED, false);
        if (OSAL_RETURN_OK != lRetOsal)
        {
            ercd = IMPDRV_EC_NG_PMSYSTEMERROR;
        }
        ...
        {
            /** Wait for State transition of Power management. */
            ercd = impdrv_osdep_wait_for_req_state(p_device_handle);
        }
    }
...
)
{
    ...
    core_info.core_num  = IMPDRV_DSP_CORE_0;
...
)
{
    ...
    st_impdrv_core_info_t   core_info;
    core_info.core_type = IMPDRV_CORE_TYPE_DSP;
    core_info.core_num  = IMPDRV_DSP_CORE_1;
...
)
{
    ...
    st_impdrv_core_info_t   core_info;
    core_info.core_type = IMPDRV_CORE_TYPE_DSP;
    core_info.core_num  = IMPDRV_DSP_CORE_2;
...
)
{
    ...
    st_impdrv_core_info_t   core_info;
    core_info.core_type = IMPDRV_CORE_TYPE_DSP;
    core_info.core_num  = IMPDRV_DSP_CORE_3;
