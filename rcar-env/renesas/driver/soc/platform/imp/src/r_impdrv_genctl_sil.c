...
static st_impdrv_commonctl_t  *sp_common_ctl;
static uint64_t               s_irq_channel;
...
)
{
    ...
    {
        ercd_sub = impdrv_cmnctl_mutex_unlock(&p_impdrv_ctl->common_ctl);
        if (IMPDRV_EC_OK != ercd_sub)
        {
            ercd = ercd_sub;
        }
...
)
{
    ...
    /** temporally use global variable */
    s_irq_channel = irq_channel;
    sp_common_ctl = &p_impdrv_ctl->common_ctl;
    ...
    if (IMPDRV_EC_OK == fb_ercd)
    {
        for (index= 0U; index < use_core_num; index++)
        {
            ...
            {
                /** Break, only one core handled for simulator. */
                break;
            }
        }
    }
...
)
{
    ...
    if (NULL == sp_common_ctl)
    {
        fb_ercd = IMPDRV_EC_NG_SYSTEMERROR;
    }
    else
    {
        for (loop_index= 0U; loop_index < IMPDRV_INTSTS_BITNUM; loop_index++)
        {
            core_info[loop_index].core_type = IMPDRV_CORE_TYPE_INVALID;
            core_info[loop_index].core_num  = 0U;
        }

        /** Confirm clear interrupt status. */

        while (IMPDRV_EC_OK == fb_ercd)
        {
            /** Initialize, variable */
            loop_conti = false;

            /** Judgement interrupt status cleared that handled in interrupt */
            fb_ercd = impdrv_cmnctl_judge_int(sp_common_ctl, s_irq_channel,
                core_info, &use_core_num, false);
            if (IMPDRV_EC_OK == fb_ercd)
            {
                for (loop_index = 0U; use_core_num > loop_index; loop_index++)
                {
                    if ((p_core_info->core_type == core_info[loop_index].core_type) &&
                        (p_core_info->core_num  == core_info[loop_index].core_num))
                    {
                        loop_conti = true;
                    }
                }
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        while (cnt < use_core_num)
        {
                ...
                ( IMPDRV_CORE_TYPE_DMAC == core_info[cnt].core_type) && (0U == core_info[cnt].core_num))
            {
                ...
                /** Dealing with simulator not running */
#if 0
