...
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_CNN_CLSAR
 * Offset address of hardware register for CLSAR.
***********************************************************************************************************************/
#define IMPDRV_REG_CNN_CLSAR        (0x0080U)           /**< Offset address of hardware register for CLSAR. */
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_CNN_SCLP
 * Offset address of hardware register for SCLP.
***********************************************************************************************************************/
#define IMPDRV_REG_CNN_SCLP         (0x0084U)           /**< Offset address of hardware register for SCLP. */
/*******************************************************************************************************************//**
 ...
 * ***********************************************************************************************************************/
#define IMPDRV_CNN_SR_CLBUSY        (0x00002000U)       /**< CLBSY register value for CCN.*/
/*******************************************************************************************************************//**
...
***********************************************************************************************************************/
#define IMPDRV_CNN_SR_WUPCOVF       (0x00004000U)       /**< WUPCOVF interrupt enable registration.*/
/*******************************************************************************************************************//**
...
/*******************************************************************************************************************//**
 * @def IMPDRV_CNN_REG_MAX_ADRS_CNFCHK
 *  End of offset address for ConfRegCheck of CNN.
***********************************************************************************************************************/
#define IMPDRV_CNN_REG_MAX_ADRS_CNFCHK  (0x0000FFFFU)   /** End of offset address for ConfRegCheck of CNN. */
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        p_corectl->irq_mask[IMPDRV_IRQMASK_MSCO] = true;
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        if (IMPDRV_EC_OK == ercd)
        {
            /** Write the Start address for Command List processing Register(CLSAR) */
            ercd = impdrv_osdep_write_reg(&p_corectl->device_io, IMPDRV_REG_CNN_CLSAR,
...
)
{
    ...
    while ((IMPDRV_EC_OK == ercd) && (param_num > index))
    {
        ...
        else if (IMPDRV_CNN_REG_MAX_ADRS_CNFCHK < p_chk_param[index].offset)
        {
            ercd = IMPDRV_EC_NG_NOTSUPPORT;
        }
        else
...
)
{
    ...
    else
    {
        ...
        if (IMPDRV_EC_OK == ercd)
        {
            ercd = impdrv_osdep_write_reg(&p_corectl->device_io, IMPDRV_REG_CNN_SWRST,
                                                            IMPDRV_CNN_CNF_CLR, false, 0U);
        }
    }
...
)
{
    ...
    if (NULL != p_corectl)
    {
        ...
        {
            if (((uint32_t)IMPDRV_IRQMASK_MSCO == i) && (false == p_corectl->irq_mask[i]))
            {
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        *p_state = IMPDRV_STATE_EXEC;
        ...
        if (0U != (IMPDRV_CNN_SR_MSCO & stat_val))
        {
            /** When the MSCO bit of SR register is not 0. */
            *p_mask_val = *p_mask_val | IMPDRV_CNN_SR_MSCO;
            cb_ercd[*p_cb_ercd_num] = IMPDRV_CB_RET_MSCO;
            (*p_cb_ercd_num)++;
        }
        if (0U != (IMPDRV_CNN_SR_WUPCOVF & stat_val))
        {
            /** When the WUPOVF bit of SR register is not 0. */
            *p_is_check_intclear = false;
            *p_mask_val = *p_mask_val | IMPDRV_CNN_SR_WUPCOVF;
