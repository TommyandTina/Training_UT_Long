...
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_OCV_LWM
 * Offset address of LWM register for OCV.
***********************************************************************************************************************/
#define IMPDRV_REG_OCV_LWM          (0x021CU)           /**< Offset address of LWM register for OCV. */
/*******************************************************************************************************************//**
 * @def IMPDRV_OCV_VCR
...
/*******************************************************************************************************************//**
 * @def IMPDRV_OCV_0X001_VAL
 * 0X001 value for OCV.
***********************************************************************************************************************/
#define IMPDRV_OCV_0X001_VAL        (0x0001U)           /**< 0X001 value for OCV. */
/*******************************************************************************************************************//**
 * @def IMPDRV_OCV_8KBSTART_VAL
 * 8KBSTART value for OCV.
***********************************************************************************************************************/
#define IMPDRV_OCV_8KBSTART_VAL     (0x0000U)           /**< 8KBSTART value for OCV. */
/*******************************************************************************************************************//**
 * @def IMPDRV_OCV_8KBNEXT_VAL
 * 8KBNEXT value for OCV.
***********************************************************************************************************************/
...
/*******************************************************************************************************************//**
 * @brief           Initialize the Local Working Memory.
 * @syncasync       Synchronous.
 * @reentrant       Non-Reentrant
 * @param[in]       p_corectl           Control data structure for specific core.
 * @return          Return value
...
)
{
    ...
    /** State check the Global Working Memory(GWM) and CPG reset */
    if (IMPDRV_EC_OK == ercd)
    {
    ...
    /** State check the Global Working Memory(GWM) */
    if (IMPDRV_EC_OK == ercd)
    {
        /** Read the Memory Initialization Register(MEMINITR) */
        ercd = impdrv_osdep_read_reg(&p_corectl->device_io, IMPDRV_REG_OCV_MEMINITR, &reg_val);
    ...
    /** Initialize the Global Working Memory(GWM) */
    if (IMPDRV_EC_OK == ercd)
    {
        /** Check reset status */
        if (0U != (IMPDRV_OCV_RESET_STATUS_VAL & reg_val))
        ...
        {
            /** Write the Memory Initialization Register(MEMINITR) for Initialization GWM */
            ercd = impdrv_osdep_write_reg(&p_corectl->device_io, IMPDRV_REG_OCV_MEMINITR,
                                                            IMPDRV_OCV_0X001_VAL, false, 0U);
            if (IMPDRV_EC_OK == ercd)
            {
                do
                {
    ...
    /** Initialize the Local Working Memory(LWM) */
    if (IMPDRV_EC_OK == ercd)
    {
        ercd = impdrv_ocvctl_init_lwm(p_corectl);
    }
...
/***********************************************************************************************************************
* Start of function impdrv_ocvctl_init_lwm()
* CD_PD_02_04_2052
* [Covers: UD_PD_UD02_04_0052]
***********************************************************************************************************************/
IMPDRV_STATIC
e_impdrv_errorcode_t impdrv_ocvctl_init_lwm(
    const st_impdrv_corectl_t   *const p_corectl
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        /** Write the Memory Local Working Memory Register for Initialization LWM */
        ercd = impdrv_osdep_write_reg(&p_corectl->device_io, IMPDRV_REG_OCV_LWM,
                                        IMPDRV_OCV_8KBSTART_VAL, true, IMPDRV_OCV_8KBSTART_VAL);
        if (IMPDRV_EC_OK == ercd)
        {
            offset = IMPDRV_OCV_OFST_START_VAL;
            while (offset < IMPDRV_OCV_OFST_END_VAL)
            {
                ercd = impdrv_osdep_write_reg(&p_corectl->device_io, offset,
                                                IMPDRV_OCV_8KBSTART_VAL, true, IMPDRV_OCV_8KBSTART_VAL);
                if (IMPDRV_EC_OK != ercd)
                {
                    break;
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        /** Write the Memory Local Working Memory Register for Initialization LWM */
        ercd = impdrv_osdep_write_reg(&p_corectl->device_io, IMPDRV_REG_OCV_LWM,
                                        IMPDRV_OCV_8KBNEXT_VAL, true, IMPDRV_OCV_8KBNEXT_VAL);
        if (IMPDRV_EC_OK == ercd)
        {
            offset = IMPDRV_OCV_OFST_START_VAL;
            while (offset < IMPDRV_OCV_OFST_END_VAL)
...
/***********************************************************************************************************************
* End of function impdrv_ocvctl_init_lwm()
***********************************************************************************************************************/
