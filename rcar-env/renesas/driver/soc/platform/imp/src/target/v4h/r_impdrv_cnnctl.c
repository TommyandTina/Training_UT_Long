...
***********************************************************************************************************************/
IMPDRV_STATIC
e_impdrv_errorcode_t impdrv_cnnctl_force_reset(
    void
);
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ercd = impdrv_cnnctl_force_reset();
    }
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ercd = impdrv_osdep_pm_get_state(&device_io, core_info, &pmstate);
    }

    /** Force reset */
    if (IMPDRV_EC_OK == ercd)
    {
        if ((IMPDRV_PM_STATE_READY == pmstate) || (IMPDRV_PM_STATE_RESET == pmstate))
        {
            ercd = impdrv_cnnctl_force_reset();
        }
    }
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        all_mask_val = IMPDRV_CNN_SR_TRAP | IMPDRV_CNN_SR_IER | IMPDRV_CNN_SR_INT
                    | IMPDRV_CNN_SR_MSCO | IMPDRV_CNN_SR_WUPCOVF | IMPDRV_CNN_SR_CLBRK;
...
/***********************************************************************************************************************
* Start of function impdrv_cnnctl_force_reset()
***********************************************************************************************************************/
IMPDRV_STATIC
e_impdrv_errorcode_t impdrv_cnnctl_force_reset(
    void
)
{
    ...
    /** Get imp_reg_spmc_00 device handle */
    ercd = impdrv_cmnctl_reg_get_handle("imp_reg_spmc_00", &device_handle);

    /** Force RESET for IP */
    if (IMPDRV_EC_OK == ercd)
    {
        ercd = impdrv_osdep_read_reg(&device_handle, 0x1F04, &read_Data);
    }
    
    if (IMPDRV_EC_OK == ercd)
    {
        write_Data = read_Data | 0x1;
        ercd = impdrv_osdep_write_reg(&device_handle, 0x1F04, write_Data, false, 0U);
    }
    
    /* Check Forced Reset for IP STatuS */
    if (IMPDRV_EC_OK == ercd)
    {
        for (i = 0U; i < 1000U; i++)
        {
            ercd = impdrv_osdep_read_reg(&device_handle, 0x1F0C, &read_Data);
            if (IMPDRV_EC_OK != ercd)
            {
                loop_end = true;
            }
            else if (1U == (read_Data & 0x1U))
            {
                loop_end = true;
                read_ok = true;
            }
    ...
    /* Forced Reset CLeaR for IP */
    if (IMPDRV_EC_OK == ercd)
    {
        ercd = impdrv_osdep_read_reg(&device_handle, 0x1F08, &read_Data);
    }

    if (IMPDRV_EC_OK == ercd)
    {
        write_Data = read_Data | 0x1;
        ercd = impdrv_osdep_write_reg(&device_handle, 0x1F08, write_Data, false, 0U);
    }
...
/***********************************************************************************************************************
* End of function impdrv_cnnctl_force_reset()
***********************************************************************************************************************/
