...
***********************************************************************************************************************/
#define IMPDRV_CNN_VCR              (0x00040100U)       /**< Hardware version value for CNN. */
/*******************************************************************************************************************//**
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        if (NULL == p_coretypectl->core_ctl[core_num].device_io.handle)
        {
...
/***********************************************************************************************************************
* Start of function impdrv_cnnctl_sync_stop()
* CD_PD_02_04_5061
* [Covers: UD_PD_UD02_04_0061]
***********************************************************************************************************************/
e_impdrv_errorcode_t impdrv_cnnctl_sync_stop(
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        p_coretypectl = &coretype_ctl[IMPDRV_CORE_TYPE_CNN];
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        if (IMPDRV_EC_OK != ercd)
        {
            (void)impdrv_cnnctl_set_state(&p_coretypectl->core_ctl[core_num], IMPDRV_STATE_READY);
        }
    }
...
/***********************************************************************************************************************
* End of function impdrv_cnnctl_sync_stop()
***********************************************************************************************************************/
...
/***********************************************************************************************************************
* Start of function impdrv_cnnctl_sync_pow_off()
* CD_PD_02_04_5062
* [Covers: UD_PD_UD02_04_0062]
***********************************************************************************************************************/
e_impdrv_errorcode_t impdrv_cnnctl_sync_pow_off(
    st_impdrv_coretypectl_t coretype_ctl[IMPDRV_CORE_TYPE_MAX]
)
{
    /** initialize local variables */
    e_impdrv_errorcode_t    ercd = IMPDRV_EC_OK;
    bool                    is_pow_supply = true;
    st_impdrv_corectl_t     *p_core_ctl_cnn;
    ...
    /** V4M's SDMAC controls CNN's power supply. */
    /** Check the state of SDMAC */
    if (IMPDRV_EC_OK == ercd)
    {
        uint32_t    max_core_num;
        uint32_t    sdmac_core_num;

        max_core_num = impdrv_dmasctl_max_core();

        for (sdmac_core_num = 0; sdmac_core_num < max_core_num; sdmac_core_num++)
        {
            if ((IMPDRV_STATE_INIT != coretype_ctl[IMPDRV_CORE_TYPE_DMAC_SLIM].core_ctl[sdmac_core_num].core_state) &&
                (IMPDRV_STATE_UNINIT != coretype_ctl[IMPDRV_CORE_TYPE_DMAC_SLIM].core_ctl[sdmac_core_num].core_state))
            {
                is_pow_supply = false;
                break;
            }
     ...
     /** Check the state of CNN */
    if ((IMPDRV_EC_OK == ercd) && (true == is_pow_supply))
    {
        p_core_ctl_cnn = &(coretype_ctl[IMPDRV_CORE_TYPE_CNN].core_ctl[0]);
        if ((IMPDRV_STATE_INIT != p_core_ctl_cnn->core_state) && (IMPDRV_STATE_UNINIT != p_core_ctl_cnn->core_state))
        {
            is_pow_supply = false;
        }
    }

    if (true == is_pow_supply)
    {
        /** Soft reset */
        if (IMPDRV_EC_OK == ercd)
        {
            ercd = impdrv_cnnctl_soft_reset(p_core_ctl_cnn);
        }

        /** Release for Power manager control */
        if (IMPDRV_EC_OK == ercd)
        {
            ercd = impdrv_osdep_pow_off_imp(&(p_core_ctl_cnn->device_io));
        }
    }
...
/***********************************************************************************************************************
* End of function impdrv_cnnctl_sync_pow_off()
***********************************************************************************************************************/
