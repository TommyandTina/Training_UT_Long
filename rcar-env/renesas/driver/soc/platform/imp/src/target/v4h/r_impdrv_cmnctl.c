...
* @param[out]       p_use_dsp_core_info Use DSP core information.
* @return           Return value
* @retval           IMPDRV_EC_OK                    Success completion
...
e_impdrv_errorcode_t impdrv_cmnctl_chk_init_data(
    ...
    uint32_t                        *const p_use_dsp_core_info
);
...
)
{
    ...
    uint32_t                        use_dsp_core_info = 0U;
    uint32_t                        write_val = 0U;
    uint32_t                        read_val = 0U;
    st_impdrv_core_info_t           core_info = {IMPDRV_CORE_TYPE_DTA, 0U};
    bool                            l_open_imptop = false;

    /** Input parameter confirmation process */
    ercd = impdrv_cmnctl_chk_init_data(p_commonctl, p_cmn_init_data,
                    p_is_dmac_mb_init, &use_core_info, &use_dsp_core_info);
    ...
    if ((IMPDRV_EC_OK == ercd) && (false != l_open_imptop))
    {
    ...
    if (NULL != p_commonctl)
    {
        /** Hold init parameter for later uses */
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        p_commonctl->use_dsp_core_ins = use_dsp_core_info;
        ...
        p_commonctl->irq_priority = p_cmn_init_data->irq_priority;
        p_commonctl->osal_cb_args = p_cmn_init_data->osal_cb_args;
    }
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        p_commonctl->use_dsp_core_ins = 0U;
    }
...
)
{
    ...
    return impdrv_dspctl_dsp_start_pre;
}
/***********************************************************************************************************************
* End of function impdrv_cmnctl_get_dsp_func()
***********************************************************************************************************************/
...
)
{
    ...
    {
        /** Store data */
        *p_irq_priority = p_commonctl->irq_priority;
        *p_osal_cb_args = p_commonctl->osal_cb_args;
    }
...
)
{
    ...
    e_impdrv_errorcode_t            ercd;
    uint32_t                        i;
    st_impdrv_commonctl_t          *p_commonctl_ins;
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        for (i = 0U; p_cmn_init_data->use_core_num > i; i++)
        {
            if (IMPDRV_CORE_TYPE_DSP == p_cmn_init_data->p_core_info[i].core_type)
            {
                *p_use_dsp_core_info |= ((uint32_t)1U << p_cmn_init_data->p_core_info[i].core_num);
            }
        }
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        for (i = 0U; (IMPDRV_INSTANCETABLE_NUM - 1U) > i; i++)
        {
            p_commonctl_ins = g_impdrv_cmn_ctl_ins.p_commonctl[s_impdrv_cnv_ins_table[i]];
            if (NULL != p_commonctl_ins)
            {
                if ((0U != (*p_use_core_info & p_commonctl_ins->use_core_ins)) ||
                    (0U != (*p_use_dsp_core_info & p_commonctl_ins->use_dsp_core_ins)))
                {
                    ercd = IMPDRV_EC_NG_INSTANCE;
                }
            }
...
)
{
    ...
    else
    {
        ...
        while (i < p_irq_param->group_core_num)
        {
            ...
            if (IMPDRV_CORE_TYPE_DSP == p_irq_param->group_core_info[i].core_type)
            {
                ercd = IMPDRV_EC_NG_NOTSUPPORT;
            }
