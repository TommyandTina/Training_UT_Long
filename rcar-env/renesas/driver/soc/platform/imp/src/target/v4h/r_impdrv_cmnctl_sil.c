...
***********************************************************************************************************************/
/***********************************************************************************************************************
* File Name    : r_impdrv_cmnctl_sil.c
* Version      : 3.19.0
...
***********************************************************************************************************************/
#define IMPDRV_INTEN0_CORE_MAX      (18U)               /**< Maximum number of cores for the Interrupt group 1. */
/*******************************************************************************************************************//**
...
***********************************************************************************************************************/
#define IMPDRV_IRQCHTABLE_NUM       (3U)                /**< Maximum number of IRQ Channel table. */
/*******************************************************************************************************************//**
...
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_DSPnCT
 * Offset address of hardware register for DSP0CT.
***********************************************************************************************************************/
#define IMPDRV_REG_DSP0CT           (0x00004010U)       /**< Offset address of hardware register for DSP0CT. */
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_DSPnCT
 * Offset address of hardware register for DSP1CT.
***********************************************************************************************************************/
#define IMPDRV_REG_DSP1CT           (0x00004020U)       /**< Offset address of hardware register for DSP1CT. */
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_DSPnCT
 * Offset address of hardware register for DSP1CT.
***********************************************************************************************************************/
#define IMPDRV_REG_DSP2CT           (0x00004030U)       /**< Offset address of hardware register for DSP2CT. */
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_DSPnCT
 * Offset address of hardware register for DSP3CT.
***********************************************************************************************************************/
#define IMPDRV_REG_DSP3CT           (0x00004040U)       /**< Offset address of hardware register for DSP3CT. */
/*******************************************************************************************************************//**
 * @def IMPDRV_INTEN0_CORE_MAX
 * Maximum number of DSP cores.
***********************************************************************************************************************/
#define IMPDRV_DSPCORE_NUM          (4U)               /**< Maximum number of DSP cores. */
/*******************************************************************************************************************//**
 * @def IMPDRV_REG_DSPNCT_VAL
 * Initial value of interrupt enable register.
***********************************************************************************************************************/
#define IMPDRV_REG_DSPNCT_VAL       (0x0003FFFFU)      /**< The setting value of DSPnCT. */
/*******************************************************************************************************************//**
 * @def IMPDRV_DSP_CORE_NUM
 * Maximum number of DSP core.
***********************************************************************************************************************/
#define IMPDRV_DSP_CORE_NUM        (4U)               /**< Maximum number of SYNCC table. */

#define IMPDRV_DSP_VDSP_DEV_MAIN    (0u)                /**< Interrupt **/
/** @} */
...
typedef struct
{
    ...
    st_impdrv_device_handle_t   device_io_dsp[IMPDRV_DSP_CORE_NUM];     /**< OSAL device handle (DSP Core).         */
...
st_impdrv_commonctl_ins_t g_impdrv_cmn_ctl_ins =
{
    ...
    0,                                                  /**< mutex_time_period                      */
    { NULL },                                           /**< device_io_dta                          */
    { {NULL}, {NULL}, {NULL}, {NULL} },                         /**< device_io_dsp[IMPDRV_DSP_CORE_NUM]     */
    { NULL, NULL, NULL, NULL }                          /**< *p_commonctl[IMPDRV_INSTANCETABLE_NUM] */
};
...
)
{
    ...
    uint32_t                        dsp_num = 0U;
    st_impdrv_core_info_t           core_info = {IMPDRV_CORE_TYPE_DTA, 0U};
    ...
    /** Request osdep to power on DTA Core */
    if ((IMPDRV_EC_OK == ercd) && (NULL == g_impdrv_cmn_ctl_ins.device_io_dta.handle))
    {
        ercd = impdrv_osdep_pow_on_imp(&g_impdrv_cmn_ctl_ins.device_io_dta, core_info);
    }
    ...
    /** Request osdep to power on DSP Core */
    for (uint32_t i=0; i < p_cmn_init_data->use_core_num; i++)
    {
        if (IMPDRV_CORE_TYPE_DSP == p_cmn_init_data->p_core_info[i].core_type)
        {
            dsp_num = p_cmn_init_data->p_core_info[i].core_num;
            if ((IMPDRV_EC_OK == ercd)&& (NULL == g_impdrv_cmn_ctl_ins.device_io_dsp[dsp_num].handle))
            {
                ercd = impdrv_osdep_pow_on_imp_dsp(&g_impdrv_cmn_ctl_ins.device_io_dsp[dsp_num],
                                                p_cmn_init_data->p_core_info[i], IMPDRV_DSP_VDSP_DEV_MAIN);
            }
            if (IMPDRV_EC_OK == ercd)
            {
                ercd = impdrv_osdep_register_dsp_irq(&g_impdrv_cmn_ctl_ins.device_io_dsp[dsp_num],
                                                      dsp_num,
                                                      p_cmn_init_data->irq_priority,
                                                      p_cmn_init_data->osal_cb_args);
            }
            if (IMPDRV_EC_OK == ercd)
            {
                ercd = impdrv_osdep_pow_off_dsp(&g_impdrv_cmn_ctl_ins.device_io_dsp[dsp_num]);
            }
        }else
        {
            //Nothing to do 
    ...
    /** Set IMPSLV.DSPnCT for corresponding to interrupt mode */
    if (IMPDRV_EC_OK == ercd)
    {
        uint32_t    reg_addr_tbl[IMPDRV_DSPCORE_NUM] = {
            IMPDRV_REG_DSP0CT,
            IMPDRV_REG_DSP1CT,
            IMPDRV_REG_DSP2CT,
            IMPDRV_REG_DSP3CT
        ...
        while (i < IMPDRV_DSPCORE_NUM)
        {
            ercd = impdrv_osdep_write_reg(&g_impdrv_cmn_ctl_ins.device_io_imptop,
                            reg_addr_tbl[i], IMPDRV_REG_DSPNCT_VAL, true, IMPDRV_REG_DSPNCT_VAL);
            if (IMPDRV_EC_OK == ercd)
            {
                i++;
            }
...
)
{
    ...
    for (cnt=0; cnt < IMPDRV_DSP_CORE_NUM; cnt++)
    {
        /** Unregister DSP IRQ */
        if (IMPDRV_EC_OK == ercd)
        {
            if (NULL != g_impdrv_cmn_ctl_ins.device_io_dsp[cnt].handle)
            {
                ercd = impdrv_osdep_unregister_dsp_irq(&g_impdrv_cmn_ctl_ins.device_io_dsp[cnt],
                                                        cnt);
            }
        }
    }
    ...
    /** Power off DTA and IMPTOP */
    if (((IMPDRV_EC_OK == ercd)
        || (IMPDRV_EC_NG_SYSTEMERROR == ercd)
        || (IMPDRV_EC_NG_CHECKFAIL == ercd))
        && (0U == instance_num))
    {
        for (cnt=0; cnt < IMPDRV_DSP_CORE_NUM; cnt++)
        {
            if (NULL != g_impdrv_cmn_ctl_ins.device_io_dsp[cnt].handle)
            {
                ercd_sub = impdrv_osdep_dsp_close(&g_impdrv_cmn_ctl_ins.device_io_dsp[cnt]);
        ...
        ercd_sub = impdrv_osdep_pow_off_imp(&g_impdrv_cmn_ctl_ins.device_io_dta);
        if ((IMPDRV_EC_OK == ercd) && (IMPDRV_EC_OK != ercd_sub))
        {
            ercd = ercd_sub;
        }
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ...
        ercd = impdrv_cmnctl_cnv_int_bit_core(s_impdrv_inten0_cnv_table, IMPDRV_INTEN0_CORE_MAX,
...
)
{
     ...
     * However, V4H is does not support of the bus interface checking.
     */
...
)
{
    ...
    const st_impdrv_corectl_func_t s_corectl_func_tbl[IMPDRV_CORE_TYPE_MAX] =
    {
        ...
        {   impdrv_impctl_is_valid_core,    impdrv_impctl_check_state,      impdrv_impctl_set_state,
            ...
            NULL,                           impdrv_impctl_conf_reg_check,   impdrv_impctl_set_cl_brk_addr,
        ...
        {   impdrv_ocvctl_is_valid_core,    impdrv_ocvctl_check_state,      impdrv_ocvctl_set_state,
            ...
            NULL,                           impdrv_ocvctl_conf_reg_check,   impdrv_ocvctl_set_cl_brk_addr,
        ...
        {   impdrv_dmactl_is_valid_core,    impdrv_dmactl_check_state,      impdrv_dmactl_set_state,
            ...
            NULL,                           impdrv_dmactl_conf_reg_check,   impdrv_dmactl_set_cl_brk_addr,
        ...
        {   impdrv_dmasctl_is_valid_core,   impdrv_dmasctl_check_state,     impdrv_dmasctl_set_state,
            ...
            NULL,                           impdrv_dmasctl_conf_reg_check,  impdrv_dmasctl_set_cl_brk_addr,
        ...
        {   impdrv_pscctl_is_valid_core,    impdrv_pscctl_check_state,      impdrv_pscctl_set_state,
            ...
            NULL,                           impdrv_pscctl_conf_reg_check,   impdrv_pscctl_set_cl_brk_addr,
        ...
        {   impdrv_cnnctl_is_valid_core,    impdrv_cnnctl_check_state,      impdrv_cnnctl_set_state,
            ...
            NULL,                           impdrv_cnnctl_conf_reg_check,   impdrv_cnnctl_set_cl_brk_addr,
        ...
        {  impdrv_dspctl_is_valid_core,     impdrv_dspctl_check_state,      impdrv_dspctl_set_state,
           ...
           impdrv_dspctl_set_core_map,      impdrv_dspctl_set_cl,           impdrv_dspctl_set_irq_mask,
...
)
{
    ...
    const st_impdrv_syncc_table_t s_impdrv_core_to_syncc_table[IMPDRV_SYNCCTABLE_NUM] =
    {
        ...
        {   IMPDRV_CORE_TYPE_INVALID, 0U,         0xFFU   },
        ...
        {   IMPDRV_CORE_TYPE_DMAC_SLIM,    0U,         0x24U   },
        {   IMPDRV_CORE_TYPE_DMAC_SLIM,    1U,         0x25U   },
        {   IMPDRV_CORE_TYPE_DMAC_SLIM,    2U,         0x26U   },
        {   IMPDRV_CORE_TYPE_DMAC_SLIM,    3U,         0x27U   },
        {   IMPDRV_CORE_TYPE_PSCEXE,  0U,         0x30U   },
        {   IMPDRV_CORE_TYPE_CNN,     0U,         0x40U   },
        {   IMPDRV_CORE_TYPE_DSP,     0U,         0x50U   },
        {   IMPDRV_CORE_TYPE_DSP,     1U,         0x51U   },
        {   IMPDRV_CORE_TYPE_DSP,     2U,         0x52U   },
        {   IMPDRV_CORE_TYPE_DSP,     3U,         0x53U   },
...
)
{
    ...
    if ((IMPDRV_INSTANCE_3 == instance_num) ||
    ...
    else if ((IMPDRV_INSTANCE_0 != instance_num)
...
/***********************************************************************************************************************
* Start of function impdrv_cmnctl_get_io_dsp()
* CD_PD_02_03_XXXX
* [Covers: UD_PD_UD02_03_XXXX]
***********************************************************************************************************************/
...
)
{
    ...
    {
        /** Store the handle data */
        *p_handle = &g_impdrv_cmn_ctl_ins.device_io_dsp[core_num];
    }
...
/***********************************************************************************************************************
* End of function impdrv_cmnctl_get_io_dsp()
***********************************************************************************************************************/
...
)
{
     ...
     * For V4H, the Check specifications of distributor register is have not been decided.
     */
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        ercd = impdrv_cmnctl_cnv_int_core_bit(
            p_cmn_init_data->p_core_info,
            p_cmn_init_data->use_core_num,
            s_impdrv_inten0_cnv_table,
            IMPDRV_INTEN0_CORE_MAX,
            p_use_core_info
        );
    }
...
)
{
    ...
    if (IMPDRV_EC_OK == ercd)
    {
        *p_val = 0U;
        ...
        {
            hit = false;
            if (IMPDRV_CORE_TYPE_DSP != p_core_info[i].core_type)
            {
            ...
            else if (IMPDRV_CORE_TYPE_DSP == p_core_info[i].core_type)
            {
                hit = true;
            }
            if (false == hit)
...
)
{
    ...
    else
    {
        ...
        *p_use_core_num = 0U;
        ...
        if (0U == *p_use_core_num)
        {
            core_info[*p_use_core_num].core_type = IMPDRV_CORE_TYPE_DSP;
            core_info[*p_use_core_num].core_num = 0U;
            *p_use_core_num += 1U;
        }
    }
    return ercd;
