...
static void Device_stop_standby(struct test_params_t *tparam, struct test_result_t *tret);

static uint32_t testno;
static st_impdrv_coretypectl_t coretype_ctl_backup[IMPDRV_CORE_TYPE_MAX];
static void prepare_test_for_stop( st_impdrv_core_info_t *p_core_info, int index_for_dsp_stop);
...
int32_t it_Stop_Main(struct test_params_t *tparam, struct test_result_t *test_result)
{
	...
	Device_stop_standby(tparam, &tret);
...
static void Return_error(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	/* Fail to turn on power impdrv_osdep_pow_off_imp returns IMPDRV_EC_NG_SEQSTATE */
	...
	/* Fail to turn on power impdrv_osdep_pow_off_imp returns IMPDRV_EC_NG_ARGNULL */
	...
	/* Fail to turn on power impdrv_osdep_pow_off_imp returns IMPDRV_EC_NG_SYSTEMERROR */
	...
	/* Fail to turn on power impdrv_osdep_pow_off_imp returns IMPDRV_EC_NG_PMSYSTEMERROR */
...
static void Device_stop(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	for (uint32_t i = 0; i < p_initdata->use_core_num; i++)
	{
		...
		if ((IMPDRV_CORE_TYPE_DSP != p_core_info->core_type))  {
			...
			result = 0;
			it_stub_reset();
			prepare_test_for_stop(p_core_info, 0);
			ercd = R_IMPDRV_Stop(handle, p_core_info);
...
static void Device_stop_standby(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	for (uint32_t i = 0; i < p_initdata->use_core_num; i++)
	{
		...
		if ((IMPDRV_CORE_TYPE_DSP == p_core_info->core_type))  {
			...
			result = 0;
			it_stub_reset();
			prepare_test_for_stop(p_core_info, 1);
			set_state(p_initdata, IMPDRV_STATE_READY);
			coretype_ctl->core_ctl[core_num].dsp_info.is_executed = true;
			ercd = R_IMPDRV_Stop(handle, p_core_info);
			set_state(p_initdata, IMPDRV_STATE_INIT);
...
static void prepare_test_for_stop( st_impdrv_core_info_t *p_core_info, int index_for_dsp_stop)
{
	...
	{
		it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_CNN_CNF_CLR);
		it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_CNN_CNF_CLR);
		it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, 0);
		it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, 1);
		it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, 0);
	}
	else if (IMPDRV_CORE_TYPE_DSP == p_core_info->core_type)
	{
		if (index_for_dsp_stop == 0){
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_VDSP_GPIO_STOP);
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_VDSP_GPIO_STDBY);
			/* impdrv_dspctl_dsp_standby */
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_PSU_STATE_LSP);
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_DMBE_IDLE);
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_PSU_STATE_LSP|IMPDRV_REGVAL_PSU_STATE_SBY);
		}
		else if (index_for_dsp_stop == 1) {
			it_stub_set_impdrv_osdep_read_reg(IMPDRV_EC_NG_PARAM, read_cnt++, IMPDRV_REGVAL_VDSP_GPIO_STOP);
			// /* impdrv_dspctl_dsp_standby */
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_VDSP_GPIO_STDBY);
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_PSU_STATE_LSP);
			it_stub_set_impdrv_osdep_read_reg(OSAL_RETURN_OK, read_cnt++, IMPDRV_REGVAL_DMBE_IDLE);
