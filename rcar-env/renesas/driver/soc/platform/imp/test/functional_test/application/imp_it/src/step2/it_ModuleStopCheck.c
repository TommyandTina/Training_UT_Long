...
static void Return_error(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	ercd = R_IMPDRV_ModuleStopCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SEQSTATE, *tret, "(No.%d) impdrv_osdep_read_reg (memory protection setting) returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_SEQSTATE));
	...
	ercd = R_IMPDRV_ModuleStopCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_osdep_read_reg (memory protection setting) returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_ARGNULL));
	...
	ercd = R_IMPDRV_ModuleStopCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_osdep_read_reg (memory protection setting) returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_PARAM));
	...
	ercd = R_IMPDRV_ModuleStopCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_osdep_read_reg (memory protection setting) returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_SYSTEMERROR));
#else
    testno += 8;
#endif
