...
static void Return_error(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	/* Fail to set attribute for IMP0 impdrv_impctl_set_irq_mask returns IMPDRV_EC_NG_SEQSTATE */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_IMP, 0);
	it_stub_set_impdrv_impctl_set_irq_mask(IMPDRV_EC_NG_SEQSTATE);
	ercd = R_IMPDRV_AttrSetIrqMask(handle, p_core_info, irq_mask);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SEQSTATE, *tret, "(No.%d) impdrv_impctl_set_irq_mask returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_SEQSTATE));

	/* Fail to set attribute for OCV0 impdrv_ocvctl_set_irq_mask returns IMPDRV_EC_NG_ARGNULL */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_OCV, 0);
	it_stub_set_impdrv_ocvctl_set_irq_mask(IMPDRV_EC_NG_ARGNULL);
	ercd = R_IMPDRV_AttrSetIrqMask(handle, p_core_info, irq_mask);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_ocvctl_set_irq_mask returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_ARGNULL));

	/* Fail to set attribute for DMAC00 impdrv_dmactl_set_irq_mask returns IMPDRV_EC_NG_PARAM */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_DMAC, 0);
	it_stub_set_impdrv_dmactl_set_irq_mask(IMPDRV_EC_NG_PARAM);
	ercd = R_IMPDRV_AttrSetIrqMask(handle, p_core_info, irq_mask);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_dmactl_set_irq_mask returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_PARAM));

	/* Fail to set attribute for PSCEXE0 impdrv_pscctl_set_irq_mask returns IMPDRV_EC_NG_NOTSUPPORT */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_PSCEXE, 0);
	it_stub_set_impdrv_pscctl_set_irq_mask(IMPDRV_EC_NG_NOTSUPPORT);
	ercd = R_IMPDRV_AttrSetIrqMask(handle, p_core_info, irq_mask);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_NOTSUPPORT, *tret, "(No.%d) impdrv_pscctl_set_irq_mask returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_NOTSUPPORT));
...
static void Device_attr_set_irq_mask(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	memset(irq_mask, true, IMPDRV_IRQMASK_MAX * sizeof(irq_mask[0]));
