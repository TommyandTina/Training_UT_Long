	...
	for (uint32_t i = 0; i < p_initdata->use_core_num; i++)
	{
		...
		if (ercd != IMPDRV_EC_OK) {
			...
			if (coretype_ctl->core_ctl[core_num].core_state != IMPDRV_STATE_INIT) {
				...
				result = -1;
			} else if (coretype_ctl->core_ctl[core_num].core_mem_init_enable != enable) {
				core_testmsg(p_core_info, "core_mem_init_enable = %u, it is not %u(test NG)\n", coretype_ctl->core_ctl[core_num].core_mem_init_enable, enable);
				result = -1;
			} else {
				result = 0;
			}
