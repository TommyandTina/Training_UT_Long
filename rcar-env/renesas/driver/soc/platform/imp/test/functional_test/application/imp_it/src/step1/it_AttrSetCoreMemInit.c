#define TEST_MSG_NAME "R_IMPDRV_AttrSetCoreMemInit"
...
#endif
static void Device_attr_set_core_mem_init(struct test_params_t *tparam, struct test_result_t *tret);

static uint32_t testno;

int32_t it_AttrSetCoreMemInit_Main(struct test_params_t *tparam, struct test_result_t *test_result)
{
...
#endif
	Device_attr_set_core_mem_init(tparam, &tret);
...
static void Parameter_error(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	e_impdrv_param_t enable = IMPDRV_PARAM_ON;

	/* Parameter_error : handle == NULL */
	ercd = R_IMPDRV_AttrSetCoreMemInit(NULL, p_core_info, enable);
	FailTestPrint(ercd, IMPDRV_EC_NG_ARGNULL, *tret, "(No.%d) handle == NULL", testno++);

	/* Parameter_error : p_core_info == NULL */
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, NULL, enable);
	FailTestPrint(ercd, IMPDRV_EC_NG_ARGNULL, *tret, "(No.%d) p_core_info == NULL", testno++);

	/* Parameter_error : enable = (IMPDRV_PARAM_INVALID - 1) */
	enable = (IMPDRV_PARAM_INVALID - 1);
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, p_core_info, enable);
	FailTestPrint(ercd, IMPDRV_EC_NG_PARAM, *tret, "(No.%d) enable = (IMPDRV_PARAM_INVALID - 1)", testno++);

	/* Parameter_error : enable = IMPDRV_PARAM_INVALID */
	enable = IMPDRV_PARAM_INVALID;
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, p_core_info, enable);
	FailTestPrint(ercd, IMPDRV_EC_NG_PARAM, *tret, "(No.%d) enable = IMPDRV_PARAM_INVALID", testno++);

	/* Parameter_error : enable = (IMPDRV_PARAM_ON + 1) */
	enable = (IMPDRV_PARAM_ON + 1);
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, p_core_info, enable);
	FailTestPrint(ercd, IMPDRV_EC_NG_PARAM, *tret, "(No.%d) enable = (IMPDRV_PARAM_ON + 1)", testno++);
}
...
static void Invalid_core(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, &core_info, enable);
	it_stub_reset();
...
static void Return_error(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	/* Fail to set attribute for IMP0 impdrv_impctl_set_mem_init returns IMPDRV_EC_NG_SEQSTATE */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_IMP, 0);
	it_stub_set_impdrv_impctl_set_mem_init(IMPDRV_EC_NG_SEQSTATE);
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, p_core_info, enable);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SEQSTATE, *tret, "(No.%d) impdrv_impctl_set_mem_init returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_SEQSTATE));

	/* Fail to set attribute for OCV0 impdrv_ocvctl_set_mem_init returns IMPDRV_EC_NG_ARGNULL */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_OCV, 0);
	it_stub_set_impdrv_ocvctl_set_mem_init(IMPDRV_EC_NG_ARGNULL);
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, p_core_info, enable);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_ocvctl_set_mem_init returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_ARGNULL));

	/* Fail to set attribute for DMAC00 impdrv_dmactl_set_mem_init returns IMPDRV_EC_NG_PARAM */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_DMAC, 0);
	it_stub_set_impdrv_dmactl_set_mem_init(IMPDRV_EC_NG_PARAM);
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, p_core_info, enable);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_dmactl_set_mem_init returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_PARAM));

	/* Fail to set attribute for PSCEXE0 impdrv_pscctl_set_mem_init returns IMPDRV_EC_NG_NOTSUPPORT */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_PSCEXE, 0);
	it_stub_set_impdrv_pscctl_set_mem_init(IMPDRV_EC_NG_NOTSUPPORT);
	ercd = R_IMPDRV_AttrSetCoreMemInit(handle, p_core_info, enable);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_NOTSUPPORT, *tret, "(No.%d) impdrv_pscctl_set_mem_init returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_NOTSUPPORT));
...
static void Device_attr_set_core_mem_init(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	/* driver_attr_set_core_mem_init : output parameter */
