#define TEST_MSG_NAME "CoreExec_TRAP"
...
#define CALLBACK_CODE3	(0xAA)
#define CALLBACK_CODE4	(0xBB)
#define CALLBACK_CODE5	(0xCC)
...
static void *g_msgcnf[SYNC_MAX_NUM];
...
{
	return (send_msg(g_msgcnf[3], ercd) | send_msg(g_msgcnf[3], code));
}

static int32_t CbFunc_4(
	const st_impdrv_core_info_t *p_core_info,		/**< Core information					*/
	const e_impdrv_cb_ret_t		ercd,				/**< Error code							*/
	const int32_t				code,				/**< TRAP or INT code (8 bit is set)	*/
	const void					*p_callback_args
...
{
	return (send_msg(g_msgcnf[4], ercd) | send_msg(g_msgcnf[4], code));
}

static int32_t CbFunc_5(
	const st_impdrv_core_info_t *p_core_info,		/**< Core information					*/
	const e_impdrv_cb_ret_t		ercd,				/**< Error code							*/
	const int32_t				code,				/**< TRAP or INT code (8 bit is set)	*/
	const void					*p_callback_args
...
{
	return (send_msg(g_msgcnf[5], ercd) | send_msg(g_msgcnf[5], code));
}
...
static const callback_info_t csinfo[SYNC_MAX_NUM] = {
	...
	{ (p_impdrv_cbfunc_t)&CbFunc_3, CALLBACK_CODE3 },
	{ (p_impdrv_cbfunc_t)&CbFunc_4, CALLBACK_CODE4 },
	{ (p_impdrv_cbfunc_t)&CbFunc_5, CALLBACK_CODE5 }
};

int32_t it_CoreExec_TRAP_Main(struct test_params_t *tparam, struct test_result_t *test_result)
{
	struct test_result_t tret = {0};

	for (int32_t i = 0; i < SYNC_MAX_NUM; i++)
	{
...
static int32_t Core_Sync(struct test_params_t *tparam, struct test_result_t *tret, impdrv_ctrl_handle_t handle)
{
	...
	sync_core = get_sync_cores(CMD_6_CORES);
	if (NULL == sync_core)
	{
	...
	for (; (NULL != sync_core) && (0 != sync_core->num); sync_core++)
	{
		...
		for (i = 0; i < sync_core->num; i++)
		{
			...
			if (IMPDRV_EC_OK != ercd)
			{
				core_testmsg(&sync_core->core[i], "Set core_map fail %s\n", impdrv_err_to_name(ercd));
				result = -1;
				goto error_start;
			}
		}
		...
		for (i = 0; i < sync_core->num; i++)
		{
			if (set_trapsynccommandlist(sync_core->core[i].core_type, tparam->claddr[i], (uint32_t)csinfo[i].cb_val))
			{
				core_testmsg(&sync_core->core[i], "set_trapsynccommandlist fail\n");
				result = -1;
				goto error_setcl;
			}
			else
