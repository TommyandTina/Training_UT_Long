...
static const st_impdrv_core_info_t invalid_core[] = {
	...
	{ IMPDRV_CORE_TYPE_IMP_SLIM,	IMP_SLIM_CORE_MAX	},
...
static const st_impdrv_core_info_t soc_coremap[] = {
	...
	{ IMPDRV_CORE_TYPE_PSCOUT,	1 },
	{ IMPDRV_CORE_TYPE_PSCOUT,	2 },
	{ IMPDRV_CORE_TYPE_PSCOUT,	3 },
...
const uint32_t read_value_for_start[READ_NUM_FOR_START] = {
	...
	[23] = IMPDRV_OCV_RESET_STATUS_VAL,
	[26] = IMPDRV_OCV_RESET_STATUS_VAL,
	[29] = IMPDRV_OCV_RESET_STATUS_VAL,
...
static const st_impdrv_imp_write_info_tbl_t imps_transfer_config[] =
{
...
static const tp_test_pattern_t imps_test_pattern_tbl[] = {
	{ 0x55555555U, (sizeof(uint32_t) * 8U) },
	{ 0xAAAAAAAAU, (sizeof(uint32_t) * 8U) },
	{ 0x33333333U, (sizeof(uint32_t) * 8U) },
	{ 0xCCCCCCCCU, (sizeof(uint32_t) * 8U) },
...
)
{
	...
	switch (cmd)
	{
		case IT_IMPDRV_CMD_DMA_TRANSFER:
		{
			...
			{
				data_info.range	= ARRAY_SIZE(imps_transfer_config);
				data_info.info	= (void *)imps_transfer_config;
			}
		...
		case IT_IMPDRV_CMD_BUSIF_TEST_PATTERN:
		{
			...
			{
				data_info.range	= ARRAY_SIZE(imps_test_pattern_tbl);
				data_info.info	= (void *)imps_test_pattern_tbl;
			}
			else if (IMPDRV_CORE_TYPE_DMAC == core_type)
			{
		...
		{
			if ((IMPDRV_CORE_TYPE_IMP == core_type) || (IMPDRV_CORE_TYPE_IMP_SLIM == core_type))
			{
...
static sync_core_t sync_core[] = {
	...
	list_3element(IMP, 4,	IMP_SLIM,	0,	OCV,		2),
	list_3element(OCV, 3,	DMAC,   	2,	DMAC_SLIM,	0),
	list_3element(OCV, 2,	DMAC,   	3,	DMAC_SLIM,	1),
	list_2element(OCV, 4,	DMAC,   	3),
	list_endelement(0)
};

static sync_core_t sync_6_cores[] = {
	list_6element(OCV, 0,	OCV, 1,		OCV, 2,		OCV, 3,			OCV, 4,			IMP, 0),
	list_6element(IMP, 1,	IMP, 2,		IMP, 3,		IMP, 4,			IMP_SLIM, 0,	PSCEXE, 0),
