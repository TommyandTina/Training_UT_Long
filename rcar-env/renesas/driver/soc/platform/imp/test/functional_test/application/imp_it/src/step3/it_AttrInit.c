			...
			if (coretype_ctl->core_ctl[core_num].core_state != IMPDRV_STATE_INIT) {
				...
				if (result == 0) {
					for (uint32_t j = 0; j < IMPDRV_EXEPARAM_MAX; j++)
					{
						...
						{
							if (coretype_ctl->core_ctl[core_num].param[0] != 0) {
								core_testmsg(p_core_info, "core_type = %s, param[%d] = %u, it is not 4294967295 (test NG)\n",
															type_to_name(core_type),
															j,
							...
							if (coretype_ctl->core_ctl[core_num].param[0] != 4294967295U) {
								core_testmsg(p_core_info, "core_type = %s, param[%d] = %u, it is not 4294967295 (test NG)\n",
															type_to_name(core_type),
															j,
															coretype_ctl->core_ctl[core_num].param[j]
						...
						if (coretype_ctl->core_ctl[core_num].param[1] != 0U) {
						...
						if (coretype_ctl->core_ctl[core_num].param[2] != 0U) {
							core_testmsg(p_core_info, "core_type = %s, param[%d] = %u, it is not 0 (test NG)\n",
														type_to_name(core_type),
														j,
														coretype_ctl->core_ctl[core_num].param[j]
						...
						if (coretype_ctl->core_ctl[core_num].param[3] != 0U) {
							core_testmsg(p_core_info, "core_type = %s, param[%d] = %u, it is not 0 (test NG)\n",
														type_to_name(core_type),
														j,
														coretype_ctl->core_ctl[core_num].param[j]
