...
static void prepare_test()
{
	for (uint32_t i = 0; i < ARRAY_SIZE(read_value_for_start); i++)
	{
		it_stub_set_impdrv_osdep_read_reg(IMPDRV_EC_OK, i, read_value_for_start[i]);
	}
}
...
static void Return_error(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	/* Fail to write register impdrv_osdep_write_reg returns IMPDRV_EC_NG_CHECKFAIL */
	...
	/* Fail to reset core for OCV0 impdrv_osdep_reset_core returns IMPDRV_EC_NG_SEQSTATE */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_OCV, 0);
	it_stub_set_impdrv_osdep_read_reg(IMPDRV_EC_OK, 0, IMPDRV_OCV_RESET_STATUS_VAL);
	it_stub_set_impdrv_osdep_reset_core(IMPDRV_EC_NG_SEQSTATE);
	ercd = R_IMPDRV_Start(handle, p_core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SEQSTATE, *tret, "(No.%d) impdrv_osdep_reset_core returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_SEQSTATE));

	/* Fail to reset core for OCV0 impdrv_osdep_reset_core returns IMPDRV_EC_NG_ARGNULL */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_OCV, 0);
	it_stub_set_impdrv_osdep_read_reg(IMPDRV_EC_OK, 0, IMPDRV_OCV_RESET_STATUS_VAL);
	it_stub_set_impdrv_osdep_reset_core(IMPDRV_EC_NG_ARGNULL);
	ercd = R_IMPDRV_Start(handle, p_core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_osdep_reset_core returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_ARGNULL));

	/* Fail to reset core for OCV0 impdrv_osdep_reset_core returns IMPDRV_EC_NG_PMSYSTEMERROR */
	p_core_info = get_core_info(p_initdata->core_info, p_initdata->use_core_num, IMPDRV_CORE_TYPE_OCV, 0);
	it_stub_set_impdrv_osdep_read_reg(IMPDRV_EC_OK, 0, IMPDRV_OCV_RESET_STATUS_VAL);
	it_stub_set_impdrv_osdep_reset_core(IMPDRV_EC_NG_PMSYSTEMERROR);
	ercd = R_IMPDRV_Start(handle, p_core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_PMSYSTEMERROR, *tret, "(No.%d) impdrv_osdep_reset_core returns %s", testno++, impdrv_err_to_name(IMPDRV_EC_NG_PMSYSTEMERROR));
