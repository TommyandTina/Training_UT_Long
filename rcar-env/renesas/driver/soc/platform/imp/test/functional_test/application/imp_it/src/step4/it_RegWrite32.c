...
static void Invalid_state(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	/* prepare parameters */
	(void)R_IMPDRV_RegGetHwInfo(handle, device_id, &reg_info);
	offset = reg_info.addr_phys - OFFSETADDR_SPMI0;
	(void)R_IMPDRV_RegRequired(handle, device_id, IMPDRV_REG_REQ_STATE_REQUIRED);
...
static void Device_reg_write32(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	{
		testmsg("(No.%d) RegGetHwInfo fail(test NG %d)\n", testno++, ercd);
		result = -1;
	}
	else
	{
		...
		if (ercd != IMPDRV_EC_OK)
		{
			testmsg("(No.%d) RegRequired fail(test NG %d)\n", testno++, ercd);
			result = -1;
		}
		else
		{
