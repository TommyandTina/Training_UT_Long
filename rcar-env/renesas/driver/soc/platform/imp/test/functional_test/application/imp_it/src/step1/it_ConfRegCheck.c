...
static void Return_error(struct test_params_t *tparam, struct test_result_t *tret)
{
	...
	/* Fail to execute Configuration Register Check for IMP0 impdrv_impctl_conf_reg_check returns IMPDRV_EC_NG_SEQSTATE */
	it_stub_set_impdrv_impctl_conf_reg_check(IMPDRV_EC_NG_SEQSTATE);
	ercd = R_IMPDRV_ConfRegCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_CHECKFAIL, *tret, "(No.%d) impdrv_impctl_conf_reg_check for %s%d returns %s", testno++, type_to_name(core_info.core_type), core_info.core_num, impdrv_err_to_name(IMPDRV_EC_NG_SEQSTATE));

	/* Fail to execute Configuration Register Check for IMP0 impdrv_impctl_conf_reg_check returns IMPDRV_EC_NG_ARGNULL */
	it_stub_set_impdrv_impctl_conf_reg_check(IMPDRV_EC_NG_ARGNULL);
	ercd = R_IMPDRV_ConfRegCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_impctl_conf_reg_check for %s%d returns %s", testno++, type_to_name(core_info.core_type), core_info.core_num, impdrv_err_to_name(IMPDRV_EC_NG_ARGNULL));

	/* Fail to execute Configuration Register Check for IMP0 impdrv_impctl_conf_reg_check returns IMPDRV_EC_NG_PARAM */
	it_stub_set_impdrv_impctl_conf_reg_check(IMPDRV_EC_NG_PARAM);
	ercd = R_IMPDRV_ConfRegCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_impctl_conf_reg_check for %s%d returns %s", testno++, type_to_name(core_info.core_type), core_info.core_num, impdrv_err_to_name(IMPDRV_EC_NG_PARAM));

	/* Fail to execute Configuration Register Check for IMP0 impdrv_impctl_conf_reg_check returns IMPDRV_EC_NG_SYSTEMERROR */
	it_stub_set_impdrv_impctl_conf_reg_check(IMPDRV_EC_NG_SYSTEMERROR);
	ercd = R_IMPDRV_ConfRegCheck(handle, p_chk_param, param_num, &core_info);
	it_stub_reset();
	FailTestPrint(ercd, IMPDRV_EC_NG_SYSTEMERROR, *tret, "(No.%d) impdrv_impctl_conf_reg_check for %s%d returns %s", testno++, type_to_name(core_info.core_type), core_info.core_num, impdrv_err_to_name(IMPDRV_EC_NG_SYSTEMERROR));
	...
	data_info = get_data_info(core_info.core_type, IT_IMPDRV_CMD_CONF_REG_CHECK);
