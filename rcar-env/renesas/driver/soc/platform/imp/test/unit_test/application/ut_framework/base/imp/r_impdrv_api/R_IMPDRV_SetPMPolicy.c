#include "base.h"
#include "util.h"
#include "patterns/R_IMPDRV_SetPMPolicy_PCL.h"

static bool TEST_Initialize_R_IMPDRV_SetPMPolicy(int32_t no)
{
	bool b = true;
	struct R_IMPDRV_SetPMPolicy_input *inputs = &R_IMPDRV_SetPMPolicy_PCL[no - 1].input;
...
static bool TEST_Validate_R_IMPDRV_SetPMPolicy(struct R_IMPDRV_SetPMPolicy_expect *outputs, struct R_IMPDRV_SetPMPolicy_expect *expected, bool validator[])
{
	bool b = true;
...
{
	bool b = true;
	struct TestParams *params = TEST_CreateParam("R_IMPDRV_SetPMPolicy", category, no);
	struct TEST_R_IMPDRV_SetPMPolicy_Pattern *pattern = &R_IMPDRV_SetPMPolicy_PCL[no - 1];
	struct R_IMPDRV_SetPMPolicy_input	*inputs = &pattern->input;
	struct R_IMPDRV_SetPMPolicy_params	func_params = {0};
	struct R_IMPDRV_SetPMPolicy_expect	outputs = {0};
	bool is_stub = !inputs->R_OSAL_ThsyncMutexLockForTimePeriod.is_stub | !inputs->R_OSAL_ThsyncMutexUnlock.is_stub | !inputs->R_OSAL_IoDeviceOpen.is_stub | !inputs->R_OSAL_PmSetPolicy.is_stub | !inputs->R_OSAL_PmSetRequiredState.is_stub | !inputs->R_OSAL_IoDeviceClose.is_stub;

	ut_init_imp(is_stub);

	b &= TEST_Initialize_R_IMPDRV_SetPMPolicy(no);
	...
	memcpy(&func_params.policy, &inputs->policy, sizeof(func_params.policy));
	...
	if (inputs->R_OSAL_PmSetPolicy.is_stub)
	{
		memcpy(&g_R_OSAL_PmSetPolicy.ReturnValue, &inputs->R_OSAL_PmSetPolicy.ReturnValue, sizeof(inputs->R_OSAL_PmSetPolicy.ReturnValue));
		g_hook_R_OSAL_PmSetPolicy = ut_stub_R_OSAL_PmSetPolicy;
	}
	if (inputs->R_OSAL_PmSetRequiredState.is_stub)
	{
		memcpy(&g_R_OSAL_PmSetRequiredState.ReturnValue, &inputs->R_OSAL_PmSetRequiredState.ReturnValue, sizeof(inputs->R_OSAL_PmSetRequiredState.ReturnValue));
		g_hook_R_OSAL_PmSetRequiredState = ut_stub_R_OSAL_PmSetRequiredState;
	}
	...
	outputs.ercd = R_IMPDRV_SetPMPolicy(func_params.handle, func_params.p_core_info, func_params.policy);

	b &= TEST_Validate_R_IMPDRV_SetPMPolicy(&outputs, &pattern->expected, NULL);
