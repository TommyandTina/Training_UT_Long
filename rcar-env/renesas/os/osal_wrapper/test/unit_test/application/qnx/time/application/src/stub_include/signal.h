...
typedef void __sa_handler_func_t(int);

/* <STAN_MACRO1, STAN_MACRO5/> */
#define __SIGACTION_FIELDS(__handler_type, __action_type) \
    union {											\
...
struct __sigaction32 {
	__SIGACTION_FIELDS(_Uintptr32t, _Uintptr32t)
};

struct __sigaction64 {
	__SIGACTION_FIELDS(_Uintptr64t, _Uintptr64t)
};

struct sigaction {
	__SIGACTION_FIELDS(__sa_handler_func_t *, __sa_action_func_t *)
};

# define sa_handler     __sa_un._sa_handler
# define sa_sigaction   __sa_un._sa_sigaction

#define SIG_ERR             ((__sa_handler_func_t *) -1)
#define SIG_DFL             ((__sa_handler_func_t *)  0)
#define SIG_IGN             ((__sa_handler_func_t *)  1)
#define SIG_HOLD            ((__sa_handler_func_t *)  2)
...
# define NSIG        _NSIG  /* valid user signals range from 1 to NSIG-1 */
# define MAXSIG      64     /* Signals from NSIG to MAXSIG are reserved. */
#endif
...
#if defined(__EXT_QNX)
#define SA_FLUSH		0x0080  /* flush pending signal of this type from thread (Neutrino-specific)*/
#endif
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/c/public/signal.h $ $Rev: 893347 $")
#endif
