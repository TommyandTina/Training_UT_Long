...
/* profile support indicator macros */
/* PSE51 */
...
/* PSE54 */
# define    _POSIX_AEP_REALTIME_MULTI          200312L

#define        _POSIX_AEP_REALTIME_LANG_C99    (1)

#endif /* defined(__EXT_POSIX1_200112) */
...
# ifndef SETIOV_CONST
/* <STAN_MACRO1> */
#  define SETIOV_CONST(__iov, __addr, __len)          \
    *(__iov) = (iov_t){ .iov_base_const = (__addr), .iov_len = (__len) }
/* </STAN_MACRO1> */
# endif
# ifndef SETIOV
/* <STAN_MACRO1> */
...
#ifdef __EXT_LF64SRC
extern off64_t lseek64(int __fildes, off64_t __offset, int __whence) __ALIASOFF("lseek");
extern int ftruncate64(int __fd, off64_t __length) __ALIASOFF("ftruncate");
extern ssize_t pread64(int __filedes, void *__buff, size_t __nbytes, off64_t __offset) __ALIASOFF("pread");
extern ssize_t pwrite64(int __filedes, const void *__buff, size_t __nbytes, off64_t __offset) __ALIASOFF("pwrite");
# if defined(__EXT_QNX)
extern off64_t ltrunc64(int __fd, off64_t __offset, int __whence) __ALIASOFF("ltrunc");
# endif
# if defined(__EXT_XOPEN_EX)
extern int truncate64(const char *__path, off64_t __length) __ALIASOFF("truncate");
extern int lockf64(int __fd, int __function, off64_t __size) __ALIASOFF("lockf");
# endif
#if defined(__EXT_LF64ALIAS) && !defined(__ALIAS_ATTRIBUTE)
#error ALIAS not configured for compiler: lseek,ftruncate,pread,pwrite,tell,truncate,flockf
#endif
#endif
...
extern unsigned alarm(unsigned __seconds);
...
// extern ssize_t pread(int __filedes, void *__buff, size_t __nbytes, off_t __offset) __ALIAS64("pread64");
// extern ssize_t pwrite(int __filedes, const void *__buff, size_t __nbytes, off_t __offset) __ALIAS64("pwrite64");
...
extern int fchownat(int __fd, const char *__path, uid_t __uid, gid_t __gid, int __flags);
extern int readlinkat(int __dirfd, const char *__pathname, char *__buf, size_t __bufsize);
extern int faccessat(int dirfd, const char * const path, int amode, int flags);
extern int unlinkat(int dirfd, const char * const path, int flags);
extern int symlinkat(const char *const psymlink, int dirfd, const char * const path);
extern int linkat(int olddirfd, const char *const oldpath,
				  int newdirfd, const char *const newpath,
				  int flags);
#endif
...
#   pragma redefine_extname ltrunc ltrunc64
#  endif
#  if defined(__EXT_XOPEN_EX)
...
extern char *getwd(char *__pathname);
...
extern void swab(const void *__restrict __src, void *__restrict __dest, ssize_t __num);
...
extern int setgroupspid(int __gidsetsize, const gid_t *__grouplist, pid_t pid);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/c/public/unistd.h $ $Rev: 916291 $")
#endif
