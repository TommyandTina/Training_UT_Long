...
#define ETHER_PROTOCOL_EVENT_PKT_AVAIL_CH1               ((TAURUS_PROTOCOL_ETHER_ID << 24) | 0x000001)
...
struct taurus_ether_init_in {
    uint64_t cookie;
    uint32_t ulStartOfDbToc;
    uint8_t  ucEthMACAddr[ETH_MACADDR_SIZE];
    uint8_t  ucEthSpeed;
    uint8_t  ucEthDuplex;
    uint8_t  ucEthMagicDection;
    /* TODO How to implement callback    P2FUNC(void, ETH_APPL_CODE, pMagicNotification)(void); */
    uint8_t  ucEthPHYInterface;
    uint8_t  ucStartOfPayloadCrcArea;
