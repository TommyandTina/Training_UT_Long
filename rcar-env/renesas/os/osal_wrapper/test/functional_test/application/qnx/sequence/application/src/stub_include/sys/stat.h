/*
 * $QNXLicenseC:
 * Copyright 2007-2019, QNX Software Systems. All Rights Reserved.
 *
 * You must obtain a written license from and pay applicable license fees to QNX
 * Software Systems before you may reproduce, modify or distribute this software,
 * or any work that includes all or part of this software.   Free development
 * licenses are available for evaluation and non-commercial purposes.  For more
 * information visit http://licensing.qnx.com or email licensing@qnx.com.
 *
 * This file may contain contributions from others.  Please review this entire
 * file for other proprietary rights or license notices, as well as the QNX
 * Development Suite License Guide at http://licensing.qnx.com/license-guide/
 * for other information.
 * $
 */

#ifndef __STAT_H_INCLUDED
#define __STAT_H_INCLUDED
#define __STAT_H_DECLARED

#ifndef __PLATFORM_H_INCLUDED
#include <sys/platform.h>
#endif

#if defined(__MODE_T)
typedef __MODE_T	mode_t;
#undef __MODE_T
#endif

#if defined(__DEV_T)
typedef __DEV_T		dev_t;
#undef __DEV_T
#endif

#if defined(__TIME_T)
typedef __TIME_T	time_t;
#undef __TIME_T
#endif

#if defined(__NLINK_T)
typedef __NLINK_T	nlink_t;
#undef __NLINK_T
#endif

#if defined(__OFF_T)
typedef __OFF_T		off_t;
#undef __OFF_T
#endif

#if defined(__OFF64_T)
typedef __OFF64_T	off64_t;
#undef __OFF64_T
#endif

#if defined(__INO_T)
typedef __INO_T		ino_t;
#undef __INO_T
#endif

#if defined(__INO64_T)
typedef __INO64_T	ino64_t;
#undef __INO64_T
#endif

#if defined(__UID_T)
typedef __UID_T		uid_t;
#undef __UID_T
#endif

#if defined(__GID_T)
typedef __GID_T		gid_t;
#undef __GID_T
#endif

#if defined(__BLKSIZE_T)
typedef __BLKSIZE_T			blksize_t;
#undef __BLKSIZE_T
#endif

#if defined(__BLKCNT64_T)
typedef __BLKCNT64_T		blkcnt64_t;
#undef __BLKCNT64_T
#endif

#if defined(__FSBLKCNT64_T)
typedef __FSBLKCNT64_T		fsblkcnt64_t;
#undef __FSBLKCNT64_T
#endif

#if defined(__FSFILCNT64_T)
typedef __FSFILCNT64_T		fsfilcnt64_t;
#undef __FSFILCNT64_T
#endif

#if defined(__BLKCNT_T)
typedef __BLKCNT_T			blkcnt_t;
#undef __BLKCNT_T
#endif

#if defined(__FSBLKCNT_T)
typedef __FSBLKCNT_T		fsblkcnt_t;
#undef __FSBLKCNT_T
#endif

#if defined(__FSFILCNT_T)
typedef __FSFILCNT_T		fsfilcnt_t;
#undef __FSFILCNT_T
#endif

#if defined(__EXT_QNX)
#if defined(__PID_T)
typedef __PID_T		pid_t;
#undef __PID_T
#endif
#endif

#if defined(__EXT_POSIX1_200809)
#if defined(__TIMESPEC)
__TIMESPEC;
#undef __TIMESPEC
#endif
#endif
#if defined(__TIMESPEC_INTERNAL)
__TIMESPEC_INTERNAL;
#undef __TIMESPEC_INTERNAL
#endif

// #include <_pack64.h>

/* <STAN_MACRO1, STAN_MACRO5> */
#define __STAT_FIELDS(__fs, __ts, __pv) \
	_FILLTO64_##__fs(ino, st_ino)	/* File serial number.					*/	\
	_FILLTO64_##__fs(off, st_size)												\
	dev_t			st_dev;			/* ID of device containing file.		*/	\
	dev_t			st_rdev;		/* Device ID, for inode that is device	*/	\
	uid_t			st_uid;														\
	gid_t			st_gid;														\
	_OLD_TS_##__pv(st_mtime)		/* Time of last data modification		*/	\
	_OLD_TS_##__pv(st_atime)		/* Time last accessed					*/	\
	_OLD_TS_##__pv(st_ctime)		/* Time of last status change			*/	\
	mode_t			st_mode;		/* see below							*/	\
	nlink_t			st_nlink;													\
	blksize_t		st_blocksize;	/* Size of a block used by st_nblocks   */	\
	_Int32t			st_nblocks;		/* Number of blocks st_blocksize blocks */	\
	blksize_t		st_blksize;		/* Prefered I/O block size for object   */	\
	_FILLTO64_##__fs(blkcnt, st_blocks)/* Number of 512 byte blocks		*/	\
	_NEW_TS_##__ts##__pv(st_mtim)	/* Time of last data modification		*/	\
	_NEW_TS_##__ts##__pv(st_atim)	/* Time last accessed					*/	\
	_NEW_TS_##__ts##__pv(st_ctim)	/* Time of last status change			*/	\


#if __OFF_BITS__ == 64
#  define _FILLTO64_32(__type, __field) __type##_t __field;
#elif __OFF_BITS__ == 32
# if defined(__LITTLEENDIAN__)
#  define _FILLTO64_32(__type, __field) __type##_t __field; __type##_t __field##_hi;
# elif defined(__BIGENDIAN__)
#  define _FILLTO64_32(__type, __field) __type##_t __field##_hi; __type##_t __field;
# else
#  error endian not configured for system
# endif
#else
# error __OFF_BITS__ value is unsupported
#endif
#define _FILLTO64_64(__type, __field) __type##64_t __field;

#define _OLD_TS_2001(__field) _Time32t __field;
#define _OLD_TS_2008(__field) _Time32t __old_##__field;

#define _NEW_TS_T322001(__field)
#define _NEW_TS_T642001(__field)
#define _NEW_TS_SYS2001(__field)
#define _NEW_TS_T322008(__field)  __extension__ union {							\
									_Time32t	__field##e;			\
									struct __timespec32	__field;	\
								};
#define _NEW_TS_T642008(__field)  __extension__ union {							\
									_Time64t	__field##e;			\
									struct __timespec64	__field;	\
								};
#if defined(__EXT_POSIX1_200809)
	#define _NEW_TS_SYS2008(__field)  __extension__ union {						\
									_Timet	__field##e;				\
									struct timespec	__field;		\
								};
	#undef __HAVE_TIMESPEC_FOR_STAT
#else
	#define _NEW_TS_SYS2008(__field)  __extension__ union {						\
									_Timet	__field##e;				\
									struct __timespec	__field;	\
								};
#endif
/* </STAN_MACRO1, STAN_MACRO5> */

// struct __stat_t32_2001 {
// 	__STAT_FIELDS(32, T32, 2001)
// };

// struct __stat_t32_2008 {
// 	__STAT_FIELDS(32, T32, 2008)
// };

// struct __stat_t64_2008 {
// 	__STAT_FIELDS(32, T64, 2008)
// };

// struct __stat_sys_2008 {
// 	__STAT_FIELDS(32, SYS, 2008)
// };

// struct stat {
// #if __PTR_BITS__ == 32
// 	__STAT_FIELDS(32, SYS, 2001)
// #else
// 	__STAT_FIELDS(32, SYS, 2008)
// #endif
// };

// #ifdef __EXT_LF64SRC
// struct __stat64_t32_2001 {
// 	__STAT_FIELDS(64, T32, 2001)
// };

// struct __stat64_t32_2008 {
// 	__STAT_FIELDS(64, T32, 2008)
// };

// struct __stat64_t64_2008 {
// 	__STAT_FIELDS(64, T64, 2008)
// };

// struct __stat64_sys_2008 {
// 	__STAT_FIELDS(64, SYS, 2008)
// };

// struct stat64 {
// #if __PTR_BITS__ == 32
// 	__STAT_FIELDS(64, SYS, 2001)
// #else
// 	__STAT_FIELDS(64, SYS, 2008)
// #endif
// };
// #endif

// #define _STAT_FORM_UNSET	0
// #define _STAT_FORM_T32_2001	1
// #define _STAT_FORM_T32_2008	2
// #define _STAT_FORM_T64_2008	3
// #if __PTR_BITS__ == 32
// #define _STAT_FORM_SYS_2008		(_STAT_FORM_T32_2008)
// #define _STAT_FORM_PREFERRED	(_STAT_FORM_T32_2001)
// #else
// #define _STAT_FORM_SYS_2008		(_STAT_FORM_T64_2008)
// #define _STAT_FORM_PREFERRED	(_STAT_FORM_T64_2008)
// #endif
// #define _STAT_FORM_MASK		0x03u

// #define _S_IFMT     0xF000              /*  Type of file                    */
// #define _S_IFIFO    0x1000              /*  FIFO                            */
// #define _S_IFCHR    0x2000              /*  Character special               */
// #define _S_IFDIR    0x4000              /*  Directory                       */
// #define _S_IFNAM    0x5000              /*  Special named file              */
// #define _S_IFBLK    0x6000              /*  Block special                   */
// #define _S_IFREG    0x8000              /*  Regular                         */
// #define _S_IFLNK    0xA000              /*  Symbolic link                   */
// #define _S_IFSOCK   0xC000              /*  Socket                          */

// #define _S_ACL_EXT    0x00010000        /*  Extended ACL(s) are in effect   */
// #define _S_TRUST_EXT  0x00020000        /*  File is trusted (Path Trust)    */

// #define S_ISFIFO(m) (((m)&_S_IFMT)==_S_IFIFO) /* Test for FIFO.             */
// #define S_ISCHR(m)  (((m)&_S_IFMT)==_S_IFCHR) /* Test for char special file.*/
// #define S_ISDIR(m)  (((m)&_S_IFMT)==_S_IFDIR) /* Test for directory file.   */
// #define S_ISBLK(m)  (((m)&_S_IFMT)==_S_IFBLK) /* Test for block specl file. */
// #define S_ISREG(m)  (((m)&_S_IFMT)==_S_IFREG) /* Test for regular file.     */

// #define S_ISLNK(m)  (((m)&_S_IFMT)==_S_IFLNK) /* Test for symbolic link.    */
// #define S_ISNAM(m)  (((m)&_S_IFMT)==_S_IFNAM) /* Test for special named file*/
// #define S_ISSOCK(m) (((m)&_S_IFMT)==_S_IFSOCK)/* Test for socket.           */

// #define S_TYPEISMQ(buf)     (S_ISNAM((buf)->st_mode)&&((buf)->st_rdev==_S_INMQ))
// #define S_TYPEISSEM(buf)    (S_ISNAM((buf)->st_mode)&&((buf)->st_rdev==_S_INSEM))
// #define S_TYPEISSHM(buf)    (S_ISNAM((buf)->st_mode)&&((buf)->st_rdev==_S_INSHD))
// #define S_TYPEISTMO(buf)    (S_ISNAM((buf)->st_mode)&&((buf)->st_rdev==_S_INTMO))

// #define S_ISTRUSTED(m) (((m)&_S_TRUST_EXT))   /* Test for path trust       */

// /*
//  *  For special named files (IFNAM), the subtype is encoded in st_rdev.
//  *  The subtypes are:
//  */
// #define _S_INSEM        00001           /*  semaphore subtype               */
// #define _S_INSHD        00002           /*  shared data subtype             */
// #define _S_INMQ         00003           /*  message queue subtype           */
// #define _S_INTMO        00004           /*  Typed memory object             */
// #define _S_QNX_SPECIAL  040000          /*  QNX special type                */

// #if defined(_POSIX_SOURCE) || !defined(NO_EXT_KEYS)
// /*
//  *  Common filetype macros
//  */
// # define S_IFMT      _S_IFMT            /*  Type of file                    */
// # define S_IFIFO     _S_IFIFO           /*  FIFO                            */
// # define S_IFCHR     _S_IFCHR           /*  Character special               */
// # define S_IFDIR     _S_IFDIR           /*  Directory                       */
// # define S_IFNAM     _S_IFNAM           /*  Special named file              */
// # define S_IFBLK     _S_IFBLK           /*  Block special                   */
// # define S_IFREG     _S_IFREG           /*  Regular                         */
// # define S_IFLNK     _S_IFLNK           /*  Symbolic link                   */
// # define S_IFSOCK    _S_IFSOCK          /*  Socket                          */
// #endif

// #if defined(__EXT_QNX)
// # define S_INSEM     _S_INSEM           /*  Semaphore                       */
// # define S_INSHD     _S_INSHD           /*  Shared Memory                   */
// # define S_INMQ      _S_INMQ            /*  Message Queue                   */
// # define S_INTMO     _S_INTMO           /*  Typed memory object             */

// # define S_IPERMS    000777             /*  Permission mask                 */
// #endif

// #define S_ISUID     004000              /* set user id on execution         */
// #define S_ISGID     002000              /* set group id on execution        */
// #define S_ISVTX     001000              /* sticky bit                       */
// #define S_ENFMT     002000              /* enforcement mode locking         */

// /*
//  *  Owner permissions
//  */
// #define S_IRWXU     000700              /*  Read, write, execute/search     */
// #define S_IRUSR     000400              /*  Read permission                 */
// #define S_IWUSR     000200              /*  Write permission                */
// #define S_IXUSR     000100              /*  Execute/search permission       */
// #define S_IREAD     S_IRUSR             /*  Read permission                 */
// #define S_IWRITE    S_IWUSR             /*  Write permission                */
// #define S_IEXEC     S_IXUSR             /*  Execute/search permission       */

// /*
//  *  Group permissions
//  */
// #define S_IRWXG     000070              /*  Read, write, execute/search     */
// #define S_IRGRP     000040              /*  Read permission                 */
// #define S_IWGRP     000020              /*  Write permission                */
// #define S_IXGRP     000010              /*  Execute/search permission       */

// /*
//  *  Other permissions
//  */
// #define S_IRWXO     000007              /*  Read, write, execute/search     */
// #define S_IROTH     000004              /*  Read permission                 */
// #define S_IWOTH     000002              /*  Write permission                */
// #define S_IXOTH     000001              /*  Execute/search permission       */

// /*
//  * utimensat(), futimens() tv_nsec special values
//  * */
// #if defined(__EXT_POSIX1_200809)
// #define UTIME_NOW	0x40000001
// #define UTIME_OMIT	0x40000002
// #endif

__BEGIN_DECLS

/*
 *  POSIX 1003.1 Prototypes.
 */

#if defined(__EXT_POSIX1_198808)
#ifdef __EXT_LF64SRC
extern int stat64(const char *__path, struct stat64 *__buf) __ALIASOFF("stat");
extern int lstat64(const char *__path, struct stat64 *__buf) __ALIASOFF("lstat");
extern int fstat64(int __fildes, struct stat64 *__buf) __ALIASOFF("fstat");
#if defined(__EXT_QNX)
extern int fstatat64(int __fd, const char *__path, struct stat64 *__statp, int __flag);
#endif
#if defined(__EXT_LF64ALIAS) && !defined(__ALIAS_ATTRIBUTE)
#error ALIAS not configured for compiler: stat,lstat,fstat,fstatat
#endif
#endif

// extern int stat(const char *__path, struct stat *__buf) __ALIAS64("stat64");
// extern int lstat(const char *__path, struct stat *__buf) __ALIAS64("lstat64");
// extern int fstat(int __fildes, struct stat *__buf) __ALIAS64("fstat64");
#if defined(__EXT_POSIX1_200809)
// extern int fstatat(int __fd, const char *__path, struct stat *__statp, int __flag) __ALIAS64("fstatat64");
#endif

#ifdef __EXT_FUNCALIAS64
# ifdef __ALIAS_ATTRIBUTE
/* Use __ALIAS64 define */
# elif defined(_PRAGMA_REDEFINE_EXTNAME)
#  pragma redefine_extname stat stat64
#  pragma redefine_extname lstat lstat64
#  pragma redefine_extname fstat fstat64
# else
#  define stat stat64
#  define lstat lstat64
#  define fstat fstat64
#  if defined(__EXT_POSIX1_200809)
#   define fstatat fstatat64
#  endif
# endif
#endif
#endif

// extern int chmod(const char *__path, mode_t __mode);
// extern int fchmod(int __fildes, mode_t __mode);
// #if defined(__EXT_QNX)	/* POSIX 1003.1g draft */
// extern int isfdtype(int __fildes, int __fdtype);
// #endif
// extern int mkdir(const char *__path, mode_t __mode);
// extern int mkfifo(const char *__path, mode_t __mode);
// extern int mknod(const char *__path, mode_t __mode, dev_t __dev);
// extern int _mknod_extra(const char *__path, mode_t __mode, dev_t __dev, unsigned __extra_type, unsigned __extra_len, void *__extra);
// extern mode_t umask(mode_t __cmask);
// #if defined(__EXT_POSIX1_200809)
// extern int fchmodat(int __fd, const char *__path, mode_t __mode, int __flags);
// extern int futimens(int __fd, const struct timespec __times[2]);
// extern int utimensat(int __fd, const char *path, const struct timespec __times[2], int __flag);
// extern int fstatat(int __fd, const char *__path, struct stat *__statp, int __flag);
// extern int mkdirat(int __fd, const char *__path, mode_t __mode);
// extern int mkfifoat(int __fd, const char *__path, mode_t __mode);
// extern int mknodat(int __fd, const char *__path, mode_t __mode, dev_t __dev);
// #endif
#if defined(__EXT_QNX)
extern mode_t _umask(pid_t __pid, mode_t __cmask);

union stat_any {
	struct stat					st;
	struct __stat_t32_2001		st_t32_2001;
	struct __stat_t32_2008		st_t32_2008;
	struct __stat_t64_2008		st_t64_2008;
#ifdef __EXT_LF64SRC
	struct stat64				st64;
	struct __stat64_t32_2001	st64_t32_2001;
	struct __stat64_t32_2008	st64_t32_2008;
	struct __stat64_t64_2008	st64_t64_2008;
#endif
};
extern int stat_convert_form(int __src_form, const void *__src_stat, int __dst_form, void *__dst_stat, unsigned __max_dst);

#define CHECK_TRUST_FILE      0U      /* Check file is trusted */
#define CHECK_TRUST_SETID     1U      /* Check setuid/setgid bits should be honoured */
#define CHECK_TRUST_EXEC      2U      /* Check execution should be permitted */
extern int fchecktrust(int __fd, const struct stat *__pstat, unsigned __operation);

#endif

// #include <_packpop.h>

__END_DECLS

#endif

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/c/public/sys/stat.h $ $Rev: 916291 $")
#endif
