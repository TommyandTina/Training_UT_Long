...
/* Debug functions are NOT supported */
#if !defined(NDEBUG)
#define NDEBUG
#endif

#if  defined(R_OSAL_ENABLE_DEBUG)
#undef R_OSAL_ENABLE_DEBUG
#endif
    ...
    e_osal_return_t error, osal_memory_buffer_handle_t buffer_hndl)
{
    ...
    {
        OSAL_TEST_ERR ("R_OSAL_MmngrGetCpuPtr in callback is fail %d\n",ret_osal);
    }
...
/***********************************************************************/
    ...
    if (OSAL_RETURN_OK != ret_osal)
    {
        OSAL_TEST_INFO ("Actual: R_OSAL_MmngrAlloc mmngr_buf[4] fail %d\n",ret_osal);
        OSAL_TEST_INFO ("Expect: R_OSAL_MmngrAlloc to return ret_osal = 1029 (OSAL_RETURN_PROHIBITED)\n");
    }
