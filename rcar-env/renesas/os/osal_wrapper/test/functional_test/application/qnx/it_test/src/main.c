    ...
    if(argc == 1)
    {
        while(!strcmp(ctr,"yes"))
        {
            ...
            else
            {
                ...
                if (parent_number >= OSAL_TEST_RETURN_CONFIRMATION && parent_number <= OSAL_TEST_PARENT_MAX)
                {
...
static void osal_test_display_menu()
{
    ...
    osal_test_return_confirmation_display_menu();
    osal_test_timer_accuracy_check_display_menu();
