...
static int32_t int_main_task(void)
{
    ...
    if(true == interrupt_flag)
    {
        ...
        app_ret = stop_interrupt(dev_handles[0], IRQ_CHANNEL, int_cb_func);
        if(0 != app_ret)
        {
    ...
    app_ret = teardown_interrupt_hw(dev_handles, memory_handle, buffer_handle, is_pm_active);
    if(0 != app_ret)
    {
