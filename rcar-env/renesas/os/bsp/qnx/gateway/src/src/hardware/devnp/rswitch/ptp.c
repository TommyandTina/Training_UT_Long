...
{
    nic_mutex_lock(&rswitch2->ptp_mutex);

    out32(rswitch2->base + PTPTIVC0, PTP_INCR);
    out32(rswitch2->base + PTPTMEC, PTPTMEC_TE);

    nic_mutex_unlock(&rswitch2->ptp_mutex);
}
...
void rswitch2_ptp_add_tx_timestamp(rswitch2_dev_t *rswitch2)
{
    ...
    while( idx < RSWITCH2_TX_TIMESTAMP_BUF_SZ) {
        ...
        if((ph = &(tx_ph[queue][idx])) && ((ph->version & 0x0f) == 0x2)) {
            ...
            out32(rswitch2->base + PTPMCCC0, PTPMCCC0_MCCR);
            tx_ts[tx_ts_cnt].ts.nsec = in32(rswitch2->base + PTPGPTPTM00);
            tx_ts[tx_ts_cnt].ts.sec = in32(rswitch2->base + PTPGPTPTM10);

            if(rswitch2->cfg.verbose & VERBOSE_PTP) {
                slogf(_SLOGC_NETWORK, _SLOG_INFO, "%s queue=%d idx=%d msgid=0x%x sequence_id=%u ts=%u.%u", __FUNCTION__, 
...
static void rswitch2_ptp_get_cnt(rswitch2_dev_t *rswitch2, ptp_time_t *cnt)
{
    ...
    if(cnt != NULL) {
        ...
        cnt->nsec = in32(rswitch2->base + PTPGPTPTM00);
        cnt->sec = in32(rswitch2->base + PTPGPTPTM10);
...
static void rswitch2_ptp_set_cnt(rswitch2_dev_t *rswitch2, ptp_time_t *cnt)
{
    ...
    out32(rswitch2->base + PTPTMDC, PTPTMDC_TD);
    out32(rswitch2->base + PTPTOVC10, 0);
    out32(rswitch2->base + PTPTOVC00, 0);
    out32(rswitch2->base + PTPTMEC, PTPTMEC_TE);
    out32(rswitch2->base + PTPTOVC10, cnt->sec);
    out32(rswitch2->base + PTPTOVC00, cnt->nsec);
...
static void rswitch2_ptp_set_compensation(rswitch2_dev_t *rswitch2, ptp_comp_t ptc)
{
    ...
    nic_mutex_lock(&rswitch2->ptp_mutex);
    // if(in32(rswitch2->base + PTPMCCC0)){
    //     nic_mutex_unlock(&rswitch2->ptp_mutex);
    //     return;
    // }
    out32(rswitch2->base + PTPTIVC0, comp & TIV_MASK);
    nic_mutex_unlock(&rswitch2->ptp_mutex);

    if(rswitch2->cfg.verbose & VERBOSE_PTP) {
        slogf(_SLOGC_NETWORK, _SLOG_INFO, "%s ppb=%u dir=%u deviation=%u current_gti=%u", __FUNCTION__,
