	...
	if (cfg->module != NULL) {
		...
		while (module->flags) {
			...
			if (module->flags & CPG_FLAGS_RST) {
				RCAR_DEBUG_PRINT("%s RST domain=0x%x module=%d\n", __FUNCTION__, cfg->domain, module->mod_id);
				cpg_support_funcs.mstp_reset(module->mod_id);
			}

			/* Module Clock Enable/Disable */
			RCAR_DEBUG_PRINT("%s %s domain=0x%x module=%d\n", __FUNCTION__, (module->flags & CPG_FLAGS_EN)?"EN":"DIS", cfg->domain, module->mod_id);
