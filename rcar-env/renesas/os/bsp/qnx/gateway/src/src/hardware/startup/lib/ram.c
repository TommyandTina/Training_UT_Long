...
#define TEMP_SYSPAGE_SIZE (KILO(32))
...
ws_init(void) {
	...
	ws_early_next = ROUND(full_ram_paddr + shdr->ram_size, sizeof(uint64_t));
	...
	avoid_ram(full_ram_paddr, shdr->startup_size);
...
paddr_t
find_top_ram_aligned_limit(paddr_t size, unsigned align, unsigned pagesize, paddr_t limit) {
	struct ram_entry		*r;
...
paddr_t find_top_ram_aligned(paddr_t size, unsigned align, unsigned pagesize) {
	return find_top_ram_aligned_limit(size, align, pagesize, ~0);
}
...
add_ram(paddr_t start, paddr_t size) {
	...
	alloc_ram(full_ram_paddr+shdr->startup_size,
...
avoid_ram(PADDR_T start, size_t size) {
	...
	if(avoid_list_size > MAX_AVOID_LIST_SIZE) {
		crash("Avoid RAM entry array overflow\n");
	}
}
 ...
 */
void
reserve_ram(unsigned size, unsigned align, const char * name, unsigned pagesize, const unsigned low_region) {
	paddr_t reserved = NULL_PADDR;
	...
	if(low_region == 0) {
		...
		if(reserved == NULL_PADDR) {
			crash("Could not reserve 0x%x bytes from top of memory.\n", size);
		}
	}

	reserved = alloc_ram(reserved, size, align ? align : pagesize);
	if(reserved == NULL_PADDR) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/startup/lib/ram.c $ $Rev: 941198 $")
#endif
