...
init_pl011(unsigned channel, const char *init, const char *defaults)
{
	...
	if (baud != 0) {
		...
		while (in32(base + PL011_FR) & PL011_FR_BUSY)
			;
		out32(base + PL011_CR, 0);          /* Disable UART */

		out32(base + PL011_ICR, 0x7FF);     /* Clear all status */
		out32(base + PL011_IBRD, ibrd);
		out32(base + PL011_FBRD, fbrd);
		out32(base + PL011_LCR_H, PL011_LCR_H_WLEN8 | PL011_LCR_H_FEN);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/startup/lib/common_arm/hw_serpl011.c $ $Rev: 931125 $")
#endif
