...
static int rswitch2_serdes_common_init_ram(rswitch2_dev_t *rswitch2)
{
    ...
    if (rswitch2->port == 0) {
        rswitch2_serdes_write(serdes->base, 0x026C, 0x180, 0x03);
    }

    rc = rswitch2_serdes_reg_wait(serdes->base, 0x0000, 0x300, BIT(15), 0);
...
static int rswitch2_serdes_common_setting(rswitch2_dev_t *rswitch2, int mode)
{
    ...
    if (mode == SERDES_SGMII) {
        rswitch2_serdes_write(serdes->base, 0x0244, 0x180, 0x0097);
        rswitch2_serdes_write(serdes->base, 0x01D0, 0x180, 0x0060);
        rswitch2_serdes_write(serdes->base, 0x01D8, 0x180, 0x2200);
        rswitch2_serdes_write(serdes->base, 0x01D4, 0x180, 0x0000);
        rswitch2_serdes_write(serdes->base, 0x01E0, 0x180, 0x003d);
    } else if (mode == SERDES_USXGMII) {
        rswitch2_serdes_write(serdes->base, 0x0244, 0x180, 0x0057);
        rswitch2_serdes_write(serdes->base, 0x01CC, 0x180, 0xC200);
        rswitch2_serdes_write(serdes->base, 0x01C4, 0x180, 0x0042);
        rswitch2_serdes_write(serdes->base, 0x01C8, 0x180, 0x0000);
        rswitch2_serdes_write(serdes->base, 0x01DC, 0x180, 0x002F);
    } else {
        return -EINVAL;
    }
...
static int rswitch2_serdes_channel_settings(rswitch2_dev_t *rswitch2, int mode)
{
    ...
    if (mode == SERDES_SGMII) {
        /* 
         * Add a line right below is to fix 'SerDes Link up failure' issue 
         * when booting QNX ifs image from eMMC
         */
...
static int rswitch2_serdes_init(rswitch2_dev_t *rswitch2)
{
    ...
    serdes->base = mmap_device_io(RSWITCH2_SERDES_SIZE, RSWITCH2_SERDES_BASE);
    if (serdes->base == (uintptr_t)MAP_FAILED) {
        rc = errno;
        slogf(_SLOGC_NETWORK, _SLOG_ERROR, "rswitch2: mmap Serdes device failed.");
        return rc;
    }

    serdes->addr = serdes->base + serdes->index * RSWITCH2_SERDES_OFFSET;
    ...
    if (rswitch2->port == 0) {
        rswitch2_serdes_write(serdes->base, 0x0000, 0x0380, 0x8000);
    }
    ...
    if (rswitch2->port == 0) {
        rswitch2_serdes_reg_wait(rswitch2->serdes.addr, 0x0000, 0x380, BIT(15), 0);
    }
...
static int rswitch2_bat_desc_init(rswitch2_dev_t *rswitch2)
{
    ...
    if (bat_desc_base == (uintptr_t)MAP_FAILED) {
        ...
        rswitch2->bat_desc = (RSWITCH2_DESC *)mmap (NULL, size, PROT_READ | PROT_WRITE | PROT_NOCACHE, 
                                MAP_SHARED | MAP_PHYS, NOFD, rswitch2->bat_desc_addr);
    } else {
        rswitch2->bat_desc_addr = bat_desc_paddr + bat_desc_off;
...
static void rswitch2_cleanup (rswitch2_dev_t *rswitch2)
{
    ...
    munmap_device_io(rswitch2->serdes.base, RSWITCH2_SERDES_SIZE);
