...
static int sd_parse_scr( sdio_dev_t *dev, sd_scr_t *scr, uint32_t *rscr )
{
	...
	scr->sd_spec4				= sdio_extract_bits( rscr, 64, 42, 1 );
	scr->sd_specx				= sdio_extract_bits( rscr, 64, 38, 4 );
	scr->cmd_support			= sdio_extract_bits( rscr, 64, 32, 4 );
...
static int sd_rw_extr( sdio_dev_t *dev, int dir, int mio, int fno, int mw, uint32_t addr, int len_msk, uint8_t *extr )
{
	...
	if( ( ebuf = sdio_alloc( SD_EXTR_SIZE ) ) == NULL ) {
		sdio_free_cmd( cmd );
		return( ENOMEM );
	}
...
[8:0] LEN/MASK mw=1 8bits, mw=0 9bits
	...
	sge.sg_count = SD_EXTR_SIZE; sge.sg_address = SDIO_DATA_PTR_P( ebuf );
...
static int sdio_send_relative_addr( sdio_hc_t *hc, int *rca )
{
	sdio_dev_t	*dev;
	sdio_cmd_t	*cmd;
	int			status;
...
3) Host polls Power Management Status register until the card indicates ready to power off (POFR=1).
	...
	if( ( status = sd_extr_wr( dev, dev->pwr_mgnt_fcn_rsa, PWR_MGNT_SETTING, PWR_MGNT_SETTING_POFN ) ) != EOK ) {
		return( status );
	}

		// wait up to 1 second for POFR
	for( timeout = 1000; timeout; timeout-- ) {
		if( ( status = sd_extr_rd( dev, dev->pwr_mgnt_fcn_rsa, PWR_MGNT_STATUS, &rval ) ) == EOK ) {
...
int sd_cache( sdio_dev_t *dev, int op, uint32_t timeout )
{
	...
	switch( op ) {
		case SDIO_CACHE_DISABLE:
			if( ( status = sd_extr_wr( dev, dev->perf_enh_fcn_rsa, PERF_ENH_CACHE, 0 ) ) == EOK ) {
				dev->flags &= ~DEV_FLAG_WCE;
			}
			break;

		case SDIO_CACHE_ENABLE:
			if( ( status = sd_extr_wr( dev, dev->perf_enh_fcn_rsa, PERF_ENH_CACHE, PERF_ENH_CACHE_EN ) ) == EOK ) {
				dev->flags |= DEV_FLAG_WCE;
			}
			break;
		...
		case SDIO_CACHE_FLUSH:
			if( ( dev->flags & DEV_FLAG_WCE ) ) {
				if( ( status = sd_extr_wr( dev, dev->perf_enh_fcn_rsa, PERF_ENH_CACHE_FLUSH, PERF_ENH_CACHE_FLUSH_START ) ) ) {
					...
					// wait up to 1 second
				for( timeout = 1000; timeout; timeout-- ) {
					if( ( status = sd_extr_rd( dev, dev->perf_enh_fcn_rsa, PERF_ENH_CACHE_FLUSH, &rval ) ) == EOK ) {
	...
	for( fcn = 1; fcn <= feh->number_extensions; fcn++ ) {
		...
		switch( fext->sfc ) {
			...
			case SFC_PERF_ENH:
				...
				if( ( status = sd_extr_rd( dev, dev->perf_enh_fcn_rsa, PERF_ENH_CACHE_SUP, &rval ) ) == EOK ) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/devb/sdmmc/sdiodi/sd.c $ $Rev: 953988 $")
#endif
