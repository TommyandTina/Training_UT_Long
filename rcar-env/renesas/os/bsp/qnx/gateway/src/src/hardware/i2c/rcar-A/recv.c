...
rcar_i2c_recv(void *hdl, void *buf, unsigned int len, unsigned int stop)
{
    ...
    rcar_i2c_slogf(dev, VERBOSE_LEVEL4, "[chan %d]%s: ", dev->chan_idx, __FUNCTION__);

    if (len == 0) {
        rcar_i2c_slogf(dev, VERBOSE_LEVEL8, "[chan %d]%s: I2C_STATUS_DONE because of recv length 0 bytes", dev->chan_idx, __FUNCTION__);
    ...
    out32(dev->regbase + RCAR_I2C_ICMIER, RCAR_I2C_ICMIER_RECV);
