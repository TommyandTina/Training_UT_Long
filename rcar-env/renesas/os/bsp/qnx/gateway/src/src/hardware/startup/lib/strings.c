...
add_typed_string(int type_index, const char *name) {
	...
	*(uint32_t *)&p[i] = type_index;
	strlcpy(&p[i+sizeof(uint32_t)], name, lsp.typed_strings.size - i);
	return(i);
}
...
add_string(const char *name) {
	...
	if(len > spare) {
		...
		grow_syspage_section(&lsp.strings, ROUND(len - spare, 128));
	}
	strlcpy(str, name, lsp.strings.size - i);
	return(i);
}

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/startup/lib/strings.c $ $Rev: 920261 $")
#endif
