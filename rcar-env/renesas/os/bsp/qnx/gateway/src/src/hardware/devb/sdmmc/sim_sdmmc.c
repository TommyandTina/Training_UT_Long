...
static SIM_HBA *sdmmc_alloc_hba( )
{
	SIM_HBA			*hba;
	SIM_SDMMC_EXT	*ext;
...
//	cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  hba %p, %s", __FUNCTION__, hba, hba_free ? "free", "keep" );
	...
	if( hba_free == CAM_TRUE ) {
		...
		cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: keep 0x%p", __FUNCTION__, hba );
	}
...
static int sdmmc_mancmd_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
	...
	for( ; ; ) {
		...
		if( ( req->flags & SDMMC_MANCMD_TYPE ) == SDMMC_MANCMD_ADTC ) flags |= SCF_CTYPE_ADTC;
		if( ( req->flags & SDMMC_MANCMD_DIR ) == SDMMC_MANCMD_READ ) flags |= SCF_DIR_IN;
		if( ( req->flags & SDMMC_MANCMD_RSP ) == SDMMC_MANCMD_R1 ) flags |= SCF_RSP_R1; else flags |= SCF_RSP_R1B;
...
int sdmmc_sim_attach( CAM_ENTRY *centry )
{
	...
	sdio_funcs_t			funcs = { 2, sdmmc_sdio_insertion, sdmmc_sdio_removal, sdmmc_sdio_event };
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/devb/sdmmc/sim_sdmmc.c $ $Rev: 957622 $")
#endif
