...
fat_parse_mbr(mbr_t *mbr, bpb_t *bpb)
{
	...
	/* If MBR partition type is 0xEE consider it as Protective MBR (PMBR).
	 * IPL supports only MBR or GPT with PMBR. */
	if (pe->os_type == GPT_PROTECTIVE_MBR) {
		gpt_t *gpt;
		gpt_entry_t *gpt_entry;
		uint8_t lba1[512];
		uint8_t lba2[512];

		read_sector(1, &lba1[0], 1);
		gpt = (gpt_t *)&lba1[0];

		if (gpt->signature != GPT_SIGNATURE) {
			ser_putstr("    Error: GPT signature is wrong\n");
			return SDMMC_ERROR;
		}

		if (fat_dev->verbose) {
			ser_putstr("Primary GPT header\n");
			ser_putstr("    GPT signature: 0x");
			ser_puthex64(gpt->signature);
			ser_putstr("\n");
			ser_putstr("    GPT revision: 0x");
			ser_puthex32(gpt->revision);
			ser_putstr("\n");
			ser_putstr("    GPT header size: 0x");
			ser_puthex32(gpt->header_size);
			ser_putstr("\n");
			ser_putstr("    GPT crc32: 0x");
			ser_puthex32(gpt->crc32);
			ser_putstr("\n");
			ser_putstr("    GPT current LBA: 0x");
			ser_puthex64(gpt->current_lba);
			ser_putstr("\n");
			ser_putstr("    GPT LBA of partition entries: 0x");
			ser_puthex64(gpt->partition_entries_lba);
			ser_putstr("\n");
			ser_putstr("    GPT Number of partition entries: 0x");
			ser_puthex32(gpt->partition_entries_num);
			ser_putstr("\n");
			ser_putstr("    GPT Size of partition entry: 0x");
			ser_puthex32(gpt->sizeof_partition_entry);
			ser_putstr("\n");
		}

		/* Search through GPT entry array */
		for (y = 0; y < gpt->partition_entries_num; y++) {
			read_sector(gpt->partition_entries_lba + y, &lba2[0], 1);
			gpt_entry = (gpt_entry_t *)&lba2[0];
			if (gpt_entry->first_lba == 0) {
				break;
			}
			/* Search through GPT entries from given sector */
			for (i = 0; i < 512; i += 128) {
				gpt_entry = (gpt_entry_t *)&lba2[i];
				if (gpt_entry->attributes & 0x4) {
					fs_info->fs_offset = gpt_entry->first_lba;
					if (fat_dev->verbose) {
						ser_putstr("BOOT LBA : 0x");
						ser_puthex64(gpt_entry->first_lba);
						ser_putstr("\n");
						ser_putstr("ATTRIBUTE: 0x");
						ser_puthex64(gpt_entry->attributes);
						ser_putstr("\n");
					}
					break;
				}
...
//{ 
//    unsigned char * s = (unsigned char *)bpb;
//    ser_puthex32((unsigned int)bpb);
//    ser_putstr(":\n");
//    for (int k = 0; k < 512; k++) {
//        ser_puthex8(s[k]);
//        ser_putstr(" ");
...
get_fat_entry32(unsigned cluster)
{
	...
	if (cache_valid && g_fat_sector == fat_sector) {
		return (*(unsigned *)(data_buf + fat_offs)) & 0x0fffffff;
	}
	if (SDMMC_OK != read_fsector(fat_sector, data_buf, 1))
		return 0;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/ipl/lib/fs/fat-fs.c $ $Rev: 862290 $")
#endif
