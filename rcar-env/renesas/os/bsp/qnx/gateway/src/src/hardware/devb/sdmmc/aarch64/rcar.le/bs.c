...
static int rcar_bs_args(sdio_hc_t *hc, char *options)
{
    ...
    static char     *opts[] = {
                        ...
                        "vdd",       /* vdd pwr base reg, size, vdd offset, vdd shift */
                        "if",        /* if pwr base reg, size, if offset, if shift */
                        "pfc",       /* signal IO base reg, size, pmmr off, poc offset, poc mask, poc shift */
...
static int rcar_bs_dinit(sdio_hc_t *hc)
{
    ...
    if (bsext->pwr_if_base && bsext->pwr_if_pbase != bsext->pwr_vdd_pbase) {
...
static int rcar_bs_init(sdio_hc_t *hc)
{
    ...
    if(hc->flags & HC_FLAG_DEV_MMC) {
        ...
        if(!bsext->pwr_vdd_pbase || ((bsext->pwr_vdd_base = mmap_device_io(bsext->pwr_vdd_size, bsext->pwr_vdd_pbase)) == (uintptr_t)MAP_FAILED)) {
            goto fail2;
        }
        ...
        if (bsext->pwr_if_pbase) {
            ...
            } else if (((bsext->pwr_if_base = mmap_device_io(bsext->pwr_if_size, bsext->pwr_if_pbase)) == (uintptr_t)MAP_FAILED)) {
                goto fail1;
            }
        }
        ...
        if (bsext->pwr_if_base || bsext->pwr_cpld_ctrl) {
            hc->caps |= HC_CAP_SV_1_8V;
            hc->caps |= HC_CAP_XPC_1_8V;
        }
...
sdio_product_t  sdio_fs_products[] = {
    { SDIO_DEVICE_ID_WILDCARD, 0, 0, "rcar", rcar_bs_init },
};
