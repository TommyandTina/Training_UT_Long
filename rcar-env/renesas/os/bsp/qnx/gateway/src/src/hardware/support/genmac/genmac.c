...
int main (int argc, char *argv[])
{
  ...
  if ((argc != 1) &&
      ((argc !=2) || strcmp(argv[1], "-m")) &&
      ((argc !=3) || strcmp(argv[2], "-p")))
  {
    fprintf(stderr, "%s: Invalid Option/syntax, \n", argv[0]);
    if( system("use genmac-v3u") == -1 ) {
  ...
  else
  {
    ...
    {
      macaddr |= (uint64_t)chipuid.uid[i] << (8*i);
    }
    ...
    if (chipuid.uid_size > MAC_SIZE)
    {
      ...
      {
        macaddr ^= (uint64_t)chipuid.uid[i] << (8*(i % MAC_SIZE));
      }
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/support/genmac/genmac.c $ $Rev: 855584 $")
#endif
