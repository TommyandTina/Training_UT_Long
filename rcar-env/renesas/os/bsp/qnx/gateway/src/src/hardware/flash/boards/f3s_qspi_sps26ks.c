                      ...
                      uint8_t *buffer)
{
    ...
    if ((rc = hyper_nor_write_memory(hdl, offset, buffer, cursz)) < 0) {
        errno = EIO;
        return -1;
    }
    ...
    left = rc;
    while (left) {
        cursz = hyper_nor_read_memory(hdl, offset, buf, left);
                      ...
                      uint32_t flags, uint32_t offset)
{
    ...
    if (hyper_nor_write_command(hdl, HYPER_PPB_ENTRY_ADDR, HYPER_PPB_ENTRY_DATA) < 0) {
        fprintf(stderr, "Entry PPB failed.\n");
        return (EIO);
    }

    /* All PPB Erase command */
    if (hyper_nor_write_command(hdl, 0, HYPER_PPB_ERASE_DATA1) < 0) {
        fprintf(stderr, "PPB Erase first command failed.\n");
        return (EIO);
    }

    if (hyper_nor_write_command(hdl, 0, HYPER_PPB_ERASE_DATA2) < 0) {
        fprintf(stderr, "PPB Erase second command failed.\n");
        return (EIO);
    }

    /* Reset/ASO Exit Command */
    if (hyper_nor_write_command(hdl, HYPER_RESET_ADDR, HYPER_RESET_DATA) < 0) {
        fprintf(stderr, "Reset/ASO exit failed.\n");
        return (EIO);
    }

    if (hyper_nor_busy_wait(hdl, HYPER_RESET_TIME) == EOK)
        return (EOK);
                   ...
                   uint32_t flags, uint32_t offset)
{
    ...
    if (hyper_nor_write_command(hdl, HYPER_DYB_ENTRY_ADDR, HYPER_DYB_ENTRY_DATA) < 0) {
        fprintf(stderr, "Entry failed.\n");
        return (EIO);
    }

    /* DYB Clear */
    if (hyper_nor_write_command(hdl, 0, HYPER_DYB_PROGRAM_DATA) < 0) {
        fprintf(stderr, "Enter DYB Clear failed.\n");
        return (EIO);
    }

    /* Write value 1 to HyperFlash */
    if (hyper_nor_write_command(hdl, offset, 0x1) < 0) {
                      ...
                      uint32_t text_offset)
{
    ...
    return hyper_nor_busy_wait(access->socket.socket_handle, 1);
}
                    ...
                    uint32_t offset)
{
    ...
    if (hyper_nor_write_command(access->socket.socket_handle, offset, HYPER_ERASE_SUSPEND_DATA) < 0)
                   ...
                   uint32_t offset)
{
    ...
    if (hyper_nor_write_command(access->socket.socket_handle, offset, HYPER_ERASE_RESUME_DATA) < 0)
...
void sps26ks_reset (f3s_dbase_t *dbase,
                    ...
                    uint32_t offset)
{
    if (hyper_nor_write_command(access->socket.socket_handle, HYPER_RESET_ADDR, HYPER_RESET_DATA) < 0) {
                      ...
                      uint8_t *buffer)
{
    ...
    rc = hyper_nor_read_memory(access->socket.socket_handle,
...
static int sps26ks_dolock(void *hdl, uint32_t cmd, uint32_t offset)
{
    ...
    if (cmd == HYPER_PPB_ENTRY_DATA)
    {
        ...
        if (hyper_nor_write_command(hdl, 0, HYPER_PPB_PROGRAM_DATA) < 0) {
    ...
    if (hyper_nor_write_command(hdl, offset, 0x0) < 0) {
                   ...
                   uint32_t offset)
{
    ...
    if (dbase->flags & F3S_PROTECT_DYN) {
        /* Write Entry Command */
        if (hyper_nor_write_command(access->socket.socket_handle, HYPER_UNLOCK1_ADDR, HYPER_UNLOCK1_DATA) < 0) {
            fprintf(stderr, "Unlock1 failed.\n");
            return (EIO);
        }

        if (hyper_nor_write_command(access->socket.socket_handle, HYPER_UNLOCK2_ADDR, HYPER_UNLOCK2_DATA) < 0) {
            fprintf(stderr, "Unlock2 failed.\n");
            return (EIO);
        }

        if (hyper_nor_write_command(access->socket.socket_handle, HYPER_DYB_ENTRY_ADDR, HYPER_DYB_ENTRY_DATA) < 0) {
            fprintf(stderr, "Entry failed.\n");
            return (EIO);
        }

        /* Read data */
        if (hyper_nor_read_registers(access->socket.socket_handle, offset, &data, 1) < 0) {
    ...
    if (dbase->flags & F3S_PROTECT_PERSISTENT) {
        ...
        if (hyper_nor_write_command(access->socket.socket_handle, HYPER_PPB_ENTRY_ADDR, HYPER_PPB_ENTRY_DATA) < 0) {
            fprintf(stderr, "Entry failed.\n");
            return (EIO);
        }
                      ...
                      uint32_t offset)
{
    ...
    if (hyper_nor_write_command(access->socket.socket_handle, HYPER_ID_ENTRY_ADDR, HYPER_ID_ENTRY_DATA) < 0) {
        fprintf(stderr, "Unlock2 failed.\n");
        return (EIO);
    }

    /* Read ID */
    if (hyper_nor_read_registers(access->socket.socket_handle, 0, buf, 80) < 0) {
                  ...
                  uint32_t offset)
{
    ...
    return hyper_nor_sector_erase(access->socket.socket_handle,
        offset, 1 << dbase->geo_vect[geo_index].unit_pow2);
}

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/flash/boards/rcar_qspi/f3s_qspi_sps26ks.c $ $Rev: 839522 $")
#endif
