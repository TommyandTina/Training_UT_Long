...
#define DMAC_CHCR_CAE            0x80000000
#define DMAC_CHCR_CAIE           0x40000000
#define DMAC_CHCR_DPM            0x30000000
#define DMAC_CHCR_DPM_DISABLE    0x00000000
#define DMAC_CHCR_DPM_NORMAL     0x10000000
#define DMAC_CHCR_DPM_RPT        0x20000000
#define DMAC_CHCR_DPM_RPT_INF    0x30000000
#define DMAC_CHCR_RPT            0x0E000000
#define DMAC_CHCR_RPT_S          0x08000000
#define DMAC_CHCR_RPT_D          0x04000000
#define DMAC_CHCR_RPT_C          0x02000000
#define DMAC_CHCR_RPT_CH         0x01000000
#define DMAC_CHCR_DPB            0x00400000
#define DMAC_CHCR_TS2            0x00100000
#define DMAC_CHCR_DSE            0x00080000
#define DMAC_CHCR_DSIE           0x00040000
#define DMAC_CHCR_DM             0x0000C000
#define DMAC_CHCR_DM_FIXED       0x00000000
#define DMAC_CHCR_DM_INCR        0x00004000
#define DMAC_CHCR_DM_DECR        0x00008000
#define DMAC_CHCR_SM             0x00003000
#define DMAC_CHCR_SM_FIXED       0x00000000
#define DMAC_CHCR_SM_INCR        0x00001000
#define DMAC_CHCR_SM_DECR        0x00002000
#define DMAC_CHCR_RS             0x00000F00
#define DMAC_CHCR_RS_AUTO        0x00000400
#define DMAC_CHCR_RS_ONCHIP      0x00000800
#define DMAC_CHCR_TS             0x00000018
#define DMAC_CHCR_TS_BYTE        0x00000000
#define DMAC_CHCR_TS_WORD        0x00000008
#define DMAC_CHCR_TS_LONG        0x00000010
#define DMAC_CHCR_TS_8           0x00100018
#define DMAC_CHCR_TS_16          0x00000018
#define DMAC_CHCR_TS_32          0x00100000
#define DMAC_CHCR_TS_64          0x00100008
#define DMAC_CHCR_IE             0x00000004
#define DMAC_CHCR_TE             0x00000002
#define DMAC_CHCR_DE             0x00000001

/* Contents of the CHCRB register */
#define DMAC_CHCRB_DRST          0x00008000

/* Memory descriptors related definitions */
#define DMAC_DESCRIPTORS_PER_GROUP     (rcar_soc == PRODUCT_ID_RCAR_S4 ? 4096 : 256)
...
#define RCAR_GEN4       (rcar_soc == PRODUCT_ID_RCAR_V3U || rcar_soc == PRODUCT_ID_RCAR_S4) 
...
#define NAME_SIZE  50
    ...
    rsrcdb_dma_to_rsrcdb_name(name, NAME_SIZE, chan->dma_type, 1);
...
#define NAME_SIZE  50
    ...
    rsrcdb_dma_to_rsrcdb_name(name, NAME_SIZE, chan->dma_type, 0);
...
dma_init(const char *options)
{
    ...
    for (type = 0; type < DMA_TYPE_NUM; type++) {
        ...
        printf("DMA: %s: %s-dmac channels %d  channel groups %d", __FUNCTION__,
                        dmac_type_names[type], dmac_channels[type], dmac_channel_group[type]);
#endif
    }
...
static void query_hwinfo(dma_channel_t* const chan)
{
    unsigned hwi_off, tag_idx;
    hwi_tag *tag;
...
dma_channel_info(const unsigned channel, dma_channel_info_t* const info)
{
    ...
    info->max_src_fragments     = DMAC_DESCRIPTORS_PER_GROUP;
    info->max_dst_fragments     = DMAC_DESCRIPTORS_PER_GROUP;
    info->max_src_segments      = DMAC_DESCRIPTORS_PER_GROUP;
    info->max_dst_segments      = DMAC_DESCRIPTORS_PER_GROUP;
...
sysdmac_parse_options(dma_channel_t* const chan, char *options)
{
    ...
    while (options && (*options != '\0')) {
        ...
        switch (opt) {
            ...
            case SYSDMAC_OPT_CAIE:
                ...
                break;
            default:
                rcar_dma_slogf(SLOG2_ERROR, "%s:%d - Invalid option %s", __func__, __LINE__, dma_opts[opt]);
                return EINVAL;
        }
    }
    ...
    if( (dmac_channels[DMA_SYS] == 0) && (dmac_channels[DMA_AUDIO] == 0) && (dmac_channels[DMA_RT] == 0)) {
        rcar_dma_slogf(SLOG2_ERROR, "%s:%d - Invalid channels\n", __func__, __LINE__);
        return EINVAL;
    }
...
static uintptr_t sysdmac_map_registers(dma_channel_t* const chan)
{
...
#endif
{
    ...
    if (optstr == NULL) {
        rcar_dma_slogf(SLOG2_ERROR,  "%s: strdup failed\n", __FUNCTION__);
        return NULL;
    }

    chan = calloc(1, sizeof(*chan));
    if (chan == NULL) {
        rcar_dma_slogf(SLOG2_ERROR, "%s: calloc failed\n", __FUNCTION__);
        goto fail0;
    }
    ...
    if ((errno = sysdmac_parse_options(chan, optstr)) != EOK) {
        rcar_dma_slogf(SLOG2_ERROR, "%s: parse options failed\n", __FUNCTION__);
        goto fail1;
    }
    ...
    if (chan->channels == 0 || chan->channels_per_group == 0) {
        if (chan->debug) {
            rcar_dma_slogf(SLOG2_ERROR, "%s: channels/chan per grp (%d/%d) are not available for %s-dmac",
                __FUNCTION__, chan->channels, chan->channels_per_group, dmac_type_names[chan->dma_type]);
        }
        goto fail1;
    }
    ...
    if (sysdmac_map_registers(chan) == (uintptr_t)MAP_FAILED) {
        rcar_dma_slogf(SLOG2_ERROR, "%s: %s DMA chan %d grp %d register map failed: %s\n", __FUNCTION__,
...
#ifdef DEBUG
    printf("DMA: %s: (channels %d, groups %d) Name %s Channel %d grp %d irq %d\n",
    ...
    if (flags & (DMA_ATTACH_EVENT_ON_COMPLETE | DMA_ATTACH_EVENT_PER_SEGMENT) && event != NULL) {
        ...
        if (chan->iid == -1) {
            rcar_dma_slogf(SLOG2_ERROR, "%s: %s DMA chan %d grp %d interrupt attach failed: %s\n", __FUNCTION__,
                chan->name, chan->chan_idx, chan->grp_idx, strerror(errno));
            goto fail3;
        }
    }
...
dma_setup_descriptor(void* const handle, const dma_transfer_t* const tinfo, uint32_t chcr)
{
    ...
    if (chan->desc_num == 0) {  // no internal descriptor memory
        if (segs > DMAC_DESCRIPTORS_PER_GROUP) {
            rcar_dma_slogf(SLOG2_ERROR, "SYSDMAC: Unsupported segment value %d\n", segs);
            return (-1);
        }
        if (chan->desc.len == 0) {
            if( dma_alloc_buffer( handle,
                ...
                rcar_dma_slogf(SLOG2_ERROR, "%s: %s DMA chan %d grp %d buf alloc failed: %s\n", __FUNCTION__,
                    chan->name, chan->chan_idx, chan->grp_idx, strerror(errno));
                return (-1);
            }
        }
    ...
    out32(chan->regs + RCAR_DMAC_DMADPBASE, dpbase | 1); // use Built-in memory or external memory
    out32(chan->regs + RCAR_DMAC_DMAFIXDPBASE, dpbase >> 32);
    out32(chan->regs + RCAR_DMAC_DMACHCRB, DMAC_CHCRB_DRST);  // Reset descriptor
    ...
    if (chan->sop) {
        if (fixsar || fixdar) {
            rcar_dma_slogf(SLOG2_ERROR, "%s: above4G not supported when smmu is enabled\n", __FUNCTION__);
            return (-1);
        }
    }
#endif
    ...
    for (sgi = 0; sgi < segs; sgi++, desc++) {
        if ((tinfo->src_flags & DMA_ADDR_FLAG_NO_INCREMENT) == 0) {
            if ((tinfo->src_addrs[sgi].paddr >> 32) != fixsar) {
                rcar_dma_slogf(SLOG2_ERROR, "%s: inconsistent MSB in source descriptors\n", __FUNCTION__);
                return (-1);
            }
        }
        ...
        if ((tinfo->dst_flags & DMA_ADDR_FLAG_NO_INCREMENT) == 0) {
            if ((tinfo->dst_addrs[sgi].paddr >> 32) != fixdar) {
                rcar_dma_slogf(SLOG2_ERROR, "%s: inconsistent MSB in destination descriptors\n", __FUNCTION__);
                return (-1);
            }
        }
        ...
        if (desc->tcr > RCAR_DMA_XFER_COUNT_MAX) {
            rcar_dma_slogf(SLOG2_ERROR, "%s: Transfer count %d is not exceeded 16,777,215\n", __FUNCTION__, desc->tcr);
            return -1;
        }
    ...
    chcr |= DMAC_CHCR_RPT | DMAC_CHCR_DPB | (chan->caie ? DMAC_CHCR_CAIE : 0);
...
dma_setup_xfer(void* const handle, const dma_transfer_t* const tinfo)
{
    ...
    switch (tinfo->xfer_unit_size) {
        case 2:
            chcr |= DMAC_CHCR_TS_WORD;
            break;
        case 4:
            chcr |= DMAC_CHCR_TS_LONG;
            break;
        case 8:
            chcr |= DMAC_CHCR_TS_8;
            break;
        case 16:
            chcr |= DMAC_CHCR_TS_16;
            break;
        case 32:
            chcr |= DMAC_CHCR_TS_32;
            break;
        case 64:
            chcr |= DMAC_CHCR_TS_64;
            break;
        default:
            chcr |= DMAC_CHCR_TS_BYTE;
            break;
    }

    if (tinfo->src_flags & DMA_ADDR_FLAG_DECREMENT) {
        chcr |= DMAC_CHCR_SM_DECR;
    } else if (!(tinfo->src_flags & DMA_ADDR_FLAG_NO_INCREMENT)) {
        chcr |= DMAC_CHCR_SM_INCR;
    }

    if (tinfo->dst_flags & DMA_ADDR_FLAG_DECREMENT) {
        chcr |= DMAC_CHCR_DM_DECR;
    }
    if (!(tinfo->dst_flags & DMA_ADDR_FLAG_NO_INCREMENT)) {
        chcr |= DMAC_CHCR_DM_INCR;
    }
    ...
    if ((tinfo->src_flags | tinfo->dst_flags) & DMA_ADDR_FLAG_DEVICE) {
        ...
        chcr |= DMAC_CHCR_RS_ONCHIP;
    } else {
        chcr |= DMAC_CHCR_RS_AUTO;
    }
    ...
    if ((tinfo->src_flags | tinfo->dst_flags) & DMA_ADDR_FLAG_SEGMENTED) {
        if (tinfo->mode_flags & DMA_MODE_FLAG_INFI_REPEAT) {
            chcr |= DMAC_CHCR_DPM_RPT_INF;
	    } else if (tinfo->mode_flags & DMA_MODE_FLAG_REPEAT) {
            chcr |= DMAC_CHCR_DPM_RPT;
        } else {
            chcr |= DMAC_CHCR_DPM_NORMAL;
        }
    ...
    if (chan->aflags & DMA_ATTACH_EVENT_ON_COMPLETE) {
        chcr |= DMAC_CHCR_IE | (chan->caie ? DMAC_CHCR_CAIE : 0);
    }
...
dma_start(void* const handle)
{
    ...
    chcr = in32(chan->regs + RCAR_DMAC_DMACHCR) & ~(DMAC_CHCR_TE|DMAC_CHCR_DSE|DMAC_CHCR_CAE);

    out32(chan->regs + RCAR_DMAC_DMACHCR, chcr | DMAC_CHCR_DE);
    if (chan->debug) {
        sysdmac_dump_registers(chan);
    }
...
dma_alloc_buffer(void* const handle, dma_addr_t* const addr, const unsigned size, const unsigned flags)
{
    ...
    if(addr->vaddr == MAP_FAILED) {
        rcar_dma_slogf(SLOG2_ERROR, "%s failed: %s\n", __FUNCTION__, strerror(errno));
        return errno;
    }
...
dma_abort(void* const handle)
{
    ...
    if ( (in32(chan->regs + RCAR_DMAC_DMACHCR) & DMAC_CHCR_CAE) ) {
        ...
        out32(chan->regs + RCAR_DMAC_DMACHCR,
                in32(chan->regs + RCAR_DMAC_DMACHCR) & ~(DMAC_CHCR_DE | DMAC_CHCR_DSIE | DMAC_CHCR_IE | DMAC_CHCR_CAIE | DMAC_CHCR_DSE | DMAC_CHCR_TE));
    }

    /* Even though we set the DMAC_CHCR_DE bit to 0, it will read as 1 until
     * the current transfer is complete; wait until the current transfer is complete
     * by checking that the DMAC_CHCR_DE bit reads 0 */
    for (i = 0; i < timeout; i++) {
        if ( !(in32(chan->regs + RCAR_DMAC_DMACHCR) & DMAC_CHCR_DE) ) {
            break;
        }
    ...
    if (i == timeout) {
        ...
        rcar_dma_slogf(SLOG2_ERROR, "%s: %s DMA chan %d: waited %d ms for DE clear\n",
...
dma_xfer_complete(void* const handle)
{
    ...
    out32(chan->regs + RCAR_DMAC_DMACHCR, status & ~(DMAC_CHCR_DSE | DMAC_CHCR_TE));

    /* Return the error status code as an indication of whether an error occurred or not */
    status = (status & DMAC_CHCR_CAE) ? (status & DMAC_CHCR_CAE) : EOK;
...
dma_check_xfer_complete(void* const handle)
{
    ...
    return  (in32(chan->regs + RCAR_DMAC_DMACHCR) & DMAC_CHCR_TE);
}
