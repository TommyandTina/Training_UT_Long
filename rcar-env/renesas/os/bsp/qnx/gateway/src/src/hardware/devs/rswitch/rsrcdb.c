...
int rswitch2_rsrcdb_create_resources(rswitch2_dev_t *rswitch2)
{
    ...
    if (gwca_rsrc_ret < 0) {
        gwca_rsrc_ret = errno;
        device_printf(rswitch2->dev, "%s: Failed querying %s resources", __func__, name);
        ...
        if( gwca_rsrc_ret != EOK ) {
            gwca_rsrc_ret = errno;
            device_printf(rswitch2->dev, "%s: Failed creating %s resources: err %d", __func__, name, gwca_rsrc_ret );
            return gwca_rsrc_ret;
        }
    } else {
    ...
    if (queue_rsrc_ret < 0) {
        ...
        if (queue_rsrc_ret != EOK) {
            queue_rsrc_ret = errno;
            device_printf(rswitch2->dev, "%s: Failed creating resources %s: err %d", __func__, name, queue_rsrc_ret);
            return queue_rsrc_ret;
        }
    } else {
...
int rswitch2_rsrcdb_attach_resources(rswitch2_dev_t *rswitch2, const rsrcdb_type_t type, const int length)
{
    ...
    if (rsrcdbmgr_attach(&req, 1) == -1) {
        device_printf(rswitch2->dev, "%s: rsrcdbmgr_attach %s failed: %s", __func__, name, strerror(errno));
        return -1;
    }
