...
rcar_config *rcar_get_board_config(size_t *num_devs) 
{
    ...
    /*  May need init I2C0 before ping to I2C device on the board */
    ...
    if (board_type == BOARD_EAGLE) {
        config_name = "v3m_eagle_config";
        config      = v3m_eagle_config;
        *num_devs   = NUM_ELTS(v3m_eagle_config);
    } else if (board_type == BOARD_STARTERKIT) {
        config_name = "v3m_sk_config";
        config      = v3m_sk_config;
        *num_devs   = NUM_ELTS(v3m_sk_config);
    } 
