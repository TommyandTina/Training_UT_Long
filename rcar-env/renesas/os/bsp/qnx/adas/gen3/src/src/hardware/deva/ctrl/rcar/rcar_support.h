...
typedef enum {
   ...
   RCAR_VERSION_E3,
   RCAR_VERSION_W2H,
   RCAR_VERSION_UNKNOWN = 0xFFFF
} rcar_version_t;

typedef enum {
   RCAR_GENERATION_2,
   RCAR_GENERATION_3,
   RCAR_GENERATION_UNKNOWN = 0xFFFF
} rcar_generation_t;


void rcar_version_init(void);
rcar_version_t rcar_version_parse(const char * str);
rcar_version_t rcar_version_get(void);
int rcar_version_set( rcar_version_t version );
rcar_generation_t rcar_generation_get(void);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/ctrl/rcar/rcar_support.h $ $Rev: 911285 $")
#endif
