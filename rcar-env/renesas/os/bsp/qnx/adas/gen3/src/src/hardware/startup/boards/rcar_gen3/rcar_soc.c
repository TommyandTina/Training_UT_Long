...
void usec_delay(uint32_t usec)
{
    ...
    uint8_t start_reg = in8(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TSTR);
    out8(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TSTR, start_reg & ~RCAR_GEN3_TMU_START0);
    ...
    out16(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TCR0, 0);
    out32(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TCOR0, usec);
    out32(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TCNT0, usec);

    /* Start TMU0 */
    out8(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TSTR, start_reg | RCAR_GEN3_TMU_START0);

    while(!(in16(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TCR0) & RCAR_GEN3_TMU_UNF));
    out8(RCAR_GEN3_TMU_0_2_BASE + RCAR_GEN3_TMU_TSTR, start_reg & ~RCAR_GEN3_TMU_START0);
}
...
static void rcar_mstp_config(uint32_t reg, uint32_t val)
{
    int timeout;
    uint32_t ctl_off = reg & 0xFFFF;
    uint32_t sts_off = (reg >> 16) & 0xFFFF;
    
    if (sts_off == 0)
        sts_off = ctl_off;

    out32(RCAR_GEN3_CPG_BASE + RCAR_GEN3_CPG_CPGWPR, ~val);
    out32(RCAR_GEN3_CPG_BASE + ctl_off, val);

    /* Based on R-Car Gen3 HW manual following info, after “clear the target bit of MSTPCRn (turn on clock)”,
     * the bit that target module in MSTPSRn shall be read back to confirm the status.
     */
    for (timeout = 100; timeout > 0; timeout--) {
        if (in32(RCAR_GEN3_CPG_BASE + sts_off) == val) {
            break;
        } else {
            usec_delay(1); /* 1 microsecond */
        }
    }
}

static void rcar_cpg_config(uint32_t reg, uint32_t val)
{
    int timeout;
    ...
    for (timeout = 100; timeout > 0; timeout--) {
        if (in32(RCAR_GEN3_CPG_BASE + reg) == val) {
            break;
        } else {
            usec_delay(1); /* 1 microsecond */
        }
    ...
    if(cpg != NULL)
    {
        ...
        while(p->clk.reg != 0)
        {
            if (tmp.reg != p->clk.reg)
            {
                if (tmp.reg != 0)
                {
                    rcar_mstp_config(tmp.reg, (in32(RCAR_GEN3_CPG_BASE + (tmp.reg & 0xFFFF)) & ~(tmp.bits)) | tmp.val);
                    PFC_DEBUG_PRINT("  module_clk disable clk reg=0x%x val=0x%x\n", RCAR_GEN3_CPG_BASE + (tmp.reg & 0xFFFF), in32(RCAR_GEN3_CPG_BASE + (tmp.reg & 0xFFFF)));
                }
                tmp.reg = p->clk.reg;
                tmp.bits = tmp.val = 0;
            }
...
void module_setup(rcar_config * config)
{
    if (!config)
        return;
...
void config_usbh(rcar_module *module)
{
    ...
    while(module->clk.reg != 0) {
        ...
        /* switch mode - host mode */
        out32(base + RCAR_GEN3_USB_UCOM_COMMCTRL, in32(base + RCAR_GEN3_USB_UCOM_COMMCTRL) & ~RCAR_GEN3_USB_UCOM_OTG_PERI);
...
void config_usbd(rcar_module *module)
{
    ...
    while(module->clk.reg != 0) {
        ...
        /* switch mode - peripheral mode */
        switch(module->base) {
            case RCAR_GEN3_HSUSB_BASE:
                out32(RCAR_GEN3_USB0_BASE + RCAR_GEN3_USB_UCOM_COMMCTRL, in32(RCAR_GEN3_USB0_BASE + RCAR_GEN3_USB_UCOM_COMMCTRL) | RCAR_GEN3_USB_UCOM_OTG_PERI_PERI);
                break;
            case RCAR_GEN3_HSUSB3_BASE:
                out32(RCAR_GEN3_USB3_BASE + RCAR_GEN3_USB_UCOM_COMMCTRL, in32(RCAR_GEN3_USB3_BASE + RCAR_GEN3_USB_UCOM_COMMCTRL) | RCAR_GEN3_USB_UCOM_OTG_PERI_PERI);
                break;
            default:
                break;
        }
...
void config_dmac(rcar_module *module)
{
    ...
    while(module->clk.reg != 0) {
        ...
        if (product_id == RCAR_V3H) {
            /* Read rate control */
            if (module->rate & 0xFF)
                out32(module->base + RCAR_GEN3_SYSDMAC_RATE_RD, BIT(31) | (module->rate & 0xFF));
            /* Write rate control */
            if ((module->rate >> 8) & 0xFF)
                out32(module->base + RCAR_GEN3_SYSDMAC_RATE_WR, BIT(31) | ((module->rate >> 8) & 0xFF));
        }
        module++;
    }
}
...
void config_impx5(rcar_module *module)
{
    ...
    again:
        ...
        out32(RCAR_GEN3_SYSC_BASE + RCAR_GEN3_SYSC_EXTMASK, 1);
        ...
        /* Finally. Clear SYSCEXTMASK */
         out32(RCAR_GEN3_SYSC_BASE + RCAR_GEN3_SYSC_EXTMASK, 0);
    }
}
...
/* Interface between ARM Realtime-Core domain (R7) & the AP System-Core domain (A53) */
void config_mfis (rcar_module *module)
{
    /* Software reset MFIS module */
    out32(RCAR_GEN3_CPG_BASE + RCAR_GEN3_CPG_SRCR2, in32(RCAR_GEN3_CPG_BASE + RCAR_GEN3_CPG_SRCR2)
        | (1 << 13));
    out32(RCAR_GEN3_CPG_BASE + RCAR_GEN3_CPG_SRSTCLR2, in32(RCAR_GEN3_CPG_BASE
        + RCAR_GEN3_CPG_SRSTCLR2) | (1 << 13));

    /* Supply clock for MFIS module */
    out32(RCAR_GEN3_CPG_BASE + RCAR_GEN3_CPG_RMSTPCR2, in32(RCAR_GEN3_CPG_BASE
        + RCAR_GEN3_CPG_RMSTPCR2) & ~(1 << 13));
    out32(RCAR_GEN3_CPG_BASE + RCAR_GEN3_CPG_SMSTPCR2, in32(RCAR_GEN3_CPG_BASE
        + RCAR_GEN3_CPG_SMSTPCR2) & ~(1 << 13));

    /* Disable write protection all register MFIS */
    out32(RCAR_GEN3_MFIS_BASE + RCAR_GEN3_MFIS_WPCNTR, RCAR_GEN3_MFIS_WPCNTR_CODE | RCAR_GEN3_MFIS_WPCNTR_WPD);
}

void config_gpu(rcar_module *module)
{
    if (product_id == RCAR_E3) {
        // There is a bug in the power on sequence of 3DG block on the E3 Rev1.0, see HW-17959
        if (product_rev == REV_1_0) {
            // Power on 3DG-B
            sysc_support_funcs.power_set(SYSC_PDR_GEN3_3DGB, 0);
            // Power on 3DG-A
            sysc_support_funcs.power_set(SYSC_PDR_GEN3_3DGA, 0);
        } else {
            // Power on 3DG-A
            sysc_support_funcs.power_set(SYSC_PDR_GEN3_3DGA, 0);
            // Power on 3DG-B
