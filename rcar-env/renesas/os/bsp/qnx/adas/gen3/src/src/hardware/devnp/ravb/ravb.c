...
static int ravb_dmac_init(ravb_dev_t *ravb)
{
    ...
    if (!(strstr(ravb->options.soc_name, "D3") || 
        ...
        out32(ravb->base + APSR, (ravb->options.txc_delay ? (1 << 14) : 0) | (ravb->options.rxc_delay ? (1 << 13) : 0));
    }
...
void ravb_rx_thread_quiesce (void *arg, int die)
{
    ravb_dev_t   *ravb = arg;
...
static int ravb_interrupt_init(ravb_dev_t  *ravb)
{
    ...
    if (cfg->num_irqs == 0) {
        if (strstr(ravb->options.soc_name, "V3U")){
            cfg->irq[0] = RCAR_V3U_INTCSYS_RAVB0_0;  //ch0 for rx
            cfg->irq[1] = RCAR_V3U_INTCSYS_RAVB0_1;  //ch1 for rx
            cfg->irq[2] = RCAR_V3U_INTCSYS_RAVB0_18; //ch18 for tx
            cfg->irq[3] = RCAR_V3U_INTCSYS_RAVB0_19; //ch19 for tx
            cfg->irq[4] = RCAR_V3U_INTCSYS_RAVB0_22; //ch22 for error & gPTP
            cfg->irq[5] = RCAR_V3U_INTCSYS_RAVB0_24; //ch24 for emac
        } else {
...
int ravb_attach (struct device *parent, struct device *self, void *aux)
{
    ...
    ravb->fd = posix_typed_mem_open( "below4G", O_RDWR, POSIX_TYPED_MEM_ALLOCATE_CONTIG);
    if (ravb->fd == -1) {
        return ENOMEM;
    }
...
#define OPT_AVB      5
    "mv88q5072",
#define OPT_MV88Q5072   6
    "txc_delay",
#define OPT_TXC_DELAY   7
    "rxc_delay",
#define OPT_RXC_DELAY   8
    NULL
};
    ...
    while (options && *options != '\0') {
        ...
        case OPT_FLOW:
            ...
            if (attach_args->set_flow > 3) {
                slogf(_SLOGC_NETWORK, _SLOG_ERROR, "ravb: Unknown flow value %d, forcing off", attach_args->set_flow);
...
static void ravb_parse_syspage (nic_config_t *cfg)
{
    ...
    if (cfg->num_irqs == 0) {
        ...
        /* Find IRQ */
        for(;;) {
            irq = hwitag_find_ivec(item, &idx);
            if (irq == HWI_ILLEGAL_VECTOR)
                break;
            cfg->irq[i++] = irq;
            cfg->num_irqs++;
        }
