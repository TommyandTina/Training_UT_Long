...
const struct sigevent *canfd_rx_irq(void *area, int id)
{
    ...
    for (rx_fifo_index = 0; rx_fifo_index < (CANFD_TOTAL_CHANNELS * CANFD_RECEIVE_FIFO_BUFFER); rx_fifo_index++)
    {
        ...
        /*mid*/
        if (CANFD_MODE == candev_init->interface)
        {
            /* CANFD interface */
        ...
        case CAN_DEVCTL_SET_MID:
            switch(cdev->devtype)
            {
                case CANDEV_TYPE_RX: /* rx */
                    ...
                    candev_init->channel[channel].rxid[index] = dcmd_data->mid;
                    val = GAFLID(dcmd_data->mid);                       /* Set rx ID */
                ...
                case CANDEV_TYPE_TX: /* tx */
                    candev_init->channel[channel].txid[index] = dcmd_data->mid;
                break;
        ...
        case CAN_DEVCTL_GET_MID:
            switch(cdev->devtype)
            {
                case CANDEV_TYPE_RX: /* rx */
                    ...
                    dcmd_data->mid = GAFLID(val);
                break;

                case CANDEV_TYPE_TX: /* tx */
                    dcmd_data->mid = candev_init->channel[channel].txid[index];
                break;
...
static void setting_baudrate(candev_info_t *candev_info)
{
    ...
    if (CANFD_MODE == candev_init->interface)
    {
        /* CANFD interface */
        /* Normal bit timing */
        /* Decrease to match with hardware manual page of 4818 */
        candev_init->nbrp -= 1;
        candev_init->nsjw -= 1;
    ...
    {
        /* Classic CAN interface */
        /* Decrease to match with hardware manual */
        candev_init->brp -= 1;
        candev_init->sjw -= 1;
        candev_init->tseg1 -= 1;
        candev_init->tseg2 -= 1;
...
static void setting_receive_rules(candev_info_t *candev_info)
{
    ...
    val = RNC(CHANNEL_0, CANFD_RECEIVE_FIFO_BUFFER);        /* 1 rule for each channel for V3U; 1 rule for channel 0 for Other */
    val |= RNC(CHANNEL_1,CANFD_RECEIVE_FIFO_BUFFER);        /* 1 rule for each channel for V3U; 1 rule for channel 1 for Other */
    if (product_id == PRODUCT_ID_RCAR_V3H && (product_rev == PRODUCT_V3H_REV_2_0 || product_rev == PRODUCT_V3H_REV_2_1))
        val |= RNC(CHANNEL_2,CANFD_RECEIVE_FIFO_BUFFER);    /* 1 rule for channel 2 in V3H20/V3H21 */ 
...
static void setting_buffer(candev_info_t *candev_info)
{
    ...
    if(candev_info->candev_init->interface == CANFD_MODE)
    ...
    for (channel = 0; channel < CANFD_TOTAL_CHANNELS; channel++)
    {
        for (i = 0; i < CANFD_TRANSMIT_RECEIVE_FIFO_BUFFER; i++)
        {
            ...
            val &= ~CFITSS;                                         /* Interval time = (2 x M x N) / pclk  */
            val &= ~CFITR;
...
void canfd_init_hw(candev_init_t *candev_init, candev_info_t *candev_info)
{
    ...
    while (GRAMINIT & in32(candev_info->base + RCAR_CANFD_RSCFD0CFDGSTS))
    {
        if (!timeout--)
        {
            LOG_ERROR("CAN RAM initialization is ongoing\n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    while (!(GRSTSTS & in32(candev_info->base + RCAR_CANFD_RSCFD0CFDGSTS)))
    {
        if (!timeout--)
        {
            LOG_ERROR("Global reset mode failed\n");
            exit(EXIT_FAILURE);
        }
    } 
    ...
    for (channel = 0; channel < CANFD_TOTAL_CHANNELS; channel++)
    {
        ...
        while (!(CRSTSTS & in32(candev_info->base + RCAR_CANFD_RSCFD0CFDCmSTS(channel))))
        {
            if (!timeout--)
            {
                LOG_ERROR("Channel %d reset mode failed !\n", channel);
                exit(EXIT_FAILURE);
            }
        }
    }
   ...
   /* Entering to global operating mode */
            ...
            in32(candev_info->base + RCAR_CANFD_RSCFD0CFDGSTS))
    {
        if (!timeout--)
        {
            LOG_ERROR("global operating failed !\n");
            exit(EXIT_FAILURE);
        }
    }
   /* Selecting operating mode */
    for (channel = 0; channel < CANFD_TOTAL_CHANNELS; channel++)
    {
        ...
        if (CHANNEL_COMMUNICATION_MODE != mode)
        {
            ...
            while (!(CHLTSTS & in32(candev_info->base + RCAR_CANFD_RSCFD0CFDCmSTS(channel))))
            {
                if (!timeout--)
                {
                    LOG_ERROR("Enter channel %d halt mode failed.\n", channel);
                    exit(EXIT_FAILURE);
                }
            }
    ...
    for (channel = 0; channel < CANFD_TOTAL_CHANNELS; channel++)
    {
       ...
       while (!(COMSTS & in32(candev_info->base + RCAR_CANFD_RSCFD0CFDCmSTS(channel))))
       {
           if (!timeout--)
           {
               LOG_ERROR("Enter channel %d communication failed.\n", channel);
               exit(EXIT_FAILURE);
           }
       }
    }
...
void canfd_init_tx_irq(candev_exp_t *candev_exp)
{
    ...
    if (-1 == r)
    {
        LOG_ERROR("InterruptAttach irqsys\n");
        exit(EXIT_FAILURE);
    }
}
...
//                    out32(candev_info->base + RCAR_CANFD_RSCFD0CFDCFFDCSTS(k), 0);
                }
            }
