...
#define SCIF_INTERNAL_SCKi_CLK_266MHZ	266660000
#define SCIF_INTERNAL_PCK_CLK_266MHZ	66660000
    ...
    if ((dev->tty.c_cflag & CREAD) && ( !(scscr & SCIF_SCSCR_RE) || !(scscr & SCIF_SCSCR_RIE)))
    {
        ...
        else if (dev->dma.status == DMA_ENABLE)
        {
            ...
            if (dev->dma.dmafuncs.setup_xfer(dev->dma.Rx.dma_chn, &tinfo)!=0){
                fprintf(stderr,"error setting up transfer!\n");
                return;
            }
            if (dev->dma.dmafuncs.xfer_start(dev->dma.Rx.dma_chn)!=0){
                fprintf(stderr,"error starting transfer!\n");
                return;
            }
        }
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devc/serscif/tto.c $ $Rev: 881606 $")
#endif
