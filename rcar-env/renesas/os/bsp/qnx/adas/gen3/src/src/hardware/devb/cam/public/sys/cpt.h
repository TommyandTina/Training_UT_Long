...
{
    _Uint32t    cam_path_id;        /* Path ID for the request */
    _Uint32t    cam_target_id;      /* Target device ID */
    _Uint64t    cam_target_lun;     /* Target LUN number */

    _Uint32t    cam_flags;          /* Flags for operation of the subsystem */

    _Uint8t     cam_status;         /* Returned CAM subsystem status */
    _Uint8t     cam_rsvd[3];

    _Uint8t     cam_scsi_status;    /* Returned scsi device status */
    _Uint8t     cam_vu_flags;
    _Uint8t     cam_func_code;

    _Uint8t     cam_sense_resid;    /* Autosense resid length: 2's comp */
    _Uint32t    cam_resid;          /* Transfer residual length: 2's comp */

    _Uint32t    cam_timeout;        /* Timeout value */

    _Uint8t     cam_sense_len;      /* Num of bytes in the Autosense buf */
    _Uint8t     cam_cdb_len;        /* Number of bytes for the CDB */
    _Uint16t    cam_sglist_cnt;     /* Num of scatter gather list entries */

    _Uint32t    cam_dxfer_len;      /* Data transfer length */

    _Uint64t    cam_sense_ptr;      /* Offset from start of struct to data */
    _Uint64t    cam_data_ptr;       /* physical address of buffer, or offset */
                                    /* from start of struct to scatter gather */
                                    /* list */
    _Uint8t     cam_cdb[16];        /* CDB bytes */
} CAM_PASS_THRU;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/cam/public/sys/cpt.h $ $Rev: 805883 $")
#endif
