...
int thermal_get_temperature(thermal_packet_t *result)
{
    ...
    for(i = dev->chan_min; i <= dev->chan_max ; i++) {
        if((ret = rcar_thermal_get_temp(dev->tsc[i], &temp)) != EOK)
            return ret;
...
int thermal_get_voltage(thermal_packet_t *result)
{
    ...
    for(i = dev->chan_min; i <= dev->chan_max ; i++) {
        if((ret = rcar_thermal_get_vol(dev->tsc[i], &mvolt)) != EOK)
            return ret;
...
int main(int argc, char *argv[])
{
    ...
    if(option_parse(argc, argv)) {
        thermal_slogf(VERBOSE_QUIET, "%s: Invalid option", argv[0]);
        exit(EXIT_FAILURE);
    }

    if (ThreadCtl(_NTO_TCTL_IO, 0) == -1) {
        thermal_slogf(VERBOSE_QUIET, "%s : ThreadCtl failed", argv[0]);
        exit(EXIT_FAILURE);
    }    

    /* Make sure the driver isn't already running */
    if(EOK != single_instance(RESMGR_NAME)) {
        thermal_slogf(VERBOSE_QUIET, "%s: Path name already exist", argv[0]);
        exit(EXIT_FAILURE);
    }

    dev = (rcar_tsc_dev_t *)calloc(1, sizeof(rcar_tsc_dev_t));
    if(!dev){
        thermal_slogf(VERBOSE_QUIET, "%s: Unable to allocate TMU device", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    if (NULL == dev->dpp) {
        thermal_slogf(VERBOSE_QUIET, "%s: Failed to create dispatch", argv[0]);
        free(dev);
        exit(EXIT_FAILURE);
    }
    ...
    if (dev->coid == -1) {
        thermal_slogf(VERBOSE_QUIET,"%s: unable to connect to side channel", argv[0]);
        resmgr_detach(dev->dpp, dev->resmgr_id, _RESMGR_DETACH_ALL);
        free(dev);
        exit(EXIT_FAILURE);
    }
    ...
    if(EOK != rcar_thermal_dev_init(dev)) {
        thermal_slogf(VERBOSE_QUIET,"%s: Parse device info failed.", argv[0]);
        resmgr_detach(dev->dpp, dev->resmgr_id, _RESMGR_DETACH_ALL);
        free(dev);
        exit(EXIT_FAILURE);
    }
