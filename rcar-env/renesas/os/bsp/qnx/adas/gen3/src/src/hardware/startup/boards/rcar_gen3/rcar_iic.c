...
static void iic_debug(const char * debug_str)
{
    ...
    icic = in8(RCAR_GEN3_IIC_BASE + IIC_ICIC);
    icsr = in8(RCAR_GEN3_IIC_BASE + IIC_ICSR);
    kprintf("%s\n", debug_str);
...
static uint8_t iic_wait(uint8_t flag)
{
    ...
    do {
        reg = in8(RCAR_GEN3_IIC_BASE + IIC_ICSR);
        try--;
    } while(try && !(reg & flag));
...
int iic_write_reg(uint8_t slave_addr, uint8_t reg_addr, uint8_t *data, uint8_t len)
{
    ...
    out8(RCAR_GEN3_IIC_BASE + IIC_ICCR, 0x01);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICCR, 0x81);

    out8(RCAR_GEN3_IIC_BASE + IIC_ICCL, 0x07);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICCH, 0x01);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICIC, IIC_TACK | IIC_WAIT);

    /* START condition */
    out8(RCAR_GEN3_IIC_BASE + IIC_ICIC, in8(RCAR_GEN3_IIC_BASE + IIC_ICIC) | IIC_DTE);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICCR, IIC_CMD_START);
    ...
    out8(RCAR_GEN3_IIC_BASE + IIC_ICIC, in8(RCAR_GEN3_IIC_BASE + IIC_ICIC) & ~IIC_DTE);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICDR, (slave_addr << 1) | IIC_WRITE);

    /* Write Reg Addr */
    if(iic_wait(IIC_WAIT)){
        return 1;
    }
    out8(RCAR_GEN3_IIC_BASE + IIC_ICDR, reg_addr);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICSR, in8(RCAR_GEN3_IIC_BASE + IIC_ICSR) & ~IIC_WAIT);
    ...
    for(i=0; i<len; i++) {
        ...
        out8(RCAR_GEN3_IIC_BASE + IIC_ICDR, data[i++]);
        out8(RCAR_GEN3_IIC_BASE + IIC_ICSR, in8(RCAR_GEN3_IIC_BASE + IIC_ICSR) & ~IIC_WAIT);
    }
    ...
    out8(RCAR_GEN3_IIC_BASE + IIC_ICCR, IIC_CMD_STOP_TX);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICSR, in8(RCAR_GEN3_IIC_BASE + IIC_ICSR) & ~IIC_WAIT);

    while(in8(RCAR_GEN3_IIC_BASE + IIC_ICSR) & IIC_BUSY);
...
int iic_read_reg(uint8_t slave_addr, uint8_t reg_addr, uint8_t *data, uint8_t len)
{
    ...
    out8(RCAR_GEN3_IIC_BASE + IIC_ICIC, in8(RCAR_GEN3_IIC_BASE + IIC_ICIC) & ~IIC_DTE);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICDR, (slave_addr << 1) | IIC_READ);

    /* TX TO RX */
    if(iic_wait(IIC_WAIT)){
        return 1;
    }
    out8(RCAR_GEN3_IIC_BASE + IIC_ICCR, IIC_CMD_TX_TO_RX);
    out8(RCAR_GEN3_IIC_BASE + IIC_ICSR, in8(RCAR_GEN3_IIC_BASE + IIC_ICSR) & ~IIC_WAIT);
    ...
    for(i=0; i<len; i++) {
        ...
        if((i+2) >= len) {
            out8(RCAR_GEN3_IIC_BASE + IIC_ICIC, in8(RCAR_GEN3_IIC_BASE + IIC_ICIC) | IIC_DTE);
            out8(RCAR_GEN3_IIC_BASE + IIC_ICCR, IIC_CMD_STOP_RX);
            out8(RCAR_GEN3_IIC_BASE + IIC_ICSR, in8(RCAR_GEN3_IIC_BASE + IIC_ICSR) & ~IIC_WAIT);
            break;
        }

        data[i++] = in8(RCAR_GEN3_IIC_BASE + IIC_ICDR);
        out8(RCAR_GEN3_IIC_BASE + IIC_ICSR, in8(RCAR_GEN3_IIC_BASE + IIC_ICSR) & ~IIC_WAIT);
    }
    ...
    out8(RCAR_GEN3_IIC_BASE + IIC_ICIC, in8(RCAR_GEN3_IIC_BASE + IIC_ICIC) & ~IIC_WAIT);
