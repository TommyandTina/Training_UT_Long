...
/**
* @brief Flags used in the @c flags field of the @c wfdcfg_timing structure
*/
enum wfdcfg_flags {
/**
...
#	define _b(x) ((_Uint32t)1 << (x))
	...
	WFDCFG_DOUBLECLOCK =_b(10),        /**< Use CEA-861-D, double clock modes */
	WFDCFG_BOTTOM_FIRST =_b(11),       /**< Bottom field first for interlacing */
	WFDCFG_QUADCLOCK =_b(12),          /**< Use quad clock modes */
...
/**
 *  @brief A port extension function that's called when there's a request to set the power mode on a port
 *  @details Members of @c #wfdcfg_keyval structure used for
 *           this extension are used as follows:
 *           - @c p: pointer to the function of type @c #wfdcfg_ext_fn_set_power_mode_t
 *           - @c i: set to 0
 */
#define WFDCFG_EXT_FN_SET_POWER_MODE "set_power_mode"

 /**
 * @brief Set the power mode on a port.
 * @details A function prototype for WFD drivers that choose to implement an extension when there's a request
 *          to the set power mode.
 *
 *          This function type must take the following arguments:
 *          - @c wfdcfg_port: A handle to the display port whose power mode is being set
 *          - @c #wfdcfg_power_mode power_mode: The power mode (of type @c #wfdcfg_power_mode) that the
 *                                              display port is to be set to
 *
 *          It must return a code from @c errno.h. Possible codes include:
 *          - @c EINVAL when an invalid port of power mode is passed
 *          - @c EOK on success
 */
 ...
 *
 *  @details This function creates one device. You must create at least one
 *           device. Failure to create a device results in the OpenWF display
 *           driver reporting an error.
 *
 ...
 *
 *  @details Memory that was allocated by wfdcfg_port_create()
 *           is released. The port's extension pointers are not valid after
 *           calling this function.
 *
 ...
 *
 *  @details Memory that was allocated by wfdcfg_mode_list_create()
 *           is released. The list's extension pointers are not valid after
 *           calling this function.
 *
...
/**
 *  @brief A port extension function that's called when there's a request to initialize a port
 *  @details Members of @c #wfdcfg_keyval structure used for
 *           this extension are used as follows:
 *           - @c p: pointer to the function of type @c #wfdcfg_ext_fn_port_init1_t
 *           - @c i: set to 0
 */
 ...
 * @details A function prototype for WFD drivers that choose to implement an extension when there's
 *          a request to create and initialize a port.
 *
 *          This function type must take the following argument:
 *          - @c wfdcfg_port: A handle to the display port which is to be initialized
 *
 *          It must return a code from @c errno.h. Possible codes include:
 *          - @c EINVAL when an invalid port is passed
 *          - @c EOK on success
 */
typedef int (wfdcfg_ext_fn_port_init1_t)(struct wfdcfg_port*);

/**
 *  @brief A port extension function that's called when there's a request to reset a port
 *  @details Members of @c #wfdcfg_keyval structure used for
 *           this extension are used as follows:
 *           - @c p: pointer to the function of type @c #wfdcfg_ext_fn_port_uninit1_t
 *           - @c i: set to 0
 */
 ...
 * @details A function prototype for WFD drivers that choose to implement an extension when there's a request to
 *          destroy a port.
 *
 *          This function type must take the following argument:
 *          - @c wfdcfg_port: A handle to the display port which is to be destroyed
 *
 *          It must return a code from @c errno.h. Possible codes include:
 *          - @c EINVAL when an invalid port is passed
 *          - @c EOK on success
...
/**
 *  @brief A port extension function that's called when there's a request to program a port
 *  @details Members of @c #wfdcfg_keyval structure used for
 *           this extension are used as follows:
 *           - @c p: pointer to the function of type @c #wfdcfg_ext_fn_port_set_mode2_t
 *           - @c i: set to 0
 */
#define WFDCFG_EXT_FN_PORT_SET_MODE2 "port_set_mode2"

/**
 * @brief Program a port.
 * @details A function prototype for WFD drivers that choose to implement an extension when there's a request to
 *          program a timing mode.
 *
 *          This function type must take the following arguments:
 *          - @c wfdcfg_port: A handle to the display port which is to be programmed
 *          - @c #wfdcfg_timing*: A handle to the mode (timing) with which you want this port programmed
 *
 *          It must return a code from @c errno.h. Possible codes include:
 *          - @c EINVAL when an invalid port is passed
 *          - @c EOK on success
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/release-2.1.x/lib/wfdcfg/public/wfdqnx/wfdcfg.h $ $Rev: 821897 $")
#endif
