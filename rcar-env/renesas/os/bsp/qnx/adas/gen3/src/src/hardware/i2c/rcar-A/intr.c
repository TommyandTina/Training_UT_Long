...
static inline int process_interrupts(rcar_i2c_dev_t *dev)
{
    ...
    out32(dev->regbase + RCAR_GEN3_I2C_ICMCR, in32(dev->regbase + RCAR_GEN3_I2C_ICMCR) & RCAR_I2C_MASK_DATA);

    msr = in32(dev->regbase + RCAR_GEN3_I2C_ICMSR);

    /* Just handle interrupts that are currently enabled */
    msr &= in32(dev->regbase + RCAR_GEN3_I2C_ICMIER);
    ...
    if (msr & RCAR_GEN3_ICMSR_MNR) {
        ...
        out32(dev->regbase + RCAR_GEN3_I2C_ICMSR, ~RCAR_GEN3_ICMSR_MNR & 0x7F);

        out32(dev->regbase + RCAR_GEN3_I2C_ICMIER, RCAR_GEN3_ICMSR_MST);
    ...
    /* Master Address Transmitted Interrupt */
    if (msr & RCAR_GEN3_ICMSR_MAT) {
        uint32_t msr_clear = RCAR_GEN3_ICMSR_MAT;

        if (msr & RCAR_GEN3_ICMSR_MDR) {
            if (!dev->isr) {
            ...
            if (dev->mode & CMODE_RECV) {
                if (dev->mode & CMODE_DODMA) {
                    ...
                    out32(dev->regbase + RCAR_GEN3_I2C_ICMCR, RCAR_I2C_DISABLE_START);
                    /* Enable DMA receive mode */
                    out32(dev->regbase + RCAR_GEN3_I2C_ICDMAER, RCAR_GEN3_ICDMAER_RMDMAE);
                    /* Start DMA transfer */
                    dev->dmafuncs.xfer_start(dev->dmahd);
                } else if ((dev->len == 1) && (dev->mode & CMODE_DOSTOP)) {
                    out32(dev->regbase + RCAR_GEN3_I2C_ICMCR, RCAR_I2C_PHASE_STOP);
                } else {
                    /* Disable Start Generation */
                    out32(dev->regbase + RCAR_GEN3_I2C_ICMCR, RCAR_I2C_DISABLE_START);
                }
            ...
            msr_clear |= RCAR_GEN3_ICMSR_MDR;
        } else if (msr & RCAR_GEN3_ICMSR_MDE) {
            if (!dev->isr) {
            ...
            msr_clear |= RCAR_GEN3_ICMSR_MDE;
        }

        out32(dev->regbase + RCAR_GEN3_I2C_ICMSR, ~(msr_clear) & 0x7F);
    ...
    if (msr & RCAR_GEN3_ICMSR_MDR) {
        if (!dev->isr) {
            rcar_i2c_slogf(dev, VERBOSE_LEVEL4, "%s: MDR interrupts, len %d", __FUNCTION__, dev->len);
        }

        if (dev->len) {
            /* Read one data byte */
            *dev->buf = in32(dev->regbase + RCAR_GEN3_I2C_ICRXD);
            dev->buf++; dev->len--;
        ...
        /* Clear MDR */
        out32(dev->regbase + RCAR_GEN3_I2C_ICMSR, ~RCAR_GEN3_ICMSR_MDR & 0x7F);

        return -1;
    }

    /* Master Data Empty Interrupt */
    if (msr & RCAR_GEN3_ICMSR_MDE) {
        if (dev->len) {
            if (!dev->isr) {
                rcar_i2c_slogf(dev, VERBOSE_LEVEL4,
            ...
            if (dev->mode & CMODE_DODMA) {
                ...
                out32(dev->regbase + RCAR_GEN3_I2C_ICDMAER, RCAR_GEN3_ICDMAER_TMDMAE);
                /* Start DMA transfer */
                dev->dmafuncs.xfer_start(dev->dmahd);
            } else {
                /* Write one data byte to TXD register */
                out32(dev->regbase + RCAR_GEN3_I2C_ICTXD, *dev->buf);
                dev->buf++;dev->len--;
            }
        } else {
            if (!dev->isr) {
        ...
        /* Need to clear MDE, if not the data transmission suspended */
        out32(dev->regbase + RCAR_GEN3_I2C_ICMSR, ~RCAR_GEN3_ICMSR_MDE & 0x7F);
...
int rcar_i2c_wait_complete(rcar_i2c_dev_t *dev) {
    ...
    to *= dev->scl_period * TIMEOUT_MULTI;
