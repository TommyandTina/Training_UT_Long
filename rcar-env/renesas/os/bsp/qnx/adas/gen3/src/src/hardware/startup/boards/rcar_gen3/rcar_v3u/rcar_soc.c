...
static uint32_t tmu_base = RCAR_GEN3_TMU_0_2_BASE;
...
static void tmu_delay(uint32_t cycles)
{
	...
	start_reg = in8(tmu_base + RCAR_GEN3_TMU_TSTR);
	out8(tmu_base + RCAR_GEN3_TMU_TSTR, start_reg & ~RCAR_GEN3_TMU_START0);

	out16(tmu_base + RCAR_GEN3_TMU_TCR0, 0);
	out32(tmu_base + RCAR_GEN3_TMU_TCOR0, cycles);
	out32(tmu_base + RCAR_GEN3_TMU_TCNT0, cycles);

	/* Start TMU0 */
	out8(tmu_base + RCAR_GEN3_TMU_TSTR, start_reg | RCAR_GEN3_TMU_START0);

	/* Check the countdown is 0 */
	while (!(in16(tmu_base + RCAR_GEN3_TMU_TCR0) & RCAR_GEN3_TMU_UNF));
...
{
	/* if CPGWPCR.WPE == 1 */
	out32(domain + RCAR_V3U_CPG_CPGWPR, ~val);
...
{
	out32(domain + gpio_gp + RCAR_V3U_GPIO_PMMER, 1);
	out32(domain + gpio_gp + RCAR_V3U_GPIO_PMMR, ~val);
...
void pfc_config(pfc_t *cfg)
{
	...
	/* Based on R-Car V3U HW Manual Rev.0.5
	 * Section 6.3.2
	...
	if (cfg->gpsr != NULL) {
		...
		while (gpsr->reg) {
			...
			if (gpsr->val) {
				...
				tmp |= BITMASK(gpsr->start_bit, gpsr->end_bit);
				pfc_reg_wr(cfg->domain, cfg->gpio_gp, gpsr->reg, tmp);
				SOC_DEBUG_PRINT("%s GPSR 0x%x:0x%x\n", __FUNCTION__, cfg->domain + cfg->gpio_gp + gpsr->reg, pfc_reg_rd(cfg->domain, cfg->gpio_gp, gpsr->reg));
			}
			gpsr++;
		}
	}
...
void sysc_config(sysc_t *cfg)
{
	...
	if (cfg->pdr != NULL) {
		pdr = cfg->pdr;
		while (pdr->index) {
			SOC_DEBUG_PRINT("%s domain=0x%x pdr=%d status=%d\n", __FUNCTION__, cfg->domain, pdr->index, pdr->status);
...
{
    while (o->base) {
        SOC_DEBUG_PRINT("%s base=0x%x reg=0x%x value=0x%x reg_len=%d\n", __FUNCTION__, o->base, o->reg, o->val, o->reg_len);
        if (o->reg_len == 16)  {
