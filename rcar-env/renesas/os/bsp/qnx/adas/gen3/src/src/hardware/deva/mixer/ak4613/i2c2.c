...
int i2c_write(uint32_t addr, uint8_t reg, uint8_t val, int index)
{
    int         status;
    struct OMSG omsg;
...
int i2c_read(uint32_t addr, uint8_t reg, uint8_t* val, int index)
{
    int             status;
    iov_t           siov[2], riov[2];
    unsigned char   bdata;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/mixer/ak4613/i2c2.c $ $Rev: 895184 $")
#endif
