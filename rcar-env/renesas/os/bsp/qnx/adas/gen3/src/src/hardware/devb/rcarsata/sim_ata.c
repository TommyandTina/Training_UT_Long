...
/******************************************************************************/
static uint64_t ata_tf_lba(EIDE_TARGET *tgt, int extended)
{
    ATA_TASKFILE    tf;
    uint64_t        lba;
...
/******************************************************************************/
static uint64_t ata_lba_error(EIDE_TARGET *tgt)
{
    uint64_t        lba = 0;
...
/******************************************************************************/
static int ata_error(EIDE_TARGET *tgt, int scsi_status, int cam_status)
{
...
/******************************************************************************/
static int ata_rw_cbf(EIDE_TARGET *tgt, uint8_t timeo)
{
    CCB_SCSIIO      *ccb = tgt->nexus;
...
/******************************************************************************/
static void ata_capacity(EIDE_TARGET *tgt, CCB_SCSIIO *ccb, int action)
{
    void            *dptr;
    READ_CAPACITY   *cp;
...
/******************************************************************************/
static int ata_std_inquiry(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    SCSI_INQUIRY    *iptr;
    ...
    memcpy((char *)iptr->prod_rev, (char *)tgt->identify.firmware_revision, 4);
    memcpy((char *)iptr->prod_id, (char *)tgt->identify.model_number, 16);
...
/******************************************************************************/
static int ata_evpd_inquiry(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    ...
    switch (ccb->cam_cdb_io.cam_cdb_bytes[2]) {
        ...
        case INQ_PC_UNIT_SERIAL_NUMBER:
            ...
            UNALIGNED_PUT16(vh->page_length, ENDIAN_BE16(20));
        ...
        case INQ_PC_DEVICE_IDENTIFICATION:
            if ((ident->cmd_setfeat_dflt & IDENT_CFD_VALID) &&
                ...
                UNALIGNED_PUT16(vh->page_length, ENDIAN_BE16(12));
                id->assoc_idtype    = IDT_NAA;
                id->proto_codeset   = CS_IDENTIFIER_BINARY;
                UNALIGNED_PUT16(id->identifier_length, ENDIAN_BE16(8));
                ...
                vh->peripheral      = D_DIR_ACC;
                vh->page_code       = INQ_PC_DEVICE_IDENTIFICATION;
                UNALIGNED_PUT16(vh->page_length, ENDIAN_BE16(71));
                id->assoc_idtype    = IDT_T10;
                id->proto_codeset   = CS_IDENTIFIER_ASCII;
                UNALIGNED_PUT16(id->identifier_length, ENDIAN_BE16(68));
                memcpy(is, "ATA     ", 8);
        ...
        case INQ_PC_BLOCK_LIMITS:
            ...
            if (IDENT_MAJOR_VERSION(ident->major_version) >= 7 &&
                ...
                UNALIGNED_PUT32(bl->max_unmap_lba_count, ENDIAN_BE32(EIDE_DSMR_MAX_LBA));
                UNALIGNED_PUT32(bl->max_unmap_desc_count, ENDIAN_BE32(EIDE_WS_SIZE / 8));
        ...
        case INQ_PC_BLOCK_DEVICE_CHARACTERISTICS:
            ...
            if (IDENT_MAJOR_VERSION(ident->major_version) >= 7 &&
                ...
                UNALIGNED_PUT16(bdc->medium_rotation_rate, ENDIAN_BE16(ident->rotation_rate));
            }
...
/******************************************************************************/
static void ata_inquiry(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    EXEC_TRACE;
...
/******************************************************************************/
static void ata_test_unit_ready(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    int             status;
...
/******************************************************************************/
static uint64_t ata_ccb_lba(uint8_t *cdb)
{
    uint64_t    lba;
...
/******************************************************************************/
static void ata_rw(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    uint16_t        sector;
    uint32_t        cylinder;
    int             multi;
...
/******************************************************************************/
static void ata_mpage_rw_err(EIDE_TARGET *tgt, CCB_SCSIIO *ccb, int op)
{
...
/******************************************************************************/
static int ata_write_cache(EIDE_TARGET *tgt, int op)
{
    SIM_HBA     *hba = tgt->hba;
    int         stat;
...
/******************************************************************************/
static void ata_mpage_caching(EIDE_TARGET *tgt, CCB_SCSIIO *ccb, int op)
{
    MPAGE_CACHING       *mpc;
    MODE_PARM_HEADER10  *mph;
...
/******************************************************************************/
static int ata_mode_sense(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    EXEC_TRACE;
    switch (ccb->cam_cdb_io.cam_cdb_bytes[2]) {
        case MP_RW_ERR:
...
/******************************************************************************/
static int ata_mode_select(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    MPAGE_CACHING       *mp;
    MODE_PARM_HEADER10  *mph;
...
/******************************************************************************/
static int ata_sync_cbf(EIDE_TARGET *tgt, uint8_t timeo)
{
    CCB_SCSIIO      *ccb = tgt->nexus;
...
/******************************************************************************/
static int ata_sync(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    SIM_HBA         *hba = tgt->hba;
    int             cmd;
...
/******************************************************************************/
static void ata_write_same(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    SIM_HBA         *hba = tgt->hba;
    ...
    ata_log(tgt, _SLOGC_SIM_RCAR_SATA, _SLOG_ERROR, 1, "%s (ptl %d:%d:%"PRIu64")",
...
/******************************************************************************/
static void ata_unmap(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    SIM_HBA                 *hba = tgt->hba;
    EIDE_WORKSPACE          *ws = &tgt->ws;
    CDB                     *cdb;
    ...
    ata_log(tgt, _SLOGC_SIM_RCAR_SATA, _SLOG_INFO, 5, "%s (ptl %d:%d:%"PRIu64")",
        __FUNCTION__, (int)hba->pathid, ccb->cam_ch.cam_target_id,
        ccb->cam_ch.cam_target_lun);
...
/******************************************************************************/
static void ata_service_action(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    int             action;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/rcarsata/sim_ata.c $ $Rev: 896546 $")
#endif
