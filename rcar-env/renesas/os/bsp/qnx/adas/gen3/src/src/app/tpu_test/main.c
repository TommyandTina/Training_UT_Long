...
void *tpu_intr_thread(void *data) {
	...
	if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_H3) {
		time = (1 / (RCAR_H3_TPU_INTERNAL_CLK / TPU_CLOCK_DIV)) * (0xFFFF) * n_overflow;
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_M3W) {
		time = (1 / (RCAR_M3_TPU_INTERNAL_CLK / TPU_CLOCK_DIV)) * (0xFFFF) * n_overflow;
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_M3N) {
		time = (1 / (RCAR_M3_TPU_INTERNAL_CLK / TPU_CLOCK_DIV)) * (0xFFFF) * n_overflow;
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_V3U) {
		time = (1 / (RCAR_V3_TPU_INTERNAL_CLK / TPU_CLOCK_DIV)) * (0xFFFF) * n_overflow;
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_V3M) {
		time = (1 / (RCAR_V3_TPU_INTERNAL_CLK / TPU_CLOCK_DIV)) * (0xFFFF) * n_overflow;
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_V3H) {
		time = (1 / (RCAR_V3_TPU_INTERNAL_CLK / TPU_CLOCK_DIV)) * (0xFFFF) * n_overflow;
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_D3) {
		time = (1 / (RCAR_D3_TPU_INTERNAL_CLK / TPU_CLOCK_DIV)) * (0xFFFF) * n_overflow;
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_E3) {
...
void pfc_setting_for_tpu_tpu0t01() 
{
    ...
    PFC_vbase  = mmap_device_io(RCAR_V3U_GPIO_SIZE,  RCAR_V3U_GPIO_DM0_BASE + RCAR_V3U_GPIO_GP2);
    if (PFC_vbase != (uintptr_t)MAP_FAILED) {
        
        tmp = in32(PFC_vbase + RCAR_V3U_GPIO_GPSR);
        tmp |= (1 << 22);
        
        out32(PFC_vbase + RCAR_V3U_GPIO_PMMR, ~tmp);
        out32(PFC_vbase + RCAR_V3U_GPIO_GPSR, tmp);
        
        tmp = in32(PFC_vbase + RCAR_V3U_GPIO_IP2SR);
        tmp &= ~(0xF << 24);
        
        out32(PFC_vbase + RCAR_V3U_GPIO_PMMR, ~tmp);
        out32(PFC_vbase + RCAR_V3U_GPIO_IP2SR, tmp);
        
        /* Unmap register */
        munmap_device_io(PFC_vbase, RCAR_V3U_GPIO_SIZE);
    }
	} else if(RCAR_GEN3_PRODUCT_ID(prr_val) == PRODUCT_ID_RCAR_D3) {
...
int main(int argc, char **argv) 
{
	...
	if((prr = mmap_device_io(4, RCAR_GEN3_PRODUCT_REGISTER)) == (uintptr_t)MAP_FAILED) {
    ...
    if ((cpg_mgr_getfuncs(&cpg_hwfuncs, sizeof(cpg_mgr_funcs_t))) != EOK) {
        ...
        cpg_hwfuncs.mstp_enable(CPG_MSTP_V3U_TPU0);
			tpu_irq = RCAR_V3U_INTCSYS_TPU;
		}
        cpg_hwfuncs.deinit();
    }
    ...
    if((tpu_hdl = tpu_hwfuncs.channel_attach(1, TPU_ATTACH_PRIORITY_STRICT | TPU_ATTACH_INTERRUPT_ENABLE)) == NULL) {
       tpu_slogf( _SLOG_ERROR,  0, "tpu_test : channel_attach failed (%s)", strerror(errno ));
       ConnectDetach(coid);
       ChannelDestroy(chid);
       return -1;
    }
#ifdef COUNTER_OPERATION_TESTING
	printf("----------------------------------------\n");
	...
	error++;
#else
#ifdef OUTPUT_A_WAVEFORM_TESTING
	printf("----------------------------------------\n");
    printf("     TPU output waveform testing\n");
    printf("----------------------------------------\n");
    error = tpu_hwfuncs.config_waveform(tpu_hdl, 100000, 200000);
#else
