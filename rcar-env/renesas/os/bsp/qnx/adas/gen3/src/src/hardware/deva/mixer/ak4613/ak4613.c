...
static int ak4613_open_i2c_fd ( ak4613_context_t *ak4613 )
{
    int status = EOK;
...
static int ak4613_close_i2c_fd ( ak4613_context_t *ak4613 )
{
    int status = EOK;
...
static int ak4613_i2c_write ( ak4613_context_t *ak4613, uint8_t reg, uint8_t val, int index )
{
    int status = EOK;
...
static int ak4613_i2c_read ( ak4613_context_t *ak4613, uint8_t reg, uint8_t* val, int index )
{
    int status = EOK;
    uint32_t reg_val;
...
int ak4613_init ( ak4613_context_t* ak4613 )
{
    ...
    if ( ak4613 == NULL ) {
        ado_error_fmt ( "ak4613 is NULL" );
        return EINVAL;
    }

    if ( (ak4613->params.tx_sample_rate != ak4613->params.rx_sample_rate) &&
         ak4613->params.tx_sample_rate && ak4613->params.rx_sample_rate ) {
        ado_error_fmt ( "tx and rx sample rate must be same" );
        return ENOTSUP;
    }

    if ( !ak4613->params.tx_sample_rate && !ak4613->params.rx_sample_rate ) {
        ado_error_fmt ( "at least one of tx and rx sample rate must be provided" );
        return ENOTSUP;
    }

    if ( !ak4613->params.tx_sample_rate ) {
        ak4613->params.tx_sample_rate = ak4613->params.rx_sample_rate;
    }

    if ( (ak4613->params.tx_sample_size != ak4613->params.rx_sample_size) &&
         ak4613->params.tx_sample_size && ak4613->params.rx_sample_size  ) {
        ado_error_fmt ( "tx and rx sample size must be same" );
        return ENOTSUP;
    }

    if ( ak4613->params.tx_sample_size != 0 && ak4613->params.tx_sample_size != 16
         && ak4613->params.tx_sample_size != 24 ) {
        ado_error_fmt ( "sample size %d not supported", ak4613->params.tx_sample_size );
        return ENOTSUP;
    }

    if ( ak4613->params.rx_sample_size != 0 && ak4613->params.rx_sample_size != 16
         && ak4613->params.rx_sample_size != 24 ) {
        ado_error_fmt ( "sample size %d not supported", ak4613->params.rx_sample_size );
        return ENOTSUP;
    }
    ...
    switch ( ak4613->params.tx_sample_rate ) {
        ...
        case 192000:
            ...
            break;
        default:
            ado_error_fmt ( "sample rate %d not supported", ak4613->params.tx_sample_rate );
            return ENOTSUP;
    }
    ...
    status = ak4613_open_i2c_fd( ak4613 );
    if ( status != EOK ) {
        return status;
    }
    ...
    data = 0x20;
    data |= tdm;
    status = ak4613_i2c_write ( ak4613, AK4613_CONTROL1, data, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    status = ak4613_i2c_write ( ak4613, AK4613_POWER_MANAGEMENT1, data, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    status = ak4613_i2c_write ( ak4613, AK4613_POWER_MANAGEMENT2, data, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    status = ak4613_i2c_write ( ak4613, AK4613_POWER_MANAGEMENT3, data, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    data = 0x00;
    data |= dfs;
    status = ak4613_i2c_write ( ak4613, AK4613_CONTROL2, data, 0 );
    if ( status != EOK ) {
        return status;
    }
     ...
     */
    status = ak4613_i2c_write ( ak4613, AK4613_DE_EMPHASIS1, 0x55, 0 );
    if ( status != EOK ) {
        return status;
    }
     ...
     */
    status = ak4613_i2c_write ( ak4613, AK4613_DE_EMPHASIS2, 0x05, 0 );
    if ( status != EOK ) {
        return status;
    }
     ...
     */
    status = ak4613_i2c_write ( ak4613, AK4613_OVERFLOW_DETECT, 0x07, 0 );
    if ( status != EOK ) {
        return status;
    }
     ...
     */
    status = ak4613_i2c_write ( ak4613, AK4613_ZERO_DETECT, 0x0F, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    status = ak4613_i2c_write ( ak4613, AK4613_INPUT_CONTROL, data, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    status = ak4613_i2c_write ( ak4613, AK4613_OUTPUT_CONTROL, data, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    for ( i = 0; i < AK4613_NUM_DACS; i++ ) {
        ...
        status = ak4613_i2c_write ( ak4613, AK4613_LOUT1_VOLUME_CONTROL + (i<<1), data, 0 );
        if ( status != EOK ) {
            return status;
        }

        status = ak4613_i2c_write ( ak4613, AK4613_ROUT1_VOLUME_CONTROL + (i<<1), data, 0 );
        if ( status != EOK ) {
            return status;
        }
    }
...
int ak4613_deinit( ak4613_context_t* ak4613 )
{
     ...
     */
    ak4613_i2c_write ( ak4613, AK4613_POWER_MANAGEMENT1, 0x0, 0 );
     ...
     */
    ak4613_i2c_write ( ak4613, AK4613_POWER_MANAGEMENT2, 0x04, 0 );
     ...
     */
    ak4613_i2c_write ( ak4613, AK4613_POWER_MANAGEMENT3, 0x0, 0 );

    if ( ak4613->debug ) {
        ak4613_register_dump ( ak4613 );
    }

    ak4613_close_i2c_fd ( ak4613 );
...
int ak4613_output_vol_get( ak4613_context_t* ak4613, unsigned voices, uint8_t* vol )
{
    ...
    for ( i = 0, dac = 0; dac < AK4613_NUM_DACS && i < voices; dac++ ) {
        if ( ak4613->dac_mask & (0x1 << dac) ) {
            status = ak4613_i2c_read ( ak4613, AK4613_LOUT1_VOLUME_CONTROL + (dac<<1), &data, 0 );
            if ( status != EOK ) {
                return status;
            }
            vol[i++] = AK4613_MAX_DIGITAL_VOL - data;

            status = ak4613_i2c_read ( ak4613, AK4613_ROUT1_VOLUME_CONTROL + (dac<<1), &data, 0 );
            if ( status != EOK ) {
                return status;
            }
            vol[i++] = AK4613_MAX_DIGITAL_VOL - data;
...
int ak4613_output_vol_set( ak4613_context_t* ak4613, unsigned voices, uint8_t* vol )
{
    ...
    for ( i = 0, dac = 0; dac < AK4613_NUM_DACS && i < voices; dac++ ) {
        if ( ak4613->dac_mask & (0x1 << dac) ) {
            status = ak4613_i2c_write ( ak4613, AK4613_LOUT1_VOLUME_CONTROL + (dac<<1), AK4613_MAX_DIGITAL_VOL - vol[i++], 0 );
            if ( status != EOK ) {
                return status;
            }
            status = ak4613_i2c_write ( ak4613, AK4613_ROUT1_VOLUME_CONTROL + (dac<<1), AK4613_MAX_DIGITAL_VOL - vol[i++], 0 );
            if ( status != EOK ) {
                return status;
            }
        }
...
int ak4613_output_mute_set( ak4613_context_t* ak4613, uint8_t mute )
{
    ...
    status = ak4613_i2c_read ( ak4613, AK4613_CONTROL1, &data, 0 );
    if ( status != EOK ) {
        return status;
    }
    ...
    return ak4613_i2c_write ( ak4613, AK4613_CONTROL1, data, 0 );
}
...
int ak4613_rate_setting( ak4613_context_t* ak4613, uint32_t sample_rate )
{
    ...
    status = ak4613_i2c_read ( ak4613, AK4613_CONTROL2, &data, 0 );
    if ( status == EOK ) {
...
int ak4613_capabilities( ak4613_context_t* ak4613, ado_mixer_dll_codec_capabilities_t *cap, int channel )
{
    ...
    if (channel == SND_PCM_CHANNEL_PLAYBACK) {
        if (ak4613->tdm) {
            ...
            cap->max_voices = ak4613_count_bits (ak4613->dac_mask) << 1;
        }
    } else {
        if (ak4613->tdm) {
            cap->max_voices = 4;
        } else {
            cap->max_voices = ak4613_count_bits (ak4613->adc_mask) << 1;
        }
    }
...
void ak4613_register_dump( ak4613_context_t* ak4613 )
{
    ...
    status = ak4613_i2c_read ( ak4613, AK4613_POWER_MANAGEMENT1, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_POWER_MANAGEMENT1 - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_POWER_MANAGEMENT2, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_POWER_MANAGEMENT2 - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_POWER_MANAGEMENT3, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_POWER_MANAGEMENT3 - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_CONTROL1, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_CONTROL1 - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_CONTROL2, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_CONTROL2 - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_DE_EMPHASIS1, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_DE_EMPHASIS1 - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_DE_EMPHASIS2, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_DE_EMPHASIS2 - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_OVERFLOW_DETECT, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_OVERFLOW_DETECT - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_ZERO_DETECT, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_ZERO_DETECT - 0x%02x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_INPUT_CONTROL, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_INPUT_CONTROL - 0x%08x", reg_val );
    }

    status = ak4613_i2c_read ( ak4613, AK4613_OUTPUT_CONTROL, &reg_val, 0 );
    if ( status == EOK ) {
        ado_debug ( DB_LVL_DRIVER, "AK4613_OUTPUT_CONTROL - 0x%08x", reg_val );
    }

    for (i = 0; i < AK4613_NUM_DACS; i++) {
        status = ak4613_i2c_read ( ak4613, AK4613_LOUT1_VOLUME_CONTROL + (i<<1), &reg_val, 0 );
        if ( status == EOK ) {
            ado_debug ( DB_LVL_DRIVER, "AK4613_LOUT%d_VOLUME_CONTROL - 0x%08x", 1 + i, reg_val );
        }
        status = ak4613_i2c_read ( ak4613, AK4613_ROUT1_VOLUME_CONTROL + (i<<1), &reg_val, 0 );
        if ( status == EOK ) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/mixer/ak4613/ak4613.c $ $Rev: 883503 $")
#endif
