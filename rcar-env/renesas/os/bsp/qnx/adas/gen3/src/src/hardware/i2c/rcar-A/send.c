...
rcar_i2c_send(void *hdl, void *buf, unsigned int len, unsigned int stop)
{
    ...
    rcar_i2c_slogf(dev, VERBOSE_LEVEL8, "[chan %d]%s: slave_addr[%02x] fmt[%08x]",
                dev->chan_idx, __FUNCTION__, dev->slave_addr, dev->slave_addr_fmt);

    if (len == 0) {
        rcar_i2c_slogf(dev, VERBOSE_LEVEL4, "[chan %d]%s: I2C_STATUS_DONE because of send length 0 bytes",
    ...
    out32(dev->regbase + RCAR_GEN3_I2C_ICMIER, RCAR_I2C_IRQ_SEND);
