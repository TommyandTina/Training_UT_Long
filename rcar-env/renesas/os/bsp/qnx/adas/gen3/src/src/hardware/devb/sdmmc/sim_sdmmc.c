...
//	cam_slogf( _SLOGC_SIM_MMC, _SLOG_INFO, 1, 1, "%s", __FUNCTION__ );
	...
	if( ( ext->eflags & SDMMC_EFLAG_BKOPS_AUTO ) ) {	// Device BKOPS
		if( sdmmc_bkops_cfg( hba, SDMMC_EFLAG_BKOPS_AUTO ) != EOK ) {
			cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  Error Enabling auto BKOPS", __FUNCTION__ );
			ext->eflags &= ~SDMMC_EFLAG_BKOPS_AUTO;
		}
	}
	else if( ext->eflags & SDMMC_EFLAG_BKOPS ) {			// Manual BKOPS
		if( sdmmc_bkops_cfg( hba, SDMMC_EFLAG_BKOPS ) != EOK ) {
			cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  Error Enabling manual BKOPS", __FUNCTION__ );
...
//	cam_slogf( _SLOGC_SIM_MMC, _SLOG_INFO, 1, 1, "%s", __FUNCTION__ );
	...
	if( ( ext->eflags & SDMMC_EFLAG_PWROFF_NOTIFY ) ) {
		if( ( status = sdmmc_pwroff_notify( hba, ext->pwroff_notify ) ) ) {
	...
	if( ( ext->eflags & SDMMC_EFLAG_BKOPS_AUTO ) ) {
		if( ( status = sdmmc_bkops_cfg( hba, 0 ) ) ) {
...
static int sdmmc_detach( SIM_HBA *hba )
{
	SIM_SDMMC_EXT	*ext;
...
__attribute__((used))
static void sdmmc_sdio_event( struct sdio_connection *connection, sdio_device_instance_t *instance, int ev )
{
	SIM_HBA				*hba;
	SIM_SDMMC_EXT		*ext;
	struct sdio_device	*device;
...
static int sdmmc_sense( CCB_SCSIIO *ccb, int sense, int asc, int ascq )
{
	SCSI_SENSE			*sptr;
...
static int sdmmc_ffu( SIM_HBA *hba, CCB_SCSIIO *ccb )
{
	...
	if( UNALIGNED_RET32(&cdb->write_buffer.buffer_id) == 0 ) {
		// Flush cache
		if( ( ext->dev_inf.caps & DEV_CAP_CACHE ) && ( ext->eflags & SDMMC_EFLAG_CACHE ) ) {
			if( ( status = sdio_cache( ext->device, SDIO_CACHE_FLUSH, SDIO_TIME_DEFAULT * 5 ) ) != EOK ) {
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  Error Cache Flush", __FUNCTION__ );
		...
		// Switch off Device BKOPS (eMMC 5.1)
		if( ( ext->eflags & SDMMC_EFLAG_BKOPS_AUTO ) ) {
			if( ( status = sdmmc_bkops_cfg( hba, 0 ) ) != EOK ) {
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  Error Disabling auto BKOPS", __FUNCTION__ );
				return( status );
			}
		}
...
int sdmmc_pwroff_notify( SIM_HBA *hba, uint8_t cfg )
{
	...
	cam_slogf( _SLOGC_SIM_MMC, _SLOG_INFO, 0, 0, "%s:  Setting POWER_OFF_NOTIFICATION to %d", __FUNCTION__, cfg );
...
int sdmmc_bkops_cfg( SIM_HBA *hba, int value )
{
	SIM_SDMMC_EXT	*ext;
	int				status = EIO;
	...
	if( ( value & SDMMC_EFLAG_BKOPS ) ) {
		if( ( status = sdio_mmc_switch( ext->device, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, ECSD_BKOPS_EN, ECSD_BKOPS_ENABLE, SDIO_TIME_DEFAULT ) ) != EOK ) {
			cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: switch ext_csd_bkops_en - ENABLE", __FUNCTION__ );
		}
	}
	else if( ( value & SDMMC_EFLAG_BKOPS_AUTO ) ) {
		if( !( ext->dev_inf.caps & DEV_CAP_BKOPS_AUTO ) ) {
			return( ENOTSUP );
		}
		if( ( status = sdio_mmc_switch( ext->device, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, ECSD_BKOPS_EN, ECSD_BKOPS_AUTO_EN, SDIO_TIME_DEFAULT ) ) != EOK ) {
			cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: switch ext_csd_bkops_en - AUTO_EN", __FUNCTION__ );
		}
	}
	else {
		if( ( status = sdio_mmc_switch( ext->device, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, ECSD_BKOPS_EN, 0, SDIO_TIME_DEFAULT ) ) != EOK ) {
			cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: switch ext_csd_bkops_en - 0", __FUNCTION__ );
		}
	}
	...
	switch( ext->bkops_status ) {
		...
		case BKOPS_STATUS_OPERATIONS_CRITICAL:
			...
			if ( ( status = sdio_mmc_switch( ext->device, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE,
				...
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s(%d): BKOPS_START BKOPS_STATUS_OPERATIONS_INPROG", __FUNCTION__,__LINE__);
...
static void sdmmc_ccb_lba( uint8_t *cdb, uint64_t *lba, uint32_t *nlba )
{
	switch( cdb[0] ) {
		case SC_READ10:
		case SC_WRITE10:
...
int sdmmc_read_write( SIM_HBA *hba, CCB_SCSIIO *ccb, int flgs )
{
	...
	sdmmc_ccb_lba( ccb->cam_cdb_io.cam_cdb_bytes, &lba, &nblks );
...
static int sdmmc_gencmd_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
	...
	for( ; ; ) {
		buf		= (void *)( req + 1 );
		flags	= SCF_CTYPE_ADTC | SCF_RSP_R1B;
		arg		= req->arg;
...
static void sdmmc_start_ccb( SIM_HBA *hba )
{
	...
	do {
		...
		if( status != CAM_REQ_INPROG ) {
			...
			// in case retune is needed
			if( sdio_retune( ext->device ) != EOK ) {
				sdmmc_reset( hba );
			}
		}
...
static int sdmmc_timer( SIM_HBA *hba )
{
	...
	if( ext->nexus || pm_state == PM_SLEEP || ext->drvr_state == SDMMC_DRVR_PAUSE ) {
		return( status );
	}
		...
		if( ext->drvr_state != ds->state ) {			// see if we are currently in the requested state
			...
			switch( ds->state ) {
				...
				default:
					...
					cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  Invalid state %d", __FUNCTION__, ds->state );
					break;
			}
		}
		else {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/sdmmc/sim_sdmmc.c $ $Rev: 906897 $")
#endif
