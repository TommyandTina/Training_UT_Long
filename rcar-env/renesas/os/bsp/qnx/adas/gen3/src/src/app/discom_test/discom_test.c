...
uint32_t calculate_crc(void)
{
    ...
    if(uif_input) {
        ...
        img_rect[2] = image_size[0];
        img_rect[3] = image_size[1];
    }
...
int main(int argc, char *argv[])
{
    ...
    while((opt = getopt_long(argc, argv,"d:i:r:a:", options, &index )) != -1) {
        switch (opt) {
             case 'd':
    ...
    if(!uif_rect[2] && !uif_rect[3]) {
        if(uif_input) {
            ...
            uif_rect[2] = screen_size[0];
            uif_rect[3] = screen_size[1];
        }
    }

    discom.pipe = uif_input ? pipeline : 0;
