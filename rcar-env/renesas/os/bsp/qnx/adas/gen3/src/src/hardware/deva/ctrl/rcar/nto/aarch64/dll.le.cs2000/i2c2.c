...
int open_i2c_fd(const char* devname)
{
    if( i2c_fd < 0 ) {
        i2c_fd = open(devname, O_RDWR);
        if (i2c_fd < 0) {
            ado_error_fmt("RCAR: i2c2: open(%s): %s", devname, strerror(errno));
            return ENODEV;
        }
        return EOK;
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/ctrl/rcar/nto/aarch64/dll.le.cs2000/i2c2.c $ $Rev: 895184 $")
#endif
