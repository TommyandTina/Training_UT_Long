...
/******************************************************************************/
ssize_t __attribute__((__format__(__printf__, 5, 6))) eide_slogf(EIDE_TARGET *tgt, int opcode,
...
/******************************************************************************/
static void eide_display_ccb(EIDE_TARGET *tgt, CCB *ccb_ptr)
{
    CCB_SCSIIO  *ccb;
    uint32_t    i;
    char        buf[50];
    ...
    for (ccb = ccb_ptr; ccb;
        ...
        switch (((CCB_HEADER *)ccb_ptr)->cam_func_code) {
            case XPT_SCSI_IO:
                ...
                eide_log(tgt, _SLOGC_SIM_RCAR_SATA, _SLOG_ERROR, log_level,
                        "addr=%p len=%d stat=%d path=%d targ=%d lun=%"PRId64""
                        "flag=%08x timeout=%d vuflag=%04x",
                        ccb->cam_ch.my_addr,
                ...
                eide_log(tgt, _SLOGC_SIM_RCAR_SATA, _SLOG_ERROR, log_level,
                        "pdrv=%p next=%p req_map=%p cbfcnp=%p xfer_len=%d "
                ...
                if (ccb->cam_ch.cam_flags & CAM_SCATTER_VALID) {
                    ...
                    for (i = 0; i < ccb->cam_sglist_cnt; i++, sg++) {
                        eide_log(tgt, _SLOGC_SIM_RCAR_SATA, _SLOG_ERROR, 5,
                            "sgentry %d: addr=0x%"PRIx64" count=%d",
                            i + 1, sg->cam_sg_address, sg->cam_sg_count);
                    }
                }
                else {
                    eide_log(tgt, _SLOGC_SIM_RCAR_SATA, _SLOG_ERROR, 5,
                            "data_ptr=0x%"PRIx64"", ccb->cam_data.cam_data_ptr);
                }
            ...
            case XPT_DEVCTL: {
                ...
                eide_log(tgt, _SLOGC_SIM_RCAR_SATA, _SLOG_ERROR, log_level,
                        ...
                        "targ=%d lun=%"PRId64" flag=%08x, dcmd=%x, dsize=%zu",
                        ccb->cam_func_code,
                        ccb->my_addr,
                        ccb->cam_ccb_len,
                        ccb->cam_status,
...
/******************************************************************************/
static void eide_strip_spaces(char *src, char *dst, int len)
{
    char    pchar;
...
/******************************************************************************/
static int eide_pio_mode(EIDE_TARGET *tgt, EIDE_IDENTIFY *ident)
{
    uint32_t        mode;
...
/******************************************************************************/
static int eide_mdma_mode(EIDE_TARGET *tgt)
{
    SIM_HBA         *hba;
    SIM_EIDE_EXT    *ext;
    EIDE_IDENTIFY   *ident;
...
/******************************************************************************/
static int eide_udma_mode(EIDE_TARGET *tgt, EIDE_IDENTIFY *ident)
{
    SIM_HBA         *hba;
    SIM_EIDE_EXT    *ext;
    uint32_t        mode;
...
/******************************************************************************/
static int eide_display_identify(EIDE_TARGET *tgt)
{
    SIM_HBA         *hba;
    EIDE_IDENTIFY   *ident;
    int             cudma, cmdma, pio;
    ...
    if (hba->verbosity > 2) {
        ...
        cam_dump((char *)ident, sizeof(*ident));
    }
...
/******************************************************************************/
static int eide_parse_identify(EIDE_TARGET *tgt, EIDE_IDENTIFY *ident)
{
    SIM_HBA         *hba;
    SIM_EIDE_EXT    *ext;
    uint32_t        cnt;
...
/******************************************************************************/
static int eide_identify_devices(SIM_HBA *hba)
{
    SIM_EIDE_EXT        *ext;
    EIDE_TARGET         *tgt;
    SIM_CONFIG_INFO     *cfg;
    ...
    eide_log(NULL, _SLOGC_SIM_RCAR_SATA, _SLOG_INFO, 5,
        ...
        "busno=0x%x, dfunc=0x%x, base=0x%"PRIx64", irq=0x%x",
        ...
        ext->IOPort_Base, cfg->IRQRegisters[0]);
...
// I/O command.
static int eide_scsi_io(EIDE_TARGET *tgt, CCB_SCSIIO *ccb)
{
    int             status;
    SIM_HBA         *hba = tgt->hba;
...
// asserted.
static int eide_reset_bus(SIM_HBA *hba, CCB_RESETBUS *ccb)
{
    EXEC_TRACE;
    UNUSED(hba);
    UNUSED(ccb);
...
static int eide_reset_dev(SIM_HBA *hba, CCB_RESETDEV *ccb)
{
    EXEC_TRACE;
    UNUSED(hba);
    UNUSED(ccb);
...
static int eide_rel_simq(SIM_HBA *hba, CCB_RELSIM *ccb)
{
    SIM_EIDE_EXT    *ext;
...
static int eide_term_io(SIM_HBA *hba, CCB_TERMIO *ccb)
{
    EXEC_TRACE;
    UNUSED(hba);
    UNUSED(ccb);
...
static int eide_path_inq(SIM_HBA *hba, CCB_PATHINQ *ccb)
{
    SIM_EIDE_EXT        *ext;
...
/******************************************************************************/
static int eide_ccb_abort(SIM_HBA *hba, CCB_ABORT *ccb_abort)
{
    EXEC_TRACE;
    UNUSED(hba);
    UNUSED(ccb_abort);
...
static int eide_power_mode(EIDE_TARGET *tgt, int mode)
{
    SIM_HBA         *hba = tgt->hba;
    int             status;
...
/******************************************************************************/
static int eide_standby_interval(EIDE_TARGET *tgt, int level, int interval)
{
    SIM_HBA         *hba = tgt->hba;
...
/******************************************************************************/
static int eide_apm_level(EIDE_TARGET *tgt, int level)
{
    SIM_HBA         *hba = tgt->hba;
...
/******************************************************************************/
static int eide_check_power_mode(EIDE_TARGET *tgt, uint32_t *level)
{
    ATA_TASKFILE    tf;
    SIM_HBA         *hba = tgt->hba;
    int             status;
...
/******************************************************************************/
static int eide_apm(EIDE_TARGET *tgt, CCB_DEVCTL *ccb)
{
    eide_apm_t      *apm;
    int             status;
    int             cstatus;
...
/******************************************************************************/
static int eide_dsn(EIDE_TARGET *tgt, CCB_DEVCTL *ccb)
{
    char            *dsn;
    int             ml;
    int             sl;
...
/******************************************************************************/
static int eide_verbosity(EIDE_TARGET *tgt, CCB_DEVCTL *ccb)
{
    SIM_HBA             *hba;
    CAM_VERBOSITY       *ver;
...
/******************************************************************************/
static int eide_endianize(resmgr_context_t *ctp, uint32_t dcmd, void *data, size_t nbytes)
{
    uint32_t            *dptr;
    int                 ssize;
    int                 status;
...
/******************************************************************************/
static int eide_devctl(EIDE_TARGET *tgt, CCB_DEVCTL *ccb)
{
    uint32_t        dcmd;
    int             estatus;
    int             cstatus;
...
/******************************************************************************/
static int eide_timer(SIM_HBA *hba)
{
    SIM_EIDE_EXT    *ext = (SIM_EIDE_EXT *)hba->ext;
    EIDE_TARGET     *tgt;
    CCB_SCSIIO      *ccb;
...
/******************************************************************************/
static void eide_pulse_handler(SIM_HBA *hba)
{
    struct _pulse   pulse;
    int             rid;
    ...
    while (1) {
        ...
        switch (pulse.code) {
            ...
            default:
                EXEC_TRACE;
                eide_log(NULL, _SLOGC_SIM_RCAR_SATA, _SLOG_ERROR, 1,
                    "%s: unknown type %x, subtype %d, pulse %x, "
                    "value %x, scoid %x", __FUNCTION__, pulse.type,
                    pulse.subtype, pulse.code, pulse.value.sival_int, pulse.scoid);
                break;
        }
        eide_start_ccb(hba);
    }
...
/******************************************************************************/
static void *eide_driver_thread(void *data)
{
    SIM_HBA         *hba;
    SIM_EIDE_EXT    *ext;
    EIDE_TARGET     *tgt;
...
/******************************************************************************/
__attribute__((used))
static int eide_geometry(EIDE_TARGET *tgt, char *arg)
{
    int     heads;
    int     cyls;
    int     sects;
    ...
    while (*options != '\0') {
        ...
        switch (opt) {
            case 0: /* Verbose */
                ...
                tgt->verbosity = strtoul(value, NULL, 0);
                break;
            ...
            case 3: /* Multi-block */
                EIDE_ARG_VAL(opts[opt], value);
                if (!(tgt->rw_multiple = strtoul(value, NULL, 0))) {
                    tgt->tflags &= ~EIDE_TFLG_USE_MBLK;
                    tgt->rw_multiple = 1;
                }
                break;
    ...
    while (*options != '\0') {
        ...
        switch (opt) {
            case 0: /* SATA controller registers base address */
                ...
                ext->IOPort_Base = hba->cfg.IOPort_Base[0] = strtoull(value, NULL, 0);
            ...
            case 1: /* Length of SATA controller registers address space */
                if (hba->cfg.IOPort_Length[0])
                    break;
                EIDE_ARG_VAL(opts[opt], value);
                hba->cfg.IOPort_Length[0] = strtoul(value, NULL, 0);
                break;
            case 2: /* IRQ */
                if (hba->cfg.NumIRQs)
                    break;
                EIDE_ARG_VAL(opts[opt], value);
                hba->cfg.IRQRegisters[0] = strtoul(value, NULL, 0);
                hba->cfg.NumIRQs = 1;
                break;
            case 3: /* Verbose */
                EIDE_ARG_VAL(opts[opt], value);
                hba->verbosity = strtoul(value, NULL, 0);
                tgt = &ext->targets[0];
                if (!tgt->verbosity) {
                    tgt->verbosity = hba->verbosity;
                }
...
static int eide_xpt_device_notify( SIM_HBA *hba, int op )
{
	SIM_CONFIG_INFO *cfg;
	XPT_DEVICE		dev;
	int				idx;
	...
	for( idx = 0; idx < cfg->NumMemWindows; idx++ ) {
		dev.mmio.phys	= cfg->MemBase[idx];
		dev.mmio.len	= cfg->MemLength[idx] ? cfg->MemLength[idx] : SATA_DEFAULT_BASE_LEN;

		status = op ? xpt_device_register( &dev, 0, NULL ) : xpt_device_deregister( &dev );
		if( status ) {
			eide_slogf( NULL, _SLOGC_SIM_AHCI, _SLOG_ERROR, 0, "%s: xpt_device_%sregister phys 0x%"PRIx64" len %d failure %s",
				__FUNCTION__, op ? "" : "de", cfg->MemBase[idx], cfg->MemLength[idx], strerror( status ) );
		}
	}

	for( idx = 0; idx < cfg->NumIOPorts; idx++ ) {
		dev.mmio.phys	= cfg->IOPort_Base[idx];
		dev.mmio.len	= cfg->IOPort_Length[idx] ? cfg->IOPort_Length[idx]: SATA_DEFAULT_BASE_LEN;
		status = op ? xpt_device_register( &dev, 0, NULL ) : xpt_device_deregister( &dev );
		if( status ) {
			eide_slogf( NULL, _SLOGC_SIM_AHCI, _SLOG_ERROR, 0, "%s: xpt_device_%sregister 0x%"PRIx64" len %dfailure %s",
				__FUNCTION__, op ? "" : "de", cfg->IOPort_Base[idx], cfg->IOPort_Length[idx], strerror( status ) );
		}
	}
...
int eide_sim_idetach(SIM_HBA *hba)
{
    ...
    eide_xpt_device_notify( hba, CAM_FALSE );
...
int eide_sim_attach(CAM_ENTRY *centry)
{
    ...
    for (hba = TAILQ_FIRST(&eide_ctrl.hlist); hba; hba = nhba) {
        ...
        eide_xpt_device_notify( hba, CAM_TRUE );
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/rcarsata/sim_eide.c $ $Rev: 896546 $")
#endif
