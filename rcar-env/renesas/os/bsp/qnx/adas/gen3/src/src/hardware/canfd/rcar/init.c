...
#define CS_MACHINE_LEN (32 + 1)
    ...
    if (options) {
        if ( strstr( options, "H3") || strstr( options, "h3") ) {
            ...
            if(strstr( options, "2.0")) 
                product_rev = PRODUCT_V3H_REV_2_0;
            else if(strstr( options, "2.1")) 
                product_rev = PRODUCT_V3H_REV_2_1;
            else
                product_rev =PRODUCT_REV_UNKNOWN;
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        LOG_WARNING("HWINFO Failed to pull in info! Get info by reading PRODUCT_ID register\n");
    }
    ...
    /* Parsing options */
    while ((opt = getopt(argc, argv, "b:d:u:m:t:x:i")) != -1)
    {
        switch (opt)
        {
            /* Set normal bitrate */
            ...
            /* Select classic CAN or CANFD interface, default is CANFD interface */
            case 'i':
                candev_init.interface = CLASSIC_CAN_MODE;
            break;
...
static int get_canfd_hwinfo(CANFDDEV_HWINFO *candev, unsigned unit)
{
    ...
    hwi_tag *tag;
     ...
     if (hwi_off != HWI_NULL_OFF)
     {
        ...
        {
            unsigned instance = 1;
            tag = hwi_tag_find(hwi_off, HWI_TAG_NAME_location, &instance);
