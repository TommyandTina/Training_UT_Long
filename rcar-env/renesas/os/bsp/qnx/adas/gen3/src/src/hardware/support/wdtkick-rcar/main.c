...
static void write_wdt_32(uintptr_t addr, uint32_t data) {
	out32(addr, data);
}

static void write_wdt_16(uintptr_t addr, uint32_t data) {
	out16(addr, (uint16_t)data);
}

static void write_wdt_8(uintptr_t addr, uint32_t data) {
	out8(addr, (uint8_t)data);
}

static uint32_t read_wdt_32(uintptr_t addr) {
	return in32(addr);
}

static uint32_t read_wdt_16(uintptr_t addr) {
	return (uint32_t)in16(addr);
}

static uint32_t read_wdt_8(uintptr_t addr) {
