    ...
    uint32_t * vol, void *instance_data)
{
    ...
    ak4613_output_vol_get( ak4613, ak4613->params.tx_voices, data );
    ...
    if (set) {
        ...
        if ( altered ) {
            ak4613_output_vol_set( ak4613, ak4613->params.tx_voices, data );
        }
    } else { /* read volume */
        for ( i = 0; i < ak4613->params.tx_voices; i++ ) {
            vol[i] = data[i];
    ...
    uint32_t * mute, void *instance_data)
{
    ...
    ak4613_output_mute_get(ak4613, &data);

    /* implement joint-mute, as the mute bit for L,R channels is common */
    if (set) {
        altered = data != *mute;
        if (altered) {
            ak4613_output_mute_set(ak4613, *mute);
        }
    } else { /* read mute */
        *mute = data;
    }
...
build_ak4613_mixer(MIXER_CONTEXT_T * ak4613)
{
    ...
    /* the OUTPUT GROUPS */
    /* ################# */
    if (!error && (elem = ado_mixer_element_pcm1 (ak4613->mixer, SND_MIXER_ELEMENT_PLAYBACK,
                SND_MIXER_ETYPE_PLAYBACK1, 1, &pcm_devices[0])) == NULL)
        error++;

    pre_elem = elem;

    if (!error && (vol_elem = ado_mixer_element_volume1 (ak4613->mixer, "Digital Volume",
    ...
    if (!error && (mute_elem = ado_mixer_element_sw2 (ak4613->mixer, "Digital Mute",
...
void ak4613_mixer_register_dump( MIXER_CONTEXT_T *ak4613 )
{
...
{
    ak4613_context_t *ak4613 = ado_mixer_get_context(mixer);

    ado_debug ( DB_LVL_DRIVER, "Setting AK4613 sample rate to %d", sample_rate );

    ak4613_rate_setting ( ak4613, sample_rate );
}

static void ak4613_mixer_capabilities (ado_mixer_t *mixer, ado_mixer_dll_codec_capabilities_t *cap, int channel)
{
    ak4613_context_t *ak4613 = ado_mixer_get_context(mixer);

    ak4613_capabilities ( ak4613, cap, channel );
}
...
int mixer_dll_init (MIXER_CONTEXT_T ** context, ado_mixer_t * mixer, void *params, void *raw_callbacks, int version)
{
    ...
    *context = ak4613;
    ...
    if (callbacks) {
        ...
        callbacks->codec_capabilities = ak4613_mixer_capabilities;
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/mixer/ak4613/ak4613_mixer.c $ $Rev: 895361 $")
#endif
