...
static void rpc_hyper_ext_mode_init(rpc_dev_t *dev)
{
    ...
    //out32(rpc->vbase + RCAR_RPC_PHYOFFSET1, RCAR_RPC_PHYOFFSET1_DDRTMG(3)
    //                        | RCAR_RPC_PHYOFFSET1_REV);
    out32(rpc->vbase + RCAR_RPC_PHYOFFSET1, RCAR_RPC_PHYOFFSET1_DDRTMG(2)
                            | RCAR_RPC_PHYOFFSET1_REV);
...
static int rpc_bits_set(int size)
{
    return ((size >> 1) & 0x2);
}
...
static int rpc_qspi_ext_read(void *hdl, uint8_t cmd, int csize, uint32_t addr, int alen, int asize, void *data, int dlen, int dsize, int dummy)
{
     ...
     */
    out32(rpc->vbase + RCAR_RPC_PHYCNT, RCAR_RPC_PHYCNT_STRTIM(6) // Internal strobe delay
                            | RCAR_RPC_CMNCR_REV);
    out32(rpc->vbase + RCAR_RPC_PHYCNT, in32(rpc->vbase + RCAR_RPC_PHYCNT)
...
static int rpc_qspi_read(void *hdl, uint8_t cmd, int csize, uint32_t addr, int alen, int asize, void *data, int dlen, int dsize, int dummy)
{
    ...
    smenr = RCAR_RPC_SMENR_CDB(rpc_bits_set(csize))    // Command Bit Size : 1/4 bits
          | RCAR_RPC_SMENR_ADB(rpc_bits_set(asize))    // Address Bit Size : 1/4 bits
          | RCAR_RPC_SMENR_SPIDB(rpc_bits_set(dsize))  // Transfer Data Bit Size : 1/4 bits
...
static int rpc_qspi_write_buffer(void *hdl, uint8_t cmd, int csize, uint32_t addr, int alen, int asize, uint8_t *data, int dlen, int dsize)
{
    ...
    /* Set Manual Mode */
    smenr = RCAR_RPC_SMENR_CDB(rpc_bits_set(csize))     
           | RCAR_RPC_SMENR_ADB(rpc_bits_set(asize))    	
           | RCAR_RPC_SMENR_SPIDB(rpc_bits_set(dsize))
           | RCAR_RPC_SMENR_CDE            				     
           | RCAR_RPC_SMENR_SPIDE(rpc_bytes_set(dlen));
    if (alen == 3)
    ...
    out32(rpc->vbase + RCAR_RPC_PHYCNT, 0
                            | RCAR_RPC_PHYCNT_PHYMEM(0)   // Serial flash in SDR mode
                            | RCAR_RPC_PHYCNT_STRTIM(6)   // Internal strobe delay
...
static int rpc_qspi_write(void *hdl, uint8_t cmd, int csize, uint32_t addr, int alen, int asize, void *data, int dlen, int dsize)
{
    ...
    smenr = RCAR_RPC_SMENR_SPIDB(rpc_bits_set(dsize));   // Transfer Data Bit Size : 1/4 bits
    if ((dlen - pos) >= 4)
...
static void query_hwinfo(rpc_dev_t *dev)
{
    ...
    rcar_rpc.pbase = RCAR_GEN3_RPC_BASE;
    rcar_rpc.size = RCAR_GEN3_RPC_SIZE;
...
void* rpc_init(f3s_socket_t *socket)
{
    ...
    char                    *freeptr;
    char                    *options;
    static char             *opts[] = {"cs", "drate", "mode", "clock", "base", "size", "ver", "typed_mem", "smmu", "wbuf", NULL};
        ...
        while (options && *options != '\0') {
            ...
            case 6:     // ver
                if(strlen(value)) {
                    ...
                    rcar_rpc.dma_ver = (char *)alloca(len);
            ...
            case 7:     // typed_mem
                if(strlen(value)) {
                    len = strlen(value) + 1;
                    rcar_rpc.dma_typed_mem = (char *)alloca(len);
