...
const struct sigevent * do_interrupt(void *area)
{
    DEV_SCIF        *dev;
    int             status = EOK;
    ...
    for (dev = ((struct dev_list*)area)->device; dev != NULL; dev = dev->next)
    {
        ...
        else /* DMA enabled */
        {
            ...
            if ((in16(dev->port + SCIF_SCLSR_OFF) & SCIF_SCLSR_TO))
            {
                ...
                {
                    /* Set EVENT_CUSTOM to process RX DMA data at thread time */
                    atomic_set(&dev->tty.eflags, EVENT_CUSTOM);
                    status |= 1;
        ...
        if((dev->mode==INTR_EVENT) && status)
        {
            iochar_send_event (&dev->tty);
        }
        // Return the status if any to the io-char library
        else if (status && !(dev->tty.flags & EVENT_QUEUED))
        {
            event = &ttyctrl.event;
            dev_lock(&ttyctrl);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devc/serscif/intr.c $ $Rev: 893164 $")
#endif
