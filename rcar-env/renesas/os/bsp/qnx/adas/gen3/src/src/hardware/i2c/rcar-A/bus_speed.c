...
int rcar_i2c_set_bus_speed(void *hdl, unsigned int speed, unsigned int *ospeed)
{
    ...
    /* Support up to Fast mode + */
    if (dev->fast_mode_plus) {
        if (speed > I2C_FASTPLUS_BAUD) {
            rcar_i2c_slogf(dev, VERBOSE_QUIET, "[chan %d]%s: set speed to 1MHz at Fast Mode+", dev->chan_idx, __FUNCTION__);
            speed = I2C_FASTPLUS_BAUD;
        }
    } else if (speed > I2C_FAST_BAUD) {
...
{
#ifdef USING_EQUATION_3
    out32(dev->regbase + RCAR_GEN3_I2C_ICCCR2, 0x7 | //CDFD = 1, HLSE = 1, and SME = 1
          (((dev->fast_mode_plus) ? 1 : 0) << 7));   //Fastmode+
    out32(dev->regbase + RCAR_GEN3_I2C_ICCCR, 0x6);  //Clock to filter glitches = 133.3/(1 + 6) = 19Mhz    
    out32(dev->regbase + RCAR_GEN3_I2C_ICMPR, dev->smd);
    out32(dev->regbase + RCAR_GEN3_I2C_ICHPR, dev->schd);
    out32(dev->regbase + RCAR_GEN3_I2C_ICLPR, dev->scld);
#else
    /* For SCL rate calculation using Equation 2 */
    out32(dev->regbase + RCAR_GEN3_I2C_ICCCR2, 0);
    out32(dev->regbase + RCAR_GEN3_I2C_ICCCR, dev->icccr);
#endif
    /* 1st bit setup cycle */
	out32(dev->regbase + RCAR_GEN3_I2C_ICFBSCR, 0x07);
}
