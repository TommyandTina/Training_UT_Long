...
static int _mmc_switch( sdio_dev_t *dev, uint32_t cmdset, uint32_t mode, uint32_t index, uint32_t value, uint32_t timeout, uint32_t wrdy )
{
	...
	sdio_setup_cmd( cmd, SCF_CTYPE_AC | rsp, MMC_SWITCH,
...
int mmc_init_hs400es( sdio_hc_t *hc, int bus_width )
{
	sdio_dev_t  *dev;
	int			status;
	int			bus_mode;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/sdmmc/sdiodi/mmc.c $ $Rev: 900145 $")
#endif
