/*
 * $QNXLicenseC:
 * Copyright 2011, 2021-2022 QNX Software Systems.
...
//kprintf("writing GICD_IROUTER<%u> @ 0x%P\n", reg_idx, gicd_paddr_base + ARM_GICD_IROUTERn + reg_offset);
	...
	if (inter_core_intr_errata() == true) {
		kprintf("%s: Applying Inter Core interrupt Errata\n", __func__);
	...
	if (lpi_params.num_lpis > 0)
	{
		...
		for (unsigned tbl_num=0; tbl_num<lpi_params.cfg.num_tbls; tbl_num++)
		{
			...
			lpi_cfg_tbl_params->propbaser |= ((_Uint64t)(bits - 1));
...
static void gic_v3_gicc_init(const unsigned cpu)
{
	...
	if (inter_core_intr_errata() == false) {
		/* Set SGI/PPI priority (all 0xA0, except for SGI0 which is 0xC0) */
		out32(gic_core_gicr + ARM_GICR_IPRIORITYR0, 0xA0A0A0C0);
	} else {
		/* Set SGI/PPI priority (all 0xA0, if need to apply errata) */
		out32(gic_core_gicr + ARM_GICR_IPRIORITYR0, 0xA0A0A0A0);
	}
	...
	if ((gic_sendipi == &sendipi_gic_v3_mm) && (inter_core_intr_errata() == false))
	{
	...
	else
	{
		...
		else
		{
			...
			_Uint32t icc_sgi1r = 0x01U;
			//XXXX: Looses Aff3 value
			icc_sgi1r |= (cpu_aff & 0xFFFF00);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/startup/boards/rcar_gen4/gic_v3.c $ $Rev: 957830 $")
#endif
