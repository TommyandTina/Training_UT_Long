...
#define CANFD_REG_BASE                              (IS_RCAR_GEN4(product_id) ? 0xE6660000 : 0xE66C0000)

/* IRQ vector */        
#define CANFD_IRQ_GL                                (IS_RCAR_GEN4(product_id) ? ((product_id == PRODUCT_ID_RCAR_V4H) ? 413 + 32 : 26 + 32) : 30 + 32)
#define CANFD_IRQ_CH                                (IS_RCAR_GEN4(product_id) ? ((product_id == PRODUCT_ID_RCAR_V4H) ? 412 + 32 : 25 + 32) : 29 + 32)
#define CANFD_TOTAL_CHANNELS_MAX                    (8)
#define CANFD_TOTAL_CHANNELS                        (IS_RCAR_GEN4(product_id) ? 8 : ((product_id == PRODUCT_ID_RCAR_V3H) && \
                                                                ((product_rev == PRODUCT_REV_2_0) || (product_rev == PRODUCT_REV_2_1))) ? 3 : 2)
...
#define CANFD_TRANSMIT_RECEIVE_FIFO_BUFFER          (IS_RCAR_GEN4(product_id)? 3 : 1)
...
#define pclk                                        (133.33f)               /* RCarGen3/RCarV3U: Clock S3D2
                                                                               RCarGen4: Peripheral clock frequency */
...
#define RCAR_CANFD_RSCFD0CFDGRMCFG              (IS_RCAR_GEN4(product_id)? (0x1314) : (0x04FC))
#define RCMC(val)                               (IS_RCAR_GEN4(product_id)?((0x01) << (30 - ((val)*2))) : (((val) & 0x01) << 0))
/* End of Global Interface Mode Select Register */

/* Channel n CAN-FD Configuration Register */
#define RCAR_CANFD_CFDCnFDCFG(n)                (0x1404 + (0x20 * (n)))
#define RCAR_CFDCnFDCFG_CFDTE                   (1 << 31)
#define RCAR_CFDCnFDCFG_CLOE                    (1 << 30)
#define RCAR_CFDCnFDCFG_FDOE                    (1 << 28)
/* Channel n CAN-FD Control Register */
#define RCAR_CFDCnFDCTR(n)                      (0x1408 + (0x20 * (n)))
/* Channel n CAN-FD Status Register */
#define CFDCnFDSTS(n)                           (0x140C + (0x20 * (n)))
/* Channel n CAN-FD CRC Register */
#define CFDCnFDCRC(n)                           (0x1410 + (0x20 * (n)))
/* Channel n Bus load Control Register */
#define CFDCnBLCT(n)                            (0x1418 + (0x20 * (n)))
/* Channel n Bus load Status Register */
#define CFDCnBLSTS(n)                           (0x141C + (0x20 * (n)))
/* Channel Normal Bit Rate Configuration Register */
#define RCAR_CANFD_RSCFD0CmNCFG(m)              (0x0000 + (0x10 * (m)))
#define NTSEG2(val)                             (IS_RCAR_GEN4(product_id)?(((val) & 0x7F) << 25) : (((val) & 0x1F) << 24))
#define NTSEG1(val)                             (IS_RCAR_GEN4(product_id)?(((val) & 0xFF) << 17) : (((val) & 0x7F) << 16))
#define NSJW(val)                               (IS_RCAR_GEN4(product_id)?(((val) & 0x7F) << 10) : (((val) & 0x1F) << 11))
...
#define COMSTS                                  (1 << 7)
#define ESIF                                    (1 << 8)
#define REC(val)                                (val & 0x00FF0000)
#define TEC(val)                                (val & 0xFF000000)
...
#define RCAR_CANFD_RSCFD0CFDCmDCFG(m)           (IS_RCAR_GEN4(product_id)?(0x1400 + (0x20 * (m))) : (0x0500 + (0x20 * (m))))
#define DSJW(val)                               (((val) & 0x07) << 24)
#define DTSEG2(val)                             (IS_RCAR_GEN4(product_id)?(((val) & 0xF) << 16) : (((val) & 0x07) << 20))
#define DTSEG1(val)                             (IS_RCAR_GEN4(product_id)?(((val) & 0x1F) << 8) : (((val) & 0x0F) << 16))
...
#define EEF6                                    (1 << 22)
#define EEF7                                    (1 << 23)
#define CANFD_GERFL_ERR_RCARGEN4(interface)            ((interface? CMPOF : 0) | \
                                                (EEF0 | EEF1 | EEF2 | EEF3 | EEF4| EEF5 | EEF6 | EEF7 | \
                                                 DEF | MES | THLES))                                         

#define CANFD_GERFL_ERR_V3H20(interface)          ((interface? CMPOF : 0) | \
                                                (EEF0 | EEF1 | EEF2 | \
                                                 DEF | MES | THLES))

#define CANFD_GERFL_ERR_OTHER(interface)          ((interface? CMPOF : 0) | \
                                                (EEF0 | EEF1 | \
                                                 DEF | MES | THLES))

#define CANFD_GERFL_ERR(interface)                (IS_RCAR_GEN4(product_id) ? CANFD_GERFL_ERR_RCARGEN4(interface) : ((product_id == PRODUCT_ID_RCAR_V3H) && ((product_rev == PRODUCT_REV_2_0) || (product_rev == PRODUCT_REV_2_1))) ? \
                                                CANFD_GERFL_ERR_V3H20(interface) : CANFD_GERFL_ERR_OTHER(interface))
...
#define AFLPN(val)                              (IS_RCAR_GEN4(product_id)?(((val) & 0x7F) << 0) : (((val) & 0x1F) << 0))
#define AFLDAE                                  (1 << 8)
/* End of Receive Rule Entry Control Register */

/* Receive Rule Configuration Register 0 */
#define RCAR_CANFD_RSCFD0CFDGAFLCFG0(ch)        (IS_RCAR_GEN4(product_id)?(0x009C + (0x04 * ((ch) / 2))) : (0x009C))
#define RNC(m,val)                              (IS_RCAR_GEN4(product_id)?(((val) & 0x1FF) << ((m)?(0) : (16))) : (((val) & 0xFF) << (24 - (m) * 8))) 
...
/******************    Receive Rule Related Registers   **************************/
 ...
 #define RCAR_CANFD_GAFL_OFFSET                  (IS_RCAR_GEN4(product_id)?(0x800) : (0x0))
...
#define RCAR_CANFD_RSCFD0CFDRFCC(x)              (IS_RCAR_GEN4(product_id)?(0x00C0 + (0x04 * (x))) : (0x00B8 + (0x04 * (x))))
...
#define RCAR_CANFD_RSCFD0CFDRFSTS(x)             (IS_RCAR_GEN4(product_id)?(0x00E0 + (0x04 * (x))) : (0x00D8 + (0x04 * (x))))
...
#define RCAR_CANFD_RSCFD0CFDRFPCTR(x)            (IS_RCAR_GEN4(product_id)?(0x0100 + (0x04 * (x))) : (0x00F8 + (0x04 * (x))))
#define RFPC(val)                                (((val) & 0xFF) << 0)
 
#define RCAR_CANFD_RSCFD0CFDRFID(x)              (IS_RCAR_GEN4(product_id)?(0x6000 + (0x80 * (x))) : (0x3000 + (0x80 * (x))))
#define RFID(val)                                (((val) & 0x1FFFFFFF) << 0)
#define RFRTR                                    (1 << 30)
#define RFIDE                                    (1 << 31)

/* Receive FIFO Buffer Access Pointer Register (x = 0 to 7) */
#define RCAR_CANFD_RSCFD0CFDRFPTR(x)             (IS_RCAR_GEN4(product_id)?(0x6000 + 0x04 + (0x80 * (x))) : (0x3004 + (0x80 * (x))))
#define RFTS                                     (0xFFFF << 0)
#define RFPTR                                    (0xFFF << 16)
#define RFDLC(val)                               (((val) >> 28) & 0x0F)
/* End of Receive FIFO Buffer Access Pointer Register */

#define RCAR_CANFD_RSCFD0CFDRFFDSTS(x)           (IS_RCAR_GEN4(product_id)?(0x6000 + 0x08 + (0x80 * (x))) : (0x3008 + (0x80 * (x))))

#define RCAR_CANFD_RSCFD0CFDRFDF(d, x)           (IS_RCAR_GEN4(product_id)?(0x6000 + 0x0C + (0x04 * (d)) + (0x80 * (x))) : (0x300C + (0x04 * (d)) + (0x80 * (x))))

/* Transmit/receive FIFO Buffer Configuration and Control Register */
#define RCAR_CANFD_RSCFD0CFDCFCC(k)              (IS_RCAR_GEN4(product_id)?(0x0120 + (0x04 * (k))) : (0x0118 + (0x04 * (k))))
#define CFDC(val)                                (IS_RCAR_GEN4(product_id)?(((val) & 0x7) << 21) : (((val) & 0x07) << 8))
#define CFTML(val)                               (IS_RCAR_GEN4(product_id)?(((val) & 0xF) << 16) : (((val) & 0x0F) << 20))
#define CFM(val)                                 (IS_RCAR_GEN4(product_id)?(((val) & 0x3) << 8) : (((val) & 0x03) << 16))
#define CFE                                      (1 << 0)
#define CFRXIE                                   (1 << 1)
#define CFTXIE                                   (1 << 2)
...
/* End of Transmit/receive FIFO Buffer Configuration and Control Register */

/* Transmit/receive FIFO Buffer Status Register*/
#define RCAR_CANFD_RSCFD0CFDCFSTS(k)             (IS_RCAR_GEN4(product_id)?(0x01E0 + (0x04 * (k))) : (0x0178 + (0x04 * (k))))
#define CFEMP                                    (1 << 0)
#define CFFLL                                    (1 << 1)
#define CFMLT                                    (1 << 2)
#define CFRXIF                                   (1 << 3)
...
#define RCAR_CANFD_RSCFD0CFDCFPCTR(k)            (IS_RCAR_GEN4(product_id)?(0x0240 + (0x04 * (k))) : (0x01D8 + (0x04 * (k))))
...
#define RCAR_CANFD_RSCFD0CFDCFID(k)              (IS_RCAR_GEN4(product_id)?(0x6400 + (0x80 * (k))) : (0x3400 + (0x80 * (k))))
...
#define RCAR_CANFD_RSCFD0CFDCFPTR(k)             (IS_RCAR_GEN4(product_id)?(0x6404 + (0x80 * (k))) : (0x3404 + (0x80 * (k))))
...
/* Transmit/receive FIFO CAN FD Configuration and Status Register */
#define RCAR_CANFD_RSCFD0CFDCFFDCSTS(k)          (IS_RCAR_GEN4(product_id)?(0x6408 + (0x80 * (k))) : (0x3408 + (0x80 * (k))))
#define CFESI                                    (1 << 0)
#define CFBRS                                    (1 << 1)
#define CFFDF                                    (1 << 2)
/* End of Transmit/receive FIFO CAN FD Configuration and Status Register */

/* Transmit/receive FIFO Buffer Access Data Field d Register */
#define RCAR_CANFD_RSCFD0CFDCFDF(d,k)            (IS_RCAR_GEN4(product_id)?(0x640C + (0x04 * (d)) + (0x80 * (k))) : (0x340C + (0x04 * (d)) + (0x80 * (k))))
...
#define CAN_TSEG1_MIN               (IS_RCAR_GEN4(product_id)? 2 : 4)
#define CAN_TSEG1_MAX               (IS_RCAR_GEN4(product_id)? 256 : 16)
#define CAN_TSEG2_MIN               (2)
#define CAN_TSEG2_MAX               (IS_RCAR_GEN4(product_id)? 128 : 8)

#define CANFD_NTSEG1_MIN            (IS_RCAR_GEN4(product_id)? 2 : 4)
#define CANFD_NTSEG1_MAX            (IS_RCAR_GEN4(product_id)? 256 : 128)
#define CANFD_NTSEG2_MIN            (2)
#define CANFD_NTSEG2_MAX            (IS_RCAR_GEN4(product_id)? 128 : 32)

#define CANFD_DTSEG1_MIN            (2)
#define CANFD_DTSEG1_MAX            (IS_RCAR_GEN4(product_id)? 32 : 16)
#define CANFD_DTSEG2_MIN            (2)
#define CANFD_DTSEG2_MAX            (IS_RCAR_GEN4(product_id)? 16 : 8)

#define CANFD_NORMAL_BITRATE_MAX    (1000000)
#define CANFD_DATA_BITRATE_MAX      (8000000)
...
typedef struct
{
    ...
    uint8_t                 interface;          /* Classic CAN or CANFD or FD only mode */
    ...
    uint8_t                 classic_tol;        /* Classical CAN only mode, FD tolerant mode */
} candev_init_t;
...
typedef struct
{
    ...
    struct can_devctl_stats devctl_stat[CANFD_TOTAL_CHANNELS_MAX];
    volatile uint32_t       canfd_stat[CANFD_TOTAL_CHANNELS_MAX];
} candev_info_t;
