...
//#define PTP_INCR            0x780000 /* Gen3 : 1/(3.2 GHz/24)*1E9*2^20 */
#define PTP_INCR            0xA00000  /* V4H: gPTP clock S0D8 =100Mhz; PTP_INCR = round((2^20GHz/S0D8) * (1 + d)); d = 1/10000000 */
...
{
    out32(ravb->base + GIC, 0);
    out32(ravb->base + GIS, 0);
}

static int ravb_ptp_request(ravb_dev_t *ravb, uint32_t request)
{
    int error;
...
void ravb_ptp_add_rx_timestamp(ravb_dev_t *ravb, ptpv2hdr_t *ph, RAVB_RX_DESC *bd)
{
    ...
    if(ravb->options.cfg.verbose & VERBOSE_PTP) {
        slogf(_SLOGC_NETWORK, _SLOG_INFO, "%s msgid=0x%x sequence_id=%u ts=%u.%u", __FUNCTION__,
            rx_ts[rx_ts_cnt].msg_type, rx_ts[rx_ts_cnt].sequence_id,
            rx_ts[rx_ts_cnt].ts.sec, rx_ts[rx_ts_cnt].ts.nsec);
    }
...
void ravb_ptp_add_tx_timestamp(ravb_dev_t *ravb)
{
    ...
    while((count = ((in32(ravb->base + TSR) & TSR_TFFL) >> 8)) > 0) {
        ...
        if((ph = &(tx_ph[queue][idx])) && ((ph->version & 0x0f) == 0x2)) {
            ...
            if(ravb->options.cfg.verbose & VERBOSE_PTP) {
                slogf(_SLOGC_NETWORK, _SLOG_INFO, "%s queue=%d idx=%d msgid=0x%x sequence_id=%u ts=%u.%u",
                    __FUNCTION__, queue, idx, tx_ts[tx_ts_cnt].msg_type, tx_ts[tx_ts_cnt].sequence_id,
                    tx_ts[tx_ts_cnt].ts.sec, tx_ts[tx_ts_cnt].ts.nsec);
            }
            /* Advance the counter including wrapping */
...
static int ravb_ptp_get_rx_timestamp (ravb_dev_t *ravb, ptp_extts_t *ts)
{
    int i;
    ...
    for (i = 0; i < RAVB_RX_TIMESTAMP_BUF_SZ; i++) {
        if ((ts->msg_type == rx_ts[i].msg_type) &&
            ...
            if(ravb->options.cfg.verbose & VERBOSE_PTP) {
                slogf(_SLOGC_NETWORK, _SLOG_INFO, "%s msgid=0x%x sequence_id=%u ts=%u.%u",
                    __FUNCTION__, ts->msg_type, ts->sequence_id, ts->ts.sec, ts->ts.nsec);
            }
            return 1;
        }
    }
...
static int ravb_ptp_get_tx_timestamp (ravb_dev_t *ravb, ptp_extts_t *ts)
{
    int i;
...
static void ravb_ptp_get_cnt(ravb_dev_t *ravb, ptp_time_t *cnt)
{
    int error;
    ...
    if(cnt != NULL) {
        ...
        if(ravb->options.cfg.verbose & VERBOSE_PTP) {
            slogf(_SLOGC_NETWORK, _SLOG_INFO, "%s: ts=%u.%u",
                __FUNCTION__, cnt->sec, cnt->nsec);
        }
    }
}

static void ravb_ptp_set_cnt(ravb_dev_t *ravb, ptp_time_t *cnt)
{
    int error;
    uint32_t gccr;
...
static void ravb_ptp_set_compensation(ravb_dev_t *ravb, ptp_comp_t ptc)
{
    ...
    if(ravb->options.cfg.verbose & VERBOSE_PTP) {
        slogf(_SLOGC_NETWORK, _SLOG_INFO, "%s ppb=%u dir=%u deviation=%u current_gti=%u",
            __FUNCTION__, ptc.comp, ptc.positive, deviation, comp);
    }
}
