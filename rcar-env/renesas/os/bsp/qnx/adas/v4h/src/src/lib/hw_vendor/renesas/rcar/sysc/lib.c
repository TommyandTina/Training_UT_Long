...
static void sysc_debug(const int level, const char* const fmt, ...)
{
    va_list arg;
...
static int rcar_sysc_init(void)
{
    ...
    if (sysc_fd == -1) {
        sysc_debug(_SLOG_ERROR, "%s: Failed to open (%s)\n", __func__, SYSC_DEV_NAME);
...
static int rcar_sysc_domain_control(const int pdr_number, const int cmd, int* const is_off)
{
    ...
    if (sysc_fd == -1) {
        sysc_debug(_SLOG_ERROR, "%s: Invalid sys fd (-1)\n", __func__);
        return SYSC_RET_ERROR;
    }
    ...
    if (devctl(sysc_fd, cmd, &sysc, sizeof(sysc), &status)) {
        sysc_debug(_SLOG_ERROR, "%s: Failed devctl status (%d)\n", __func__, status);
        return SYSC_RET_ERROR;
    }
...
static int rcar_sysc_domain_power_is_off(const int pdr_number)
{
    ...
    if (ret == SYSC_RET_ERROR) {
        sysc_debug(_SLOG_ERROR, "%s: Failed to power off domain pdr (%d)\n", __func__, pdr_number);
        return -1;
    }
