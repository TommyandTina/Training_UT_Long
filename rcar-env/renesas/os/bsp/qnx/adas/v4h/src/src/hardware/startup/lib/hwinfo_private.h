...
#define HWIATTR_COMMON_INITIALIZER \
		{ \
			...
			.num_irq = 0, .num_errata = 0, .num_dma = 0, .dll = NULL, .optstr = NULL \
		}
...
#define HWIATTR_SET_NUM_DMA(__hwitag_p__, __num__) \
		do { \
			(__hwitag_p__)->num_dma = (__num__); \
		} while(0)
...
#define HWIATTR_UART_SET_NUM_DMA(__hwitag_p__, ...)		HWIATTR_SET_NUM_DMA(&(__hwitag_p__)->common, __VA_ARGS__)
...
* hwibus_add_sdio
* 
* Adds an 'hwinfo' entry for SDIO devices.
* Added attribute tags are as follows...
* 
* 	- common attributes
* 	- input clock (1 per possible source)
...
* 			on error
*/
#define HWIATTR_SDIO_T_INITIALIZER \
		{ \
			.common = HWIATTR_COMMON_INITIALIZER, \
			.num_clks = 0, .num_busattr = 0, \
		}

#define HWIATTR_SDIO_SET_NUM_CLK(__hwitag_p__, __num__) \
	do { \
		(__hwitag_p__)->num_clks = (__num__); \
	} while(0)

#define HWIATTR_SDIO_SET_NUM_BUSATTR(__hwitag_p__, __num__) \
	do { \
		(__hwitag_p__)->num_busattr = (__num__); \
	} while(0)

#define HWIATTR_SDIO_SET_LOCATION(__hwitag_p__, ...)	HWIATTR_SET_LOCATION(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_SDIO_SET_NUM_DMA(__hwitag_p__, ...)		HWIATTR_SET_NUM_DMA(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_SDIO_SET_NUM_IRQ(__hwitag_p__, ...)		HWIATTR_SET_NUM_IRQ(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_SDIO_SET_NUM_ERRATA(__hwitag_p__, ...)	HWIATTR_SET_NUM_ERRATA(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_SDIO_SET_DLL(__hwitag_p__, ...)			HWIATTR_SET_DLL(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_SDIO_SET_OPTSTR(__hwitag_p__, ...)		HWIATTR_SET_OPTSTR(&(__hwitag_p__)->common, __VA_ARGS__)

extern unsigned hwibus_add_sdio(unsigned parent_hwi_off, hwiattr_sdio_t *attr);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/hwinfo_private.h $ $Rev: 837667 $")
#endif
