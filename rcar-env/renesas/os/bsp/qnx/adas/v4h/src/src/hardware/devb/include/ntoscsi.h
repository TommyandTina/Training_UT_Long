...
#define SC_RW_MAX_LBA32				0xffffffff
#define SC_RW_MAX_LBA16				0xffff
...
#define SC_SYNC16						0x91	// Synchronize Cache
...
#define SC_SEC_PROTO_OUT 0xb5	// Security Protocol Out
...
#define INQ_PC_ATA_INFORMATION						0x89
...
#define WS_OPT_LBDATA	0x02
	...
	struct {
		uchar_t		opcode;
#define WB_MODE_DNLD_MC		0x04	// Download microcode
#define WB_MODE_DNLD_MCS	0x05	// Download microcode and save
#define WB_MODE_DNLD_MC_O	0x06	// Download microcode with offsets
#define WB_MODE_DNLD_MC_OS	0x07	// Download microcode with offsets and save
#define WB_MODE_DNLD_MC_DA	0x0e	// Download microcode with offsets and defer activation
#define WB_MODE_ACT_MC		0x0f	// Active deferred microcode
		uchar_t		mode;
		uchar_t		buffer_id;
		uchar_t		buffer_offset[3];
		uchar_t		parm_list_len[3];
		uchar_t		control;
	} write_buffer;
...
#define SP_SCSA		0xeb
#define SP_UFS		0xec
#define SP_SDCARD	0xed
#define SP_ATA		0xef
		uchar_t		sp;								// security protocol
		uchar_t		sps[2];							// security protocol specific
		uchar_t		inc_512;
		uchar_t		rsvd3;
		uchar_t		length[4];						// allocation length / transfer length
		uchar_t		rsvd10;
		uchar_t		control;
	} security;
...
#define LD_OPT_IMMED	0x01
		uchar_t		lun_opt;
		uchar_t		rsvd2;
#define LD_CMD_PCM_MSK		0x0f
		uchar_t		pcm;	// power condition modifier
...
#define FM_FMT_FORMAT_PARTITION	0x02
	...
	struct {
		uchar_t		opcode;
		uchar_t		rsvd1;
#define RL_SEL_SAM	0x00
#define RL_SEL_WLUN	0x01
#define RL_SEL_ALL	0x02
		uchar_t		select_report;
		uchar_t		rsvd3[3];
		uchar_t		allocation_length[4];
		uchar_t		rsvd10;
		uchar_t		control;
...
#define SCS_INTER		0x10	// Intermediate/Good (Obsolete)
#define SCS_INTERMET	0x14	// Intermediate/Condition Met/Good (Obsolete)
#define SCS_CONFLICT	0x18	// Reservation Conflict
#define SCS_TERM		0x21	// Command Terminated
#define SCS_QUEUEFULL	0x28	// Queue Full, Task Set Full
#define SCS_ACA_ACTIVE	0x30	// ACA Active
#define SCS_ABORTED		0x40	// Task Aborted
...
#define SK_DATA_PROT	7		// Data Protect
	#define ASC_WRITE_PROTECTED			0x27
		#define ASCQ_VU_WRITE_PROTECTED	0xff		// QNX extention
...
typedef struct _block_limits {
	uchar_t		rsvd4;
	uchar_t		max_cmp_write_len;
	uchar_t		optimal_xfer_len_granularity[2];
	uchar_t		max_xfer_len[4];
	uchar_t		optimal_xfer_len[4];
	uchar_t		max_prefetch_len[4];
	uchar_t		max_unmap_lba_count[4];
	uchar_t		max_unmap_desc_count[4];
	uchar_t		optimal_unmap_granularity[4];
	uchar_t		unmap_granularity_alignment[4];
	uchar_t		max_write_same_len[8];
	uchar_t		max_atomic_xfer_len[4];
	uchar_t		atomic_alignment[4];
	uchar_t		atomic_xfer_len_granularity[4];
	uchar_t		max_atomic_xfer_len_bndry[4];
	uchar_t		max_atomci_bndry_size[4];
} BLOCK_LIMITS;

typedef struct _block_limits_ext {
	uchar_t		rsvd4_5[2];
	uchar_t		max_streams[2];
	uchar_t		optimal_stream_write_size[2];
	uchar_t		stream_granularity_size[4];
	uchar_t		rsvd14;
} BLOCK_LIMITS_EXT;

typedef struct _logical_block_provisioning {
	uchar_t		threshold_exp;
#define LBP_FLG_LBPU		0x80
#define LBP_FLG_LBPWS		0x40
#define LBP_FLG_LBPWS10		0x20
#define LBP_FLG_LBPRZ_MSK	0x0c
#define LBP_FLG_LBPRZ_PIP	0x08
#define LBP_FLG_LBPRZ_ZERO	0x04
#define LBP_FLG_LBPRZ_VS	0x00
#define LBP_FLG_ANC_SUP		0x02
#define LBP_FLG_DP			0x01
	uchar_t		flags;
#define LBP_PTYPE_MSK		0x07
#define LBP_PTYPE_THIN		0x02
#define LBP_PTYPE_RSRC		0x01
#define LBP_PTYPE_NA		0x00
	uchar_t		minp_ptype;			// minimum percentage, provision_type
	uchar_t		threshold_percentage;
} LOGICAL_BLOCK_PROVISIONING;

typedef struct _ata_device_signature {
#define ADS_TID_PATA	0x00
#define ADS_TID_SATA	0x34
	uchar_t		tp_id;				// transport identifier
	uchar_t		intr_pmp;
	uchar_t		status;
	uchar_t		error;
	uchar_t		lba0_23[3];
	...
	uchar_t		lba24_47[3];
	uchar_t		rsvd11;
	uchar_t		sector_count[2];
	uchar_t		rsvd14_19[6];
} ATA_DEVICE_SIGNATURE;

typedef struct _ata_information {
	uchar_t		rsvd4_7[4];
	uchar_t		sat_vid[8];			// vendor id
	uchar_t		sat_pid[16];		// product id
	uchar_t		sat_prl[4];			// product revision level
	uchar_t		dev_sig[20];		// see ATA_DEVICE_SIGNATURE
	uchar_t		cmd_code;
	uchar_t		rsvd57_59[3];
	uchar_t		id_data[512];		// identify data
} ATA_INFORMATION;
...
#define MP_INFO_EXCEPTIONS	0x1c	// Information Exceptions
...
#define MP_DS_DPOFUA		0x10	// Disable Page Out and Force Unit Access Support
...
#define LP_SUPPORTED			0x00	// Pages supported
#define LP_TEMPERATURE			0x0d
#define LP_SOLID_STATE_MEDIA	0x11
#define LP_INFO_EXCEPTIONS		0x2f
#define LP_TAPE_CAPACITY		0x31
typedef struct _lpage_supported {
	uchar_t		pc_page;
	uchar_t		rsvd;
	uchar_t		page_length[2];
} LPAGE_SUPPORTED;
...
typedef struct _lpage_solid_state_media {
	uchar_t		pc_page;
	uchar_t		rsvd;
	uchar_t		page_length[2];
	LOG_PARM_HEADER	parm1;
	uint8_t		percentage_used;
} LPAGE_SOLID_STATE_MEDIA;

typedef struct _lpage_info_exceptions {
	uchar_t		pc_page;
	uchar_t		rsvd;
	uchar_t		page_length[2];
	LOG_PARM_HEADER	parm1;
...
#define RD_CAP_MAX_LBA32		0xffffffff
	uint32_t	lba;
	uint32_t	blk_size;
} READ_CAPACITY;
...
typedef struct _report_luns_parm_header {
	uchar_t		lun_list_length[4];
	uchar_t		rsvd4_7[4];
} REPORT_LUNS_PARM_HEADER;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/devb/include/ntoscsi.h $ $Rev: 905621 $")
#endif
