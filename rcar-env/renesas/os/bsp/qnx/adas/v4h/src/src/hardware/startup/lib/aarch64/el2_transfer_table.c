...
el2_hvc_check(paddr_t dummy) {
	...
	trying_hvc = 0;
	// Ensure that updating trying_hvc is completed before next instruction is executed
	asm volatile("dsb sy");
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/aarch64/el2_transfer_table.c $ $Rev: 969760 $")
#endif
