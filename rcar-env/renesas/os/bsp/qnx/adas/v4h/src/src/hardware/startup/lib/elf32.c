...
static Elf32_Phdr *
elf32_phdr(Elf32_Ehdr *hdr, paddr32_t addr)
{
	Elf32_Ehdr	*p;
...
load_elf32(paddr32_t addr)
{
	...
	if((phdr_start = elf32_phdr(&hdr, addr)) == NULL) {
		return(~0L);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/elf32.c $ $Rev: 791709 $")
#endif
