...
const struct sigevent * spi_slave_intr(void * area, int id)
{
    ...
    intr = in32(dev->vbase + RCAR_MSIOF_STR);
    out32(dev->vbase + RCAR_MSIOF_STR, intr);

    /* Receive data transfer request interrupt */
    if (SISTR_RDREQ & intr) {
        idx = dev->rx_buff.head % dev->blksize;
        ...
        if (SPI_BIT_LEN_8 == dev->cfg.bitlen) {
            buf_8[idx] = in32(dev->vbase + RCAR_MSIOF_RFDR) >> fs;
        } else if (SPI_BIT_LEN_16 == dev->cfg.bitlen) {
            buf_16[idx] = in32(dev->vbase + RCAR_MSIOF_RFDR) >> fs;
        } else {
            buf_32[idx] = in32(dev->vbase + RCAR_MSIOF_RFDR) >> fs;
        }
        dev->rx_buff.head++;
    }
    ...
    if (SISTR_TDREQ & intr) {
        ...
        out32(dev->vbase + RCAR_MSIOF_TFDR, 0x00 << fs);
    }
    /* Receive FIFO full */
    ...
    if (SISTR_RFUDF & intr) {
        dev->status |= SPI_STATUS_RECV_FIFO_UDF;
    }
    if (SISTR_RFOVF & intr) {
        dev->status |= SPI_STATUS_RECV_FIFO_OVF;
    }
    /* Receive frame sync error */
    if (SISTR_RFSERR & intr) {
        /* Clear status bit */
        out32(dev->vbase + RCAR_MSIOF_STR, SIIER_RFSERRE);
        dev->status |= SPI_STATUS_SYNC_ERR;
    }
    /* End of frame interrupt */
    if (SISTR_REOF & intr) {
        /* The flag SISTR_REOF is set but if the dev->rx_buff.head < dev->cfg.wordlen then
           we need to read all data from FIFO before reset FIFO */
        if (dev->rx_buff.head < dev->cfg.wordlen) {
            while (dev->rx_buff.head < dev->cfg.wordlen) {
                idx = dev->rx_buff.head % dev->blksize;
                if (SPI_BIT_LEN_8 == dev->cfg.bitlen) {
                    buf_8[idx] = in32(dev->vbase + RCAR_MSIOF_RFDR) >> fs;
                } else if (SPI_BIT_LEN_16 == dev->cfg.bitlen) {
        ...
        /* Clear status */
        out32(dev->vbase + RCAR_MSIOF_STR,
               in32(dev->vbase + RCAR_MSIOF_STR));
