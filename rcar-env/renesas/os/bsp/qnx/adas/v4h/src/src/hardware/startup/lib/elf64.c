...
static Elf64_Phdr *
elf64_phdr(Elf64_Ehdr *hdr, paddr_t addr)
{
	Elf64_Ehdr	*p;
	...
	*hdr = *p;
	...
	return startup_memory_map(hdr->e_phnum * sizeof(Elf64_Phdr),
...
static void
load_elf64mmu(paddr_t addr, Elf64_Ehdr *hdr, Elf64_Phdr *phdr)
{
	int			i;
	...
	for(i = 0; i < hdr->e_phnum; ++i, ++phdr) {
		if (phdr->p_type == PT_LOAD) {
			...
			paddr_t paddr  = phdr->p_paddr & ~PAGEBITS;
			paddr_t daddr;
...
load_elf64(paddr_t addr)
{
	...
	if((phdr_start = elf64_phdr(&hdr, addr)) == NULL) {
		return ~0L;
	}
	...
	if (!(shdr->flags1 & STARTUP_HDR_FLAGS1_VIRTUAL)) {
		...
		for(i = 0; i < hdr.e_phnum; ++i, ++phdr) {
			switch(phdr->p_type) {
			case PT_LOAD:
				if(MAKE_1TO1_PTR(phdr->p_paddr) != (void *)(uintptr_t)phdr->p_vaddr) {
					alloc_ram(phdr->p_vaddr - shdr->paddr_bias, phdr->p_memsz, 1);
					memmove((void *)(uintptr_t)phdr->p_vaddr,
						MAKE_1TO1_PTR(addr + phdr->p_offset),
						phdr->p_filesz);
		...
		load_elf64mmu(addr, &hdr, phdr_start);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/elf64.c $ $Rev: 791709 $")
#endif
