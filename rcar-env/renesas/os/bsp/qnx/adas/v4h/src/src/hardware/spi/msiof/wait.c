...
int msiof_wait(msiof_spi_t *dev, int len)
{
    ...
    while (1) {
        ...
        if (MsgReceivePulse(dev->chid, &pulse, sizeof(pulse), NULL) == -1) {
            fprintf(stderr, "MSIOF : msiof_wait failed %s(%d)\n", strerror(errno), errno);
            spi_dump(dev);
            return (errno);
        }
