...
static sdmmc_hc_t rcar_hc;
static rcar_sdmmc_bs_t *rcar_bs;
...
static void rcar_set_frq(sdmmc_t *sdmmc, unsigned frq) {
...
static void rcar_set_bus_mode(sdmmc_t *sdmmc, int mode) {
    return;
}

/* sets the data bus width */
static void rcar_set_bus_width(sdmmc_t *sdmmc, int width) {
...
static void rcar_set_timing(sdmmc_t *sdmmc, int timing) {
    unsigned base = sdmmc->hc->sdmmc_pbase;
...
static int rcar_init_ctrl(sdmmc_t *sdmmc) {
    unsigned base = sdmmc->hc->sdmmc_pbase;
    int i, status;
    ...
    if (rcar_bs && rcar_bs->init) {
        rcar_bs->init(sdmmc);
    } else {
        rcar_usec_delay(10000);
    }

    sdhi_write(base, MMC_SD_INFO1_MASK, SDH_INFO1_MASK_ALL);
    sdhi_write(base, MMC_SD_INFO2_MASK, SDH_INFO2_MASK_ALL);
    ...
    sdhi_write(base, MMC_SD_CLK_CTRL, 0x00000080);    // Automatic Control=Disable, Clock Output=Disable
    
    /* Mask all DMA interrupts */
    sdhi_write(base, MMC_DM_CM_INFO1_MASK, 0xFFFFFFFF);
    sdhi_write(base, MMC_DM_CM_INFO2_MASK, 0xFFFFFFFF);
    ...
    rcar_set_frq(sdmmc, 400);

    return SDMMC_OK;
}

/* clean up the SDMMC controller */
int rcar_sdmmc_fini(sdmmc_t *sdmmc) {
    return 0;
}
...
static int rcar_send_cmd(sdmmc_t *sdmmc) {
    ...
    if (cmd->rsp != 0) {
        if ((command->sdmmc_cmd & SDH_CMD_RSPR2) == SDH_CMD_RSPR2) {
            uint32_t    *resp = &cmd->rsp[0];
            uint32_t    rsp_127_104 = sdhi_read(base, MMC_SD_RSP76);
            uint32_t    rsp_103_72 = sdhi_read(base, MMC_SD_RSP54);
            uint32_t    rsp_71_40 = sdhi_read(base, MMC_SD_RSP32);
            uint32_t    rsp_39_8 = sdhi_read(base, MMC_SD_RSP10);

           /*
            * CRC is not included in the response register,
            * we have to left shift 8 bit to match the 128 CID/CSD structure
            */
            resp[3] = ((rsp_127_104 & 0x00ffffff) << 8) | ((rsp_103_72  & 0xff000000) >> 24);
            resp[2] = ((rsp_103_72  & 0x00ffffff) << 8) | ((rsp_71_40   & 0xff000000) >> 24);
            resp[1] = ((rsp_71_40   & 0x00ffffff) << 8) | ((rsp_39_8    & 0xff000000) >> 24);
            resp[0] = (rsp_39_8     & 0xffffff)   << 8;
        } else {
...
static int rcar_pio_read(sdmmc_t *sdmmc, void *buf, unsigned len) {
    unsigned        base = sdmmc->hc->sdmmc_pbase;
    sdmmc_cmd_t     *cmd  = &sdmmc->cmd;
    int             status;
    int             i;
    ...
    if (sdhi_read(base, MMC_SD_INFO2) & SDH_INFO2_ALL_ERR) {
        ...
        if (sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
            ser_putstr("rcar_pio_read() failed. MMC_SD_INFO2: 0x");
            ser_puthex(cmd->erintsts);
            ser_putstr("\n");
        }

        sdhi_write(base, MMC_SD_INFO1, sdhi_read(base, MMC_SD_INFO1) & ~(SDH_INFO2_ALL_ERR));
    ...
    /* Now read from FIFO */
    while (len > 0) {
        if (sdhi_read(base, MMC_SD_INFO2) & SDH_INFO2_BRE) {
            int blsz = sdhi_read(base, MMC_SD_SIZE);
            sdhi_write(base, MMC_SD_INFO2, ~SDH_INFO2_BRE);
            for (; blsz > 0; blsz -= 4) {
                *pbuf++ = sdhi_read(base, MMC_SD_BUF0);
            }
            len -= sdhi_read(base, MMC_SD_SIZE);
...
static int rcar_dma_mxblks(sdmmc_t *sdmmc) {
    /* No limit */
    return ~0;
}


static int rcar_signal_voltage( sdmmc_t *sdmmc, int signal_voltage )
{
    unsigned        base = sdmmc->hc->sdmmc_pbase;
    uint16_t        clkctl;
    uint16_t        info2;
    ...
    if (!rcar_bs || !rcar_bs->signal_voltage) {
        //ser_putstr("rcar_signal_voltage() no board-specifc function to change signal voltage\n");
        return SDMMC_OK;
    }
    
    if (signal_voltage == SIGNAL_VOLTAGE_3_3) {
        return rcar_bs->signal_voltage(sdmmc, SIGNAL_VOLTAGE_3_3);
    }
    ...
    if (cnt <= 0) {
        if (sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
            ser_putstr("rcar_signal_voltage() failed. Busy state, can't change the voltage!\n");
        }
        return (EAGAIN);
    }

    clkctl = SDH_CLKCTRL_SCLKEN;
    if (sdhi_read(base, MMC_SD_CLK_CTRL) & SDH_CLKCTRL_SDCLKOFFEN) {
        clkctl |= SDH_CLKCTRL_SDCLKOFFEN;
    }

    sdhi_write(base, MMC_SD_CLK_CTRL, ~(clkctl) & sdhi_read(base, MMC_SD_CLK_CTRL));

    /* Check to see if DAT0 bit is 0  */
    if (sdhi_read(base, MMC_SD_INFO2) & 0x0080) {
        if (sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
            ser_putstr("rcar_signal_voltage() failed. DAT0 is not low\n");
        }
        sdhi_write(base, MMC_SD_CLK_CTRL, sdhi_read(base, MMC_SD_CLK_CTRL) | clkctl);
        return (EIO);
    }

    if (rcar_bs->signal_voltage(sdmmc, SIGNAL_VOLTAGE_1_8)) {
        if (sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
            ser_putstr("rcar_signal_voltage() failed to set voltage to 1.8v\n");
        }
        sdhi_write(base, MMC_SD_CLK_CTRL, sdhi_read(base, MMC_SD_CLK_CTRL) | clkctl);
        return (EIO);
    }
    ...
    do {
        /* Wait for 1ms */
        rcar_usec_delay(1000);
    } while (!(sdhi_read(base, MMC_SD_INFO2) & 0x0080) && (--cnt != 0));

    if (cnt == 0) {
        if (sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
            ser_putstr("rcar_signal_voltage() failed. DAT0 is not high\n");
        }
        return (EIO);
    }

    if (clkctl & SDH_CLKCTRL_SDCLKOFFEN) {
        sdhi_write(base, MMC_SD_CLK_CTRL, sdhi_read(base, MMC_SD_CLK_CTRL) | SDH_CLKCTRL_SDCLKOFFEN);
    }
...
static int rcar_tune(sdmmc_t *sdmmc, int op)
{
    unsigned        rsp[4];
    uint8_t         td[TUNING_8BIT_BLK_SIZE];
    const uint8_t   *tuning_block_pattern;
    int             tap_num;
    int             taps[RCAR_SDHI_TUNING_RETRIES] = { 0 };
    int             smpcmp[RCAR_SDHI_TUNING_RETRIES] = { 0 };
    int             tlen;
    int             status;
    int             i;
    ...
    memset(taps, 0, sizeof(int) * RCAR_SDHI_TUNING_RETRIES);
    memset(smpcmp, 0, sizeof(int) * RCAR_SDHI_TUNING_RETRIES);
    ...
    for (i = 0; i < 2 * tap_num; i++) {
        ...
        if (0 != (status = rcar_pio_read(sdmmc, td, TUNING_8BIT_BLK_SIZE))) {
            if (sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
                ser_putstr("tuning rcar_pio_read error at tap 0x");
				ser_puthex(i);
                ser_putstr("\n");
            }
        } else {
...
/*--------------------------------------*/
void rcar_sdmmc_init_hc(sdmmc_t *sdmmc, unsigned base, unsigned clock, int verbose) {
    rcar_hc.set_clk        = rcar_set_frq;
    rcar_hc.set_bus_width  = rcar_set_bus_width;
    rcar_hc.set_bus_mode   = rcar_set_bus_mode;
    rcar_hc.set_timing     = rcar_set_timing;
    rcar_hc.send_cmd       = rcar_send_cmd;
    rcar_hc.pio_read       = rcar_pio_read;
    rcar_hc.init_hc        = rcar_init_ctrl;
    rcar_hc.dinit_hc       = rcar_sdmmc_fini;
    rcar_hc.signal_voltage = rcar_signal_voltage;
    rcar_hc.tuning         = rcar_tune;
    rcar_hc.sdmmc_pbase    = base;
    rcar_hc.clock          = clock;

    sdmmc->hc            = &rcar_hc;
    sdmmc->verbose       = verbose;
    sdmmc->hc->max_blks  = rcar_dma_mxblks;
    sdmmc->caps          = 0;
}

void rcar_sdmmc_init_bs(rcar_sdmmc_bs_t *bs) {
    rcar_bs                = bs;
}
