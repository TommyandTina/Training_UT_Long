/*
 * $QNXLicenseC:
 * Copyright 2013-2019, QNX Software Systems.
 *
...
/* Gets the card state */
static int sdmmc_card_ready_for_data(sdmmc_t *sdmmc)
{
	...
	do {
		...
		/* Bits 8 define the card is ready for data */
		sdmmc->card.state = rsp & MMC_READY_FOR_DATA;
		retries --;
	} while ((MMC_READY_FOR_DATA != sdmmc->card.state) && (retries > 0));
...
static void mmc_parse_ext_csd(sdmmc_t *sdmmc, unsigned char *raw_ecsd)
{
	...
	if ((sdmmc->caps & SDMMC_CAP_HS400) && (ecsd->card_type & ECSD_CARD_TYPE_HS400)) {
		if ((sdmmc->caps & SDMMC_CAP_SV_1_8V) && (ecsd->card_type & ECSD_CARD_TYPE_HS400_1_8V)) {
			caps |= SDMMC_CAP_HS400 | SDMMC_CAP_HS;
			card->speed = DTR_MAX_HS400;
		}
	}

	if ((sdmmc->caps & SDMMC_CAP_HS200) && (ecsd->card_type & ECSD_CARD_TYPE_200MHZ)) {
		if ((sdmmc->caps & SDMMC_CAP_SV_1_8V) && (ecsd->card_type & ECSD_CARD_TYPE_HS200_1_8V)) {
			caps |= SDMMC_CAP_HS200 | SDMMC_CAP_HS;
			card->speed = DTR_MAX_HS200;
		}
	}

	if (!(caps & SDMMC_CAP_HS) && (sdmmc->caps & SDMMC_CAP_HS) && (ecsd->card_type & ECSD_CARD_TYPE_52MHZ)) {
		caps |= SDMMC_CAP_HS;
		card->speed = DTR_MAX_HS52;
	}

	if (!(caps & SDMMC_CAP_HS)) {
		card->speed = DTR_MAX_HS26;
	}

	if ((sdmmc->caps & SDMMC_CAP_DDR ) && ( ecsd->card_type & ECSD_CARD_TYPE_DDR)) {
		if ((sdmmc->caps & SDMMC_CAP_SV_1_8V) && (ecsd->card_type & ECSD_CARD_TYPE_DDR_1_8V)) {
			caps |= SDMMC_CAP_DDR;
		}
	}

	sdmmc->caps = (sdmmc->caps & ~SDMMC_CAP_TIMING_MASK) | caps;
	...
	if (sdmmc->verbose) {
		...
		ser_putstr("	PARTITION_CONFIG 0x"); ser_puthex(ecsd->part_cfg); ser_putstr("\n");
	}

	if (ecsd->ext_csd_rev >= ECSD_REV_V4_2) {
		card->blk_num = raw_ecsd[ECSD_SEC_CNT + 0] << 0 |
...
static int _mmc_init_hs200(sdmmc_t *sdmmc)
{
	sdmmc_hc_t *hc = sdmmc->hc;
	...
	if (mmc_switch(sdmmc, MMC_SWITCH_MODE_WRITE, ECSD_HS_TIMING, ECSD_HS_TIMING_HS200, MMC_SWITCH_CMDSET_DFLT)) {
		ser_putstr("Can't switch to High Speed mode\n");
		return SDMMC_ERROR;
	}

	hc->set_timing(sdmmc, TIMING_HS200);
	hc->set_clk(sdmmc, DTR_MAX_HS200);
	if (hc->tuning) {
		hc->tuning(sdmmc, MMC_SEND_TUNING_BLOCK);
	}
	return SDMMC_OK;
}

static int mmc_init_hs200(sdmmc_t *sdmmc)
{
	...
	if( ( sdmmc->mmc_ecsd.card_type & ECSD_CARD_TYPE_HS200_1_2V ) && ( sdmmc->caps & SDMMC_CAP_HS200 ) ) {
		if( ( status = hc->signal_voltage( sdmmc, SIGNAL_VOLTAGE_1_2 ) ) != SDMMC_OK) {
			ser_putstr("_signal_voltage 1.2V failure\n");
		}
	}

	if( status && ( sdmmc->mmc_ecsd.card_type & ECSD_CARD_TYPE_HS200_1_8V ) && ( sdmmc->caps & SDMMC_CAP_HS200 ) ) {
		 if( ( status = hc->signal_voltage( sdmmc, SIGNAL_VOLTAGE_1_8 ) ) != SDMMC_OK ) {
			ser_putstr("_signal_voltage 1.8V failure\n");
		}
	}
	...
	return _mmc_init_hs200(sdmmc);
}

static int mmc_init_hs400(sdmmc_t *sdmmc)
{
	sdmmc_hc_t	*hc = sdmmc->hc;
	int			status;
	...
	if( ( sdmmc->mmc_ecsd.card_type & ECSD_CARD_TYPE_HS400_1_2V ) && ( sdmmc->caps & SDMMC_CAP_SV_1_2V ) ) {
		if( ( status = hc->signal_voltage( sdmmc, SIGNAL_VOLTAGE_1_2 ) ) != SDMMC_OK) {
			ser_putstr("_signal_voltage 1.2V failure\n");
		}
	}

	if( status && ( sdmmc->mmc_ecsd.card_type & ECSD_CARD_TYPE_HS400_1_8V ) && ( sdmmc->caps & SDMMC_CAP_SV_1_8V ) ) {
		if( ( status = hc->signal_voltage( sdmmc, SIGNAL_VOLTAGE_1_8 ) ) != SDMMC_OK ) {
			ser_putstr("_signal_voltage 1.8V failure\n");
		}
	}
	...
	if( ( status = _mmc_init_hs200(sdmmc) ) ) {
		return( status );
	}

	// set HS
	mmc_switch(sdmmc, MMC_SWITCH_MODE_WRITE, ECSD_HS_TIMING, ECSD_HS_TIMING_HS, MMC_SWITCH_CMDSET_DFLT);

	hc->set_timing(sdmmc, TIMING_HS);
	hc->set_clk(sdmmc, DTR_MAX_HS52);

	// Check if card is ready for data
	if ((status = sdmmc_card_ready_for_data(sdmmc)) == SDMMC_OK) {
		if (sdmmc->card.state != MMC_READY_FOR_DATA) {
			ser_putstr("Card is not ready(HS)\n");
		}
	}

	// set bus width to 8bit DDR
	if (mmc_switch(sdmmc, MMC_SWITCH_MODE_WRITE, ECSD_BUS_WIDTH, ECSD_BUS_WIDTH_8_BIT | 4, MMC_SWITCH_CMDSET_DFLT)) {
		ser_putstr("Can't switch to ECDD_BUS_WIDTH (8bit ddr)\n");
		return SDMMC_ERROR;
	}

	// set HS400
	if (mmc_switch(sdmmc, MMC_SWITCH_MODE_WRITE, ECSD_HS_TIMING, ECSD_HS_TIMING_HS400, MMC_SWITCH_CMDSET_DFLT)) {
		ser_putstr("Can't switch to ECSD_HS_TIMING (HS400)\n");
		return SDMMC_ERROR;
	}

	hc->set_timing(sdmmc, TIMING_HS400);
	hc->set_clk(sdmmc, DTR_MAX_HS400);

	// Check if card is ready for data
	if ((status = sdmmc_card_ready_for_data(sdmmc)) == SDMMC_OK) {
		if (sdmmc->card.state != MMC_READY_FOR_DATA) {
			ser_putstr("Card is not ready(HS400)\n");
		}
	}
...
static int mmc_send_op_cond(sdmmc_t *sdmmc, unsigned ocr )
{
	sdmmc_hc_t *hc = sdmmc->hc;
	card_t	*card = &sdmmc->card;
	unsigned rsp[4];
	...
	/* Probe for SDC card */
	CMD_CREATE (sdmmc->cmd, MMC_SEND_OP_COND, ocr, rsp, 0, 0, 0);
	for (i = 0; i < POWER_UP_WAIT; i++) {
		...
		if ((i == 0) && (SDMMC_OK != sdmmc_check_ocr(sdmmc, rsp[0]))) {
			ser_putstr("Failed to power up MMC card rsp[0] ");
			ser_puthex(rsp[0]);
			ser_putstr("\n");
			return SDMMC_ERROR;
		}
...
static int mmc_init_bus(sdmmc_t *sdmmc)
{
	...
	if (sdmmc->caps & SDMMC_CAP_HS400) {
		 if(mmc_init_hs400(sdmmc) == SDMMC_OK ) {
			return SDMMC_OK;
		}
	}

	if (sdmmc->caps & SDMMC_CAP_HS200) {
		 if(mmc_init_hs200(sdmmc) == SDMMC_OK ) {
			return SDMMC_OK;
		}
	}

	if ( sdmmc->caps & SDMMC_CAP_HS ) {
		if( mmc_init_hs(sdmmc) == SDMMC_OK ){
			return SDMMC_OK;
		}
	}
...
static int sdmmc_init_card(sdmmc_t *sdmmc)
{
	...
	if (IS_EMMC_CARD(card)) {
		if (mmc_send_op_cond(sdmmc, sdmmc->ocr | OCR_HCS )) {
			return SDMMC_ERROR;
		}
	} else {
...
int sdmmc_set_partition(sdmmc_t *sdmmc, unsigned partition)
{
	if (mmc_switch(sdmmc, MMC_SWITCH_MODE_WRITE, ECSD_PART_CONFIG, sdmmc->mmc_ecsd.part_cfg | partition, MMC_SWITCH_CMDSET_DFLT)) {
		ser_putstr("Failed to switch partition\n");
		return SDMMC_ERROR;
	}
