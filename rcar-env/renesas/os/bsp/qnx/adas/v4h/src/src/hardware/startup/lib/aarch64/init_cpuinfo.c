...
init_one_cpuinfo(unsigned cpunum)
{
	...
	// Detect if we have Virtualization Host Extensions
	if(aa64_sr_rd64(id_aa64mmfr1_el1) & 0x0f00) {
		cpu->flags |= AARCH64_CPU_FLAG_VHE;
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/aarch64/init_cpuinfo.c $ $Rev: 937218 $")
#endif
