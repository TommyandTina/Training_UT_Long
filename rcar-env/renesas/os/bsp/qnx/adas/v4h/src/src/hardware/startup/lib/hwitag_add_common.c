...
static void hwitag_add_dma(unsigned hwi_off, unsigned chnl);
...
#define ADD_DMA_TAGS(h, n) \
		do { \
			unsigned i = (n); \
			while (i--) hwitag_add_dma((h), 0); \
		} while(0)
...
void hwitag_add_common(unsigned hwi_off, void *_attr)
{
	...
	ADD_DMA_TAGS(hwi_off, attr->num_dma);
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/hwitag_add_common.c $ $Rev: 680332 $")
#endif
