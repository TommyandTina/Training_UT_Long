...
/*
 * For use with shm_ctl_special() to indicate that the provided value uses
 * aarch64 page table attributes and not the legacy ARMv7 encoding.
 * This flag can be combined with any of the AARCH64_PTE_ATTR_BITS and
 * AARCH64_PTE_SH_BITS values.
 */
#define AARCH64_SHMCTL          1
...
#define	AARCH64_USER_END		(1ULL << 39)  // fixed 512GB for both 4K and 64K pages.

/*
 * Virtual address space layout for 64K page size configuration.
 --------------------------------------------------------------------------
  47 <--- L1 ---> 42|41 <--- L2 ---> 29|28 <--- L3 ---> 16|15 <--- POFF --->
 --------------------------------------------------------------------------
 * 1 L3 Table cover 8192 * 64K = 512MB
 * 1 L2 Table cover 8192 * 512 = 4TB (which is 1 L1 entry)
 *
 * 00000000.00000000 - 000001ff.ffffffff - 4TB (42 bits limit to 39 bits for now) user space
 * fffff800.00000000 - fffff9ff.ffffffff - 4TB (42 bits) msgxfer space
 * fffffc00.00000000 - ffffffff.ffffffff - 4TB ( limit to 39 bits for now) kernel space
 *
 * The bottom 4TB (user space) is mapped via TTBR0 using an L2 table.
 * TTBR0 specifies the ASID.
 *
 * The Kernel has 1 L1 entry, 1 L2 table for 4TB space.
 * Each processor has its own L1 table.
 *
 * L1[62] maps the XFER_BASE range used to map a user L2 table to access
 * another address space during message transfer operations.
 *
 * L1[63] maps the KERN_BASE range used for global system mappings.
 * The 4TB space is broken up as follows:
 *
 * fffffc00.20000000 - fffffc00.3fffffff - L3_BASE      (512MB)
 * fffffc00.00000000 - fffffc00.0000ffff - L2_BASE      (64KB)
 * fffffc00.40000000 - fffffc00.5fffffff - STARTUP_BASE (512MB)
 * fffffc00.60000000 - fffffc00.7fffffff - BOOTPGM_BASE (512MB)
 * fffffc00.80000000 - ffffffff.ffffffff - UNUSED_BASE	(4TB - 2GB)
 *
 *	L2_BASE is a 64KB region that maps the L2 table for the system mappings.
 *	This is mapped by pointing L2[0] back to itself, which causes L2 to act
 *	as L3 tables so that L3[0] point to L2.
 *
 *	L3_BASE is a 512MB region that maps the L3 tables for the system mappings.
 *	It is mapped via the recursive L2[0] entry, which causes all the L2 to
 *	act as an L2 table for this 512MB range so that its L2 entries act as L3
 *	tables.
 *
 * STARTUP_BASE is a 512MB region that contains startup created mappings.
 ...
 *
 * UNUSED_BASE is the start of unused virtual address space mapped by L1[63].
 */

#define	__L1_SHIFT_64K			42u
#define	__L2_SHIFT_64K			29u
#define	__L3_SHIFT_64K			16u
#define	__NPTE_PTBL_64K			8192u
#define	__L1_IDX_64K(v)			((unsigned)((v) >> __L1_SHIFT_64K) & (__NPTE_PTBL_64K-1))
#define	__L2_IDX_64K(v)			((unsigned)((v) >> __L2_SHIFT_64K) & (__NPTE_PTBL_64K-1))
#define	__L3_IDX_64K(v)			((unsigned)((v) >> __L3_SHIFT_64K) & (__NPTE_PTBL_64K-1))
#define	__L1_VOFF_64K(v)		((v) & ((1ULL << __L1_SHIFT_64K) - 1))
#define	__L2_VOFF_64K(v)		((v) & ((1ULL << __L2_SHIFT_64K) - 1))
#define	__L3_VOFF_64K(v)		((v) & ((1ULL << __L3_SHIFT_64K) - 1))

#define	__L1_BASE_IDX_64K		62ULL
#define	__L1_XFER_IDX_64K		62ULL
#define	__L1_KERN_IDX_64K		63ULL
#define	__XFER_BASE_64K			(AARCH64_SYS_BASE + (__L1_XFER_IDX_64K << __L1_SHIFT_64K))
#define	__XFER_SIZE_64K			((1ULL << __L1_SHIFT_64K) - 1)
#define	__KERN_BASE_64K			(AARCH64_SYS_BASE + (__L1_KERN_IDX_64K << __L1_SHIFT_64K))

#define	__L1_L1_IDX_64K			0ULL
#define	__L1_L2_IDX_64K			1ULL
#define	__L1_L3_IDX_64K			(__L1_L1_IDX_64K)
#define	__L1_OFF_64K			(__L1_L1_IDX_64K << __L1_SHIFT_64K)
#define	__L3_OFF_64K			(__L1_L3_IDX_64K << __L1_SHIFT_64K)

#define	__L2_L2_IDX_64K			0ULL
#define	__L2_STARTUP_IDX_64K		2ULL
#define	__L2_BOOTPGM_IDX_64K		3ULL

#define	__STARTUP_OFF_64K		(__L2_STARTUP_IDX_64K << __L2_SHIFT_64K)
#define	__BOOTPGM_OFF_64K		(__L2_BOOTPGM_IDX_64K << __L2_SHIFT_64K)
#define	__UNUSED_OFF_64K		(__BOOTPGM_OFF_64K + (1u << __L2_SHIFT_64K))

#define	AARCH64_STARTUP_BASE_64K	(__KERN_BASE_64K + __STARTUP_OFF_64K)
#define	AARCH64_BOOTPGM_BASE_64K	(__KERN_BASE_64K + __BOOTPGM_OFF_64K)
#define	AARCH64_UNUSED_BASE_64K		(__KERN_BASE_64K + __UNUSED_OFF_64K)

#define	__L1_BASE_64K			((ptp_t *)(__KERN_BASE_64K + __L1_OFF_64K))
#define	__L2_BASE_64K			((ptp_t *)(__KERN_BASE_64K + __L1_OFF_64K))
#define	__L3_BASE_64K			((ptp_t *)(__KERN_BASE_64K + __L1_OFF_64K))
#define	__KVA_64K(v)			((v) - __KERN_BASE_64K)
#define	__L1_MAP_64K(v)			(__KVA_64K(v) >> __L1_SHIFT_64K)
#define	__L2_MAP_64K(v)			(__KVA_64K(v) >> __L2_SHIFT_64K)
#define	__L3_MAP_64K(v)			(__KVA_64K(v) >> __L3_SHIFT_64K)

#define	__KTOL1_64K(v)			((ptp_t *)__L1_BASE_64K + __L1_MAP_64K(v)) //not used
#define	__KTOL2_64K(v)			((ptp_t *)__L2_BASE_64K + __L2_MAP_64K(v))
#define	__KTOL3_64K(v)			((pte_t *)__L3_BASE_64K + __L3_MAP_64K(v))

/*
 * Virtual address space layout for 4K page size:
--------------------------------------------------------------------------
  47<-- L0 -->39|38 <-- L1 --> 30|29 <-- L2 --> 21|20 <-- L3 --> 12|11 <--- POFF --->
--------------------------------------------------------------------------
 *
...
#define	__L2_VOFF(v)		((v) & ((1ULL << __L2_SHIFT) - 1))
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/system/public/aarch64/mmu.h $ $Rev: 931477 $")
#endif
