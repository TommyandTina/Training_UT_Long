...
void cpu_syspage_remap(uintptr_t vaddr, paddr_t paddr, size_t size, int flags);
...
int load_elf64mmu_4m(paddr_t addr, Elf64_Ehdr *hdr, Elf64_Phdr *phdr);
...
void do_init_system_private(void);
...
void chip_done(void);
const struct armv_chip *armv_chip_detect(void);
unsigned long arm_cpuspeed(void);
...
unsigned output_callouts(unsigned *const rwsize);
...
paddr_t		find_top_ram_aligned_limit(paddr_t size, unsigned align,
                                     unsigned pagesize, paddr_t limit);
...
void            reserve_ram(unsigned size, unsigned align, const char *, unsigned pagesize, const unsigned low_region);
...
void cpu_tweak_cmdline(struct bootargs_entry *bap, const char *);
...
void __ssp_fail(const char *msg);
...
void board_cpuconfig1(int cpu);
void board_cpuconfig2(int cpu);
...
void gicv_asinfo(paddr_t gicv, paddr_t banked_gich, paddr_t direct_gich, unsigned direct_num, unsigned direct_spacing);
...
void		hwitag_add_location(const unsigned hwi_off, const _Uint64t base, const _Uint32t len, const _Uint16t reg_shift, const _Uint16t addr_space);
...
void		hwi_add_location(const _Uint64t base, const _Uint32t len, const _Uint16t reg_shift, const _Uint16t addr_space);
...
int		fdt_smp_spin_start(unsigned cpu, void (*const start)(void));
unsigned long	fdt_get_cpu_freq(void);
void 	fdt_tweak_cmdline(struct bootargs_entry *bap, const char *name, paddr_t fdt_paddr);

void		spin_shim_rtn(void);
extern void	(*spin_shim_start)(void);
extern unsigned long spin_shim_id;
...
extern unsigned long				timer_prog_time;
...
extern PADDR_T						full_image_paddr;
extern PADDR_T						full_ram_paddr;
extern PADDR_T						full_imagefs_paddr;

#define RESERVED_RAM_MAX_NUM		16
...
extern struct reserved_ram			*reserved_ram_list;
...
void   init_8250_32b(unsigned, const char *, const char *);
...
extern struct callout_rtn	display_char_8250_32b;
extern struct callout_rtn	poll_key_8250_32b;
extern struct callout_rtn	break_detect_8250_32b;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/startup.h $ $Rev: 960172 $")
#endif
