...
int sysc_init_common(const sysc_opts_t* const sysc_opts, const uint32_t product_id)
{
    ...
    sysc_dev.apmu_pbase  = sysc_opts->apmu_base;
    sysc_dev.verbose = sysc_opts->verbose;
    sysc_dev.soc_id  = RCAR_PRODUCT_ID(product_id);
    sysc_dev.soc_rev = RCAR_PRODUCT_REV(product_id);

    if ( !IS_RCAR_GEN4(sysc_dev.soc_id) && !IS_RCAR_GEN3(sysc_dev.soc_id) ) {
...
sysc_support_funcs_t sysc_support_funcs = {
    ...
    .apmu_power_set  = rcar_apmu_power_set,
};
