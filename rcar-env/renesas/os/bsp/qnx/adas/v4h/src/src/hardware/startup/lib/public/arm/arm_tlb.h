...
static __inline__ void arm_setup_tlb(arm_tlb_t *table, unsigned long *tlb)
{
	...
	while (table && (table->start != -1L)) {
		...
		/* Fill the holes: cache/buff/exe disabled  */
		for (; offset < (base >> PAGE_SHIFT); offset++) {
			tlb[offset] = (offset << PAGE_SHIFT) | SECTION | AP_RW | DOMAIN_15 | B_DIS | C_DIS | E_DIS;
		}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/public/arm/arm_tlb.h $ $Rev: 838168 $")
#endif
