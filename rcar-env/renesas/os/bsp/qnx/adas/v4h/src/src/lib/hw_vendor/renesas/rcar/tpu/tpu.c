...
static struct tpu_info_s tpu_info[] = {
    /* Channel 0 */  { RCAR_TPU_0_OFFSET, TPU_TSTR_CST0 },
    /* Channel 1 */  { RCAR_TPU_1_OFFSET, TPU_TSTR_CST1 },
    /* Channel 2 */  { RCAR_TPU_2_OFFSET, TPU_TSTR_CST2 },
    /* Channel 3 */  { RCAR_TPU_3_OFFSET, TPU_TSTR_CST3 },
};
...
static uintptr_t tpu_map_registers(tpu_chan_t *chan)
{
    ...
    // check whether the memory for the TPU channel group is already mapped in.
...
static int tpu_create_resources(tpu_chan_t *chan)
{
    ...
    if (rsrc_ret < 0) {
        rsrc_ret = errno;
        TPU_SLOG_ERROR("%s: Failed querying %s tpu resources", __FUNCTION__, chan->name);
        return rsrc_ret;
    } else if (rsrc_ret == 0) {
        rsrc_req.start = 0;
        ...
        if( rsrc_ret != EOK ) {
            rsrc_ret = errno;
            TPU_SLOG_ERROR("%s: Failed creating tpu resources: err %d", __FUNCTION__, rsrc_ret );
            return rsrc_ret;
        }
    } else {
        return EALREADY;
...
static void * tpu_channel_attach(int priority, tpu_attach_flags flags)
{
    ...
    // The lower TPU channel numbers have highest priority
    ...
    // map in TPU registers for TPU channel group, including descriptor memory
...
{
	return in16(chan->vbase + RCAR_TPU_TSTR) & chan->tstr_mask;
}
...
{
    /* Clear status */
    out16(chan->regs + RCAR_TPU_TSR, 0);
    /* Disable interrupts */
    out16(chan->regs + RCAR_TPU_TIER, 0);		    
...
{
    /* Clear status */
    out16(chan->regs + RCAR_TPU_TSR, ~chan->intr_msk);
    /* Enable interrupts */
    out16(chan->regs + RCAR_TPU_TIER, chan->intr_msk);
...
static int tpu_timer_start_counter(void * hdl)
{
	...
	if(NULL != chan) {
		/* Start counter */
        out16(chan->vbase + RCAR_TPU_TSTR, in16(chan->vbase + RCAR_TPU_TSTR) | chan->tstr_mask);
...
static int tpu_timer_set_clock(void * hdl, tpu_prescalers_t prescaler, tpu_clock_edge_t clock_edge)
{
    ...
    if(NULL != chan) {
        ...
        if(error == 0) {
            ...
            out16(chan->regs + RCAR_TPU_TCR, 
                    (in16(chan->regs + RCAR_TPU_TCR) & ~(TPU_TCR_TPSC_MSK | TPU_TCR_CKEG_MSK)) | tcr);          

            TPU_SLOG_INFO(2, "%s: TCR=0x%04x (0x%x)", __func__, in16(chan->regs + RCAR_TPU_TCR), tcr);
...
static int tpu_timer_set_clear_src(void * hdl, tpu_clear_src_t cclr)
{
	...
	if(NULL != chan) {
        ...
        if (ret == EOK ) {
            ...
            out16(chan->regs + RCAR_TPU_TCR, 
                    (in16(chan->regs + RCAR_TPU_TCR) & ~TPU_TCR_CCLR_MASK) | tcr);
...
static int tpu_timer_set_mode(void * hdl, tpu_mode_t mode) 
{
	...
	if(NULL != chan) {
        ...
        if (ret == EOK ) {
            ...
            out16(chan->regs + RCAR_TPU_TMDR, tmdr);

            chan->mode = mode;

            TPU_SLOG_INFO(2, "%s: TMDR=0x%04x (0x%x)", __func__, in16(chan->regs + RCAR_TPU_TMDR), tmdr);
...
static int tpu_timer_set_pin_state(void * hdl, tpu_pin_state_t state, int initial) 
{
    ...
    if(NULL != chan) {
        ...
        if (ret == EOK) {
            ...
            out16(chan->regs + RCAR_TPU_TIOR, tior);

            TPU_SLOG_INFO(2, "%s: TIOR=0x%04x (0x%x)", __func__, in16(chan->regs + RCAR_TPU_TIOR), tior);
	...
	if(NULL != chan) {
        switch (reg) {
            case TPU_REG_GENERAL_A:
                out16(chan->regs + RCAR_TPU_TGRA, val);
                break;
            case TPU_REG_GENERAL_B:
                out16(chan->regs + RCAR_TPU_TGRB, val);
                break;
            case TPU_REG_GENERAL_C:
                out16(chan->regs + RCAR_TPU_TGRC, val);
                break;
            case TPU_REG_GENERAL_D:
                out16(chan->regs + RCAR_TPU_TGRD, val);
                break;
            default:
                TPU_SLOG_ERROR("%s: Invalid argument", __func__);
                ret = EINVAL;
...
static int tpu_timer_get_general_reg(void *hdl, tpu_general_reg_t reg, uint16_t * const val) 
{
	...
	if(NULL != chan) {
        switch (reg) {
        case TPU_REG_GENERAL_A:
            *val = in16(chan->regs + RCAR_TPU_TGRA);
            break;
        case TPU_REG_GENERAL_B:
            *val = in16(chan->regs + RCAR_TPU_TGRB);
            break;
        case TPU_REG_GENERAL_C:
            *val = in16(chan->regs + RCAR_TPU_TGRC);
            break;
        case TPU_REG_GENERAL_D:
            *val = in16(chan->regs + RCAR_TPU_TGRD);
            break;
        default:
            TPU_SLOG_ERROR("%s: Invalid argument", __func__);
            ret = EINVAL;
...
static int tpu_timer_set_count(void * hdl, uint16_t count)
{
	...
	if(NULL != chan) {
        out16(chan->regs + RCAR_TPU_TCNT, count);

        TPU_SLOG_INFO(2, "%s: TCNT=%04x (%04x)", __func__, in16(chan->regs + RCAR_TPU_TCNT), count);
...
static int tpu_timer_get_count(void *hdl, uint16_t * const count)
{
	...
	if(NULL != chan) {
		*count = in16(chan->regs + RCAR_TPU_TCNT);
    ...
    if(NULL != chan) {
        ...
        if (ret == EOK) {
            if ((chan->prescaler == prescalers[prescaler]) && (chan->period == period)) {
                ...
                if ((!error) && tpu_timer_set_clock(hdl, prescalers[prescaler], TPU_CLOCK_EDGE_RISE))
                    error++;
                ...
                if ((!error) && tpu_timer_set_general_reg(hdl, TPU_REG_GENERAL_A, duty))
                    error++;
                if ((!error) && tpu_timer_set_general_reg(hdl, TPU_REG_GENERAL_B, period))
                    error++;
...
static int tpu_start(void * hdl, uint16_t * count) 
{
	...
	if(NULL != chan) {
        if (chan->mode == TPU_MODE_PWM) {
            uint16_t tior = in16(chan->regs + RCAR_TPU_TIOR);
		...
		TPU_SLOG_INFO(2, "%s: TCR=%04x, TMDR=%04x, TIOR=%04x, TIER=%04x, TCNT=%04x", __func__, 
                        in16(chan->regs + RCAR_TPU_TCR),
                        in16(chan->regs + RCAR_TPU_TMDR),
                        in16(chan->regs + RCAR_TPU_TIOR),
                        in16(chan->regs + RCAR_TPU_TIER),
                        in16(chan->regs + RCAR_TPU_TCNT));
...
static int tpu_timer_intr_isr(void *hdl, uint16_t * const tsr)
{
    ...
    if(NULL != chan) {
        val  = in16(chan->regs + RCAR_TPU_TSR);
        val &= in16(chan->regs + RCAR_TPU_TIER);

        // Clear interrupt flags
        out16(chan->regs + RCAR_TPU_TSR, ~val);
