/*
 * $QNXLicenseC:
 * Copyright 2018-2020, QNX Software Systems.
 *
 ...
 *
 * Overview of the differences between hardware devices
 *
 * eMMC
 *  - Support for 1 RPMB partition
 *  - Sector size of 256 bytes
 *  - 512 byte frames for all commands (1:N)
 *
 * UFS
 *  - Support for up to 4 RPMB partitions
 *  - Sector size of 256 bytes
 *  - 512 byte frames for all commands (1:N)
 *
 * NVMe
 *  - Support for up to 7 RPMB partitions
 *  - Sector size of 512 bytes
 *  - single 256 byte header frame
 *   - optional 512 data frames following (command dependent) (0:N)
 */
...
#define RPMB_ERR_INVALID_CONFIG		0x0008
...
} rpmb_frame_jedec_t, RPMB_FRAME;

_Static_assert(sizeof(rpmb_frame_jedec_t) == 512, "invalid rpmb_frame_jedec_t size");

#define RPMB_STUFF_NVME_SIZE		223
#define RPMB_DATA_NVME_SIZE			512
#define RPMB_FRAME_NVME_SIZE		(sizeof( struct rpmb_frame_nvme_s ))
...
typedef struct __attribute__((__packed__)) rpmb_frame_nvme_s {
	...
	/*_Uint8t		data[];		   data payload in chunks of 512 bytes */
} rpmb_frame_nvme_t, RPMB_FRAME_NVME;

_Static_assert(sizeof(rpmb_frame_nvme_t) == 256, "invalid rpmb_frame_nvme_t size");
...
#define RPMB_MAX_PART	(7u)
typedef struct rpmb_partition_s {
	_Uint64t		start_lba;			/* Starting lba */
	_Uint64t		num_lba;			/* Total number of lba (256 or 512 bytes each) */
} rpmb_partition_t, RPMB_PARTITON;
...
#define RPMB_FLAG_WP			0x01
	...
	_Uint64t		start_lba;			/* Starting lba */
	_Uint64t		num_lba;			/* Total number of lba (256 or 512 bytes each) */
	_Uint32t		parts;				/* Available partitions */
#define RPMB_AUTH_ALG_HMAC		0x01	/* HMAC-SHA256 */
	_Uint32t		algo;				/* Authentication algorithm */
	rpmb_partition_t partitions[RPMB_MAX_PART];		/* RPMB partitions */
	_Uint32t		rsvd1[34];
} rpmb_info_t, RPMB_INFO;
...
#define	RPMB_FLG_RL_WRITE	0x02		/* Reliable Write */
	/**
	 * eMMC/UFS = number of full frames
	 * NVMe number of 512 byte chunks
	 */
	_Uint32t		nframes;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/devb/cam/public/sys/rpmb.h $ $Rev: 969525 $")
#endif
