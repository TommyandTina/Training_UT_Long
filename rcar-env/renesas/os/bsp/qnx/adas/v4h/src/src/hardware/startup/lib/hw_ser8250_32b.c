...
/*
 * Drive a 32-bit 8250 style serial chip.
 */
...
static void
init_8250_32b_common(unsigned const channel, unsigned long const baud, unsigned long const clk,
    unsigned long const div)
{
    ...
    if (aspace[channel] == 0)
    {
        ...
        do {
        } while(!(chip_read32(REG_LS) & LSR_TSRE));
        ...
        // 8 bits no parity
        chip_write32(REG_MC, (chip_read32(REG_MC) & 0xE4) | MCR_DTR|MCR_RTS|MCR_OUT2);
        chip_done();
    }
}

void
init_8250_32b(unsigned channel, const char *init, const char *defaults) {
    unsigned long    baud;
    unsigned long    div;
    unsigned long    clk;
    ...
    init_8250_32b_common(channel, baud, clk, div);
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/hw_ser8250_32b.c $ $Rev: 937906 $")
#endif
