...
uint16_t ravb_cl45_read (void *hdl, uint8_t phy_idx, uint16_t device, uint16_t reg)
{
    ...
    val = ravb_mdi_read (ravb, phy_idx, MMD_ADDR_DATA);

    if (ravb->options.cfg.verbose & VERBOSE_PHY) {
        slogf(_SLOGC_NETWORK, _SLOG_ERROR, "ravb: PHY dev 0x%02x reg 0x%04x data 0x%04x", device, reg, val);
    }
...
static void ravb_mii_callback(void *handle, uint8_t phyaddr, uint8_t linkstate)
{
    ...
    switch (linkstate) {
        case MDI_LINK_UP:
            ...
            if (cfg->media_rate == PHY_SPEED_1000M) {
...
static void ravb_do_mdi_monitor(void* arg)
{
    ...
    } else if (ravb_is_mv_phy(ravb)) {
        ravb_mv_phy_state(ravb);
    } else {
...
{
    uint8_t phy_idx = (uint8_t)ravb->options.cfg.phy_addr;
...
int ravb_mediachange(struct ifnet *ifp)
{
    ...
    switch (ifm->ifm_media & IFM_TMASK) {
        ...
        case IFM_1000_T:
            ravb->set_speed = PHY_SPEED_1000M;
            ravb->set_duplex = 0;
            ravb->options.set_flow = 0;
    ...
    switch (ravb->mdi->PhyData[cfg->phy_addr]->VendorOUI) {
        ...
        case MARVELLX:
            ...
            /* Re-initialize PHY here: confirm if needed */
            ravb_mv_phy_init(ravb);
            break;
...
static void ravb_mediainit(ravb_dev_t *ravb)
{
    ...
    if (ravb_is_mv_phy(ravb)) {
        ...
        return;
    } else if(ravb_is_br_phy(ravb)) {
        ifmedia_add(ifm, IFM_ETHER | IFM_100_TX | IFM_FDX, 0, NULL);
    } else if (ravb->options.emu_phy != -1) {
        ifmedia_add(ifm, IFM_ETHER | IFM_1000_T | IFM_FDX, 0, NULL);
    } else {
        /*
...
int ravb_phy_init(ravb_dev_t *ravb)
{
    ...
    for (;;) {
        if (cl45) {
            rc = MDI_Register_ExtendedCl45(ravb, ravb_cl45_write,
                    ravb_cl45_read, ravb_mii_callback, &ravb->mdi, NULL, 0, 0);
        } else {
        ...
        /* Initialize emulated PHY before MDI_FindPhy() */
        ...
        /*
         * Terminate scan as soon as PHY is found, or
         * both mii and cl45 APIs have been used but PHY is not found
         */
        if ((phy_addr != -1) || (++cl45 > 1)) {
            break;
        }
    }
