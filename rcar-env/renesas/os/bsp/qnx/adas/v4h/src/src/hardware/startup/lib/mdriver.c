	...
	for(i = 0; i < num; ++i, ++md) {
		if(md->intr != _NTO_INTR_SPARE) {
			if(md->handler(MDRIVER_STARTUP_FINI, md->data)) {
				...
				md->data = (void *)(uintptr_t)md->internal;
				md->internal = -1;
			}
		}
	}
...
mdriver_add(char *name, int intr, int (*handler)(int state, void *data),
				PADDR_T data_paddr, unsigned data_size) {
	uint8_t					*p;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/mdriver.c $ $Rev: 780356 $")
#endif
