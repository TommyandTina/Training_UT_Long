...
typedef enum {
	...
	DMA_CAP_BUFFER_CONTROL =	0x02000000	/* Buffer control are supported */
} dma_channel_caps;
...
typedef enum {
	...
	DMA_ATTACH_PROCESS =		0x00000100	/* Attach DMA interrupt with _NTO_INTR_FLAGS_PROCESS flag*/
} dma_attach_flags;

typedef enum {
	DMA_MODE_FLAG_REPEAT =		0x00000001,	/* Continuously repeat transfer */
	DMA_MODE_FLAG_BURST =		0x00000002,	/* Use burst mode */
	DMA_MODE_FLAG_BUFFER_CONTROL =	0x0000004,	/* Buffer is controlled */
} dma_mode_flags;
...
typedef struct _dma_transfer {
	...
	unsigned	req_id;	/* HW request ID when DMA_ADDR_FLAG_DEVICE is set */

	// buffer control, require DMA_CAP_BUFFER_CONTROL
	unsigned    buffer_mbu;
	unsigned    buffer_ulb;
} dma_transfer_t;
...
typedef struct _dma_functions {
	...
	void*    (*channel_attach)(const char *options, const struct sigevent *event,
	                           unsigned *channel, int priority, dma_attach_flags flags);
	void     (*channel_release)(void *handle);
	int      (*alloc_buffer)(void *handle, dma_addr_t *addr, unsigned size, unsigned flags);
	...
	void*    (*channel_attach_smmu)(const char *options, const struct sigevent *event,
	                                unsigned *channel, int priority, dma_attach_flags flags,
	                                struct smmu_object *sop);
	int      (*check_xfer_complete)(void *handle);
} dma_functions_t;
...
#define DMA_ADD_FUNC(table, entry, func, limit) \
	if (((size_t)&(((dma_functions_t *)0)->entry)) + \
	    sizeof (void (*)(void)) <= ((unsigned int)limit)) \
		(table)->entry = (func);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/lib/dma/public/hw/dma.h $ $Rev: 893668 $")
#endif
