...
unsigned
output_callouts(unsigned *const rwsize) {
	output_callout_t		*one_callout;
	unsigned	size;
	int sizing = (rwsize) ? 1 : 0;
	...
	size = ROUND(rtn_size, sizeof(uint64_t));
	if(sizing) {
		rtn_size = 0; // Init trigger in callout_output_one
		*rwsize = rw_size;
	}
	return(size);
}
...
callout_output_one(int sizing, void *rp) {
	...
	if(rtn_size == 0) {
		...
		rtn_size = ROUNDPG(rw_off + rw_size);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/callouts.c $ $Rev: 911753 $")
#endif
