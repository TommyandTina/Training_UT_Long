...
#define ECSD_CARD_TYPE					196
	...
	#define ECSD_CARD_TYPE_200MHZ		(ECSD_CARD_TYPE_HS200_1_8V | ECSD_CARD_TYPE_HS200_1_2V)
...
struct _sdmmc_hc
{
	...
	unsigned	ocr;				// SDHC clock
	unsigned	clock;				// SDHC clock
	unsigned	clk_max;				// SDHC clock
	unsigned	did;				// SOC ID
	unsigned	version;			// EMMC version
	unsigned	vdd;
	unsigned	signal_vlt;			// signal_voltage
	unsigned	timing;
	unsigned	caps;
...
#define SDMMC_CAP_HS400			(1 << 4)
#define SDMMC_CAP_TIMING_MASK	(SDMMC_CAP_HS | SDMMC_CAP_UHS | SDMMC_CAP_HS200 | SDMMC_CAP_HS400)

#define SDMMC_CAP_SV_1_2V		(1 << 24)	// 1.2V signal voltage supported
#define SDMMC_CAP_SV_1_8V		(1 << 25)	// 1.8V signal voltage supported
#define SDMMC_CAP_SV_3_0V		(1 << 26)	// 3.0V signal voltage supported
#define SDMMC_CAP_SV_3_3V		(1 << 27)	// 3.3V signal voltage supported

#define SDMMC_CAP_UHS			(SDMMC_CAP_DDR | SDMMC_CAP_SDR | SDMMC_CAP_HS200)
...
extern int sdmmc_set_partition(sdmmc_t *sdmmc, unsigned partition);
extern int sdmmc_fini(sdmmc_t *sdmmc);
#endif /* #ifndef _SDMMC_H_ */

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/ipl/lib/private/sdmmc.h $ $Rev: 904597 $")
#endif
