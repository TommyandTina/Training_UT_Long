...
{
    /* CMT0 (Type 0) */
    RCAR_GEN4_INTCSYS_SPI_500,
    RCAR_GEN4_INTCSYS_SPI_501,
    /* CMT1, CMT2, CMT3 (Type 1) */
    RCAR_GEN4_INTCSYS_SPI_448,
    RCAR_GEN4_INTCSYS_SPI_449,
    RCAR_GEN4_INTCSYS_SPI_450,
    RCAR_GEN4_INTCSYS_SPI_451,
    RCAR_GEN4_INTCSYS_SPI_452,
    RCAR_GEN4_INTCSYS_SPI_453,
    RCAR_GEN4_INTCSYS_SPI_454,
    RCAR_GEN4_INTCSYS_SPI_455,
    RCAR_GEN4_INTCSYS_SPI_456,
    RCAR_GEN4_INTCSYS_SPI_457,
    RCAR_GEN4_INTCSYS_SPI_458,
    RCAR_GEN4_INTCSYS_SPI_459,
    RCAR_GEN4_INTCSYS_SPI_460,
    RCAR_GEN4_INTCSYS_SPI_461,
    RCAR_GEN4_INTCSYS_SPI_462,
    RCAR_GEN4_INTCSYS_SPI_463,
    RCAR_GEN4_INTCSYS_SPI_464,
    RCAR_GEN4_INTCSYS_SPI_465,
    RCAR_GEN4_INTCSYS_SPI_466,
    RCAR_GEN4_INTCSYS_SPI_467,
    RCAR_GEN4_INTCSYS_SPI_468,
    RCAR_GEN4_INTCSYS_SPI_469,
    RCAR_GEN4_INTCSYS_SPI_470,
    RCAR_GEN4_INTCSYS_SPI_471,
};

/* IRQ number for V4H */
static uint16_t V4H_CMT_OVF_IRQS[] =
{
    /* CMT0 (Type 0) */
    RCAR_GEN4_INTCSYS_SPI_260,
    RCAR_GEN4_INTCSYS_SPI_261,
    /* CMT1, CMT2, CMT3 (Type 1) */
    RCAR_GEN4_INTCSYS_SPI_262,
    RCAR_GEN4_INTCSYS_SPI_263,
    RCAR_GEN4_INTCSYS_SPI_264,
    RCAR_GEN4_INTCSYS_SPI_265,
    RCAR_GEN4_INTCSYS_SPI_266,
    RCAR_GEN4_INTCSYS_SPI_267,
    RCAR_GEN4_INTCSYS_SPI_268,
    RCAR_GEN4_INTCSYS_SPI_269,
    RCAR_GEN4_INTCSYS_SPI_270,
    RCAR_GEN4_INTCSYS_SPI_271,
    RCAR_GEN4_INTCSYS_SPI_272,
    RCAR_GEN4_INTCSYS_SPI_273,
    RCAR_GEN4_INTCSYS_SPI_274,
    RCAR_GEN4_INTCSYS_SPI_275,
    RCAR_GEN4_INTCSYS_SPI_276,
    RCAR_GEN4_INTCSYS_SPI_277,
    RCAR_GEN4_INTCSYS_SPI_278,
    RCAR_GEN4_INTCSYS_SPI_279,
    RCAR_GEN4_INTCSYS_SPI_280,
    RCAR_GEN4_INTCSYS_SPI_281,
    RCAR_GEN4_INTCSYS_SPI_282,
    RCAR_GEN4_INTCSYS_SPI_283,
    RCAR_GEN4_INTCSYS_SPI_284,
    RCAR_GEN4_INTCSYS_SPI_285,
};
...
#define CS_MACHINE_LEN (32 + 1)
    ...
    if (option) {
        if ( strstr( option, "H3")  || strstr( option, "h3")  || 
            ...
            uint32_t  rcar_moder_reg = 0;
            uintptr_t vbase = (uintptr_t)mmap_device_io(4, RCAR_GEN4_RST_DM0_BASE + RCAR_GEN4_RST_MODEMR0);
            if (vbase != (uintptr_t)MAP_FAILED) {
                rcar_moder_reg = in32(vbase);
                munmap_device_io(vbase, 4);
            }
            ...
            if(rcar_moder_reg & BIT(29))
                ...
                cmt_clk_rclk = RCAR_GEN3_CPG_RCLK_32768;
            cmt_clk_cpex = SYSPAGE_ENTRY(qtime)->cycles_per_sec / 2;
            cmt_irq_map = &V4H_CMT_OVF_IRQS[0];
        } else {
...
static uintptr_t rcar_cmt_map_registers(cmt_chan_t *chan)
{
    ...
    if( chan->pbase == 0 ) {
        switch( chan->grp_idx ) {
            case 0: 
                chan->pbase = RCAR_CMT0_BASE; 
                chan->regs = RCAR_CMT0_REG_OFFSET;
                break;
            case 1: 
                chan->pbase = RCAR_CMT1_BASE;
                chan->regs = RCAR_CMT1_REG_OFFSET;
                break;
            case 2:
                chan->pbase = RCAR_CMT2_BASE;
                chan->regs = RCAR_CMT1_REG_OFFSET;
                break;
            case 3:
                chan->pbase = RCAR_CMT3_BASE;
                chan->regs = RCAR_CMT1_REG_OFFSET;
                break;
        }

        chan->size = RCAR_CMT_SIZE;
    }
    ...
    chan->regs += chan->vbase + chan->chan_idx_in_grp * RCAR_CMT_REG_SIZE;
        ...
        const void * data, int priority, int timer_bitsz, cmt_attach_flags flags)
{
    ...
    out32(chan->vbase + RCAR_CMT_CMCLKE,
            in32(chan->vbase + RCAR_CMT_CMCLKE) | (1 << ( (chan->grp_idx == 0 ? 5 : 0) + chan->chan_idx_in_grp)));
...
rcar_cmt_wait_for_access(void *handle)
{
    ...
    for (;;) {
        if ((in32(chan->regs + RCAR_CMT_CMCSR) & CMT_CMCSR_WRFLG) == 0)
            return 0;
...
rcar_cmt_read_cmcnt_reg(void *handle)
{
    ...
    if (chan->counter_support_48bit)
        cmcnt = ((uint64_t)in32(chan->regs + RCAR_CMT_CMCNTH)) << 32;

    cmcnt += in32(chan->regs + RCAR_CMT_CMCNT);
...
rcar_cmt_set_cmcnt_reg(void *handle, uint64_t value)
{
    ...
    for (i = 0 ; i < timeout; i++) {
        if ((in32(chan->regs + RCAR_CMT_CMCSR) & CMT_CMCSR_WRFLG) == 0) {
            ...
            if (chan->counter_support_48bit)
                out32(chan->regs + RCAR_CMT_CMCNTH, value >> 32);

            out32(chan->regs + RCAR_CMT_CMCNT, value);
...
rcar_cmt_counter_set_constant(void *handle, uint64_t value)
{
    ...
    out32(chan->regs + RCAR_CMT_CMCOR, cm_const);

    if (chan->counter_support_48bit)
        out32(chan->regs + RCAR_CMT_CMCORH, (cm_const >> 32) & 0xFFFF);
...
rcar_cmt_counter_get_constant(void *handle)
{
    ...
    cm_const = in32(chan->regs + RCAR_CMT_CMCOR);

    if (chan->counter_support_48bit)
        cm_const |= (uint64_t)in32(chan->regs + RCAR_CMT_CMCORH) << 32;
...
rcar_cmt_counter_setup(void *handle, uint64_t cmconst, int cntr_size, cmt_cm_mode_flags mode, cmt_clk_div_flags cks_div)
{
    ...
    // Stop counter
    out32(chan->regs + RCAR_CMT_CMSTR, 0);
    ...
    out32(chan->regs + RCAR_CMT_CMCSR, csr);
    if (chan->counter_support_48bit)
        out32(chan->regs + RCAR_CMT_CMCSRH, csrh);

    cmconst &= chan->counter_mask;
    if(!cmconst)
        cmconst = chan->counter_mask;

    out32(chan->regs + RCAR_CMT_CMCOR, cmconst);

    if (chan->counter_support_48bit)
        out32(chan->regs + RCAR_CMT_CMCORH, (cmconst >> 32) & 0xFFFF);
...
rcar_cmt_counter_start(void *handle)
{
    ...
    out32(chan->regs + RCAR_CMT_CMSTR, CMT_CMSTR_STR);
...
rcar_cmt_counter_overflow(void *handle)
{
    ...
    cmcsr = in32(chan->regs + RCAR_CMT_CMCSR);

    // Clear interrupt flags
    out32(chan->regs + RCAR_CMT_CMCSR, cmcsr & ~(CMT_CMCSR_CMF | CMT_CMCSR_OVF));
...
rcar_cmt_channel_release(void *handle)
{
    ...
    // Stop counting
    out32(chan->vbase + RCAR_CMT_CMSTR, 0);
