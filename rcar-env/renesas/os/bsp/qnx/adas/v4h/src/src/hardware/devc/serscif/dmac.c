    ...
    for (;;) {
        ...
        switch (pulse.code) {
            ...
            case RCAR_SCIF_TDMA_EVENT:
                bytes_left = dev->dma.dmafuncs.bytes_left(dev->dma.Tx.dma_chn);
                if (bytes_left == 0) {
                    dev->dma.dmafuncs.xfer_complete(dev->dma.Tx.dma_chn);
                } else {
                    /* avoid adding log messages here since simply typing on the
                     * serial console is generating bytes_left != 0 thus, flooding slog.
                     * however, manually sending data to /dev/ser1
                     * always results in bytes_left=0
                     */
                    /* abort any pending transfers, if any */
            ...
            default:
                /* ignore invalid pulse code */
                break;
        }
    }
    /* cleanly exit the process by invoking term_thread */
    kill(getpid(), SIGTERM);
    return (NULL);
}
...
static void create_dma_attach_opt(DEV_SCIF *dev, char* const dma_attach_opt)
{
    if ((dev->dma.chan_min != -1) && (dev->dma.chan_max != -1)) {
        if (dev->dma.typed_mem) {
            sprintf(dma_attach_opt, "dma=sys,range_min=%d,range_max=%d,typed_mem=%s",
                    dev->dma.chan_min, dev->dma.chan_max, dev->dma.typed_mem);
        } else {
            sprintf(dma_attach_opt, "dma=sys,range_min=%d,range_max=%d",
                    dev->dma.chan_min, dev->dma.chan_max);
        }
    } else {
        if (dev->dma.typed_mem) {
...
int scif_dma_init(DEV_SCIF *dev)
{
    ...
    if (dev->dma.smmu_obj != NULL) {
        ...
        dev->dma.Tx.dma_chn = dev->dma.dmafuncs.channel_attach(dma_attach_opt, &dev->dma.Tx.event, NULL, priority, attach_flags);
    }
    ...
    if (dev->dma.smmu_obj != NULL) {
        ...
        dev->dma.Rx.dma_chn = dev->dma.dmafuncs.channel_attach(dma_attach_opt, &dev->dma.Rx.event, NULL, priority, attach_flags);
    }
