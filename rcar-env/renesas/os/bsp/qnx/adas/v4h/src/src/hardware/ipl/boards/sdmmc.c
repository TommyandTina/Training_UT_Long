...
static void mmc_parse_ext_csd(sdmmc_t *sdmmc, unsigned char *raw_ecsd)
{
	...
	if ((ecsd->card_type & ECSD_CARD_TYPE_200MHZ) && (sdmmc->caps & SDMMC_CAP_HS200) ) {
		card->speed = DTR_MAX_HS200;
	} else if (ecsd->card_type & ECSD_CARD_TYPE_52MHZ) {
...
{
	sdmmc_hc_t *hc = sdmmc->hc;
	CMD_CREATE (sdmmc->cmd, SD_SWITCH_VOLTAGE, 0, 0, 0, 0, 0);
...
static int sd_send_op_cond(sdmmc_t *sdmmc)
{
	...
	unsigned rsp[4];
	int i;
	unsigned ocr = OCR_PWRUP_CMP | OCR_HCS;

	if ((sdmmc->caps & SDMMC_CAP_SV_1_8V) && (sdmmc->caps & SDMMC_CAP_UHS)) {
		ocr = ocr | OCR_S18R;
	}
	...
	for (i = 0; i < POWER_UP_WAIT; i++) {
		...
		CMD_CREATE (sdmmc->cmd, SD_SEND_OP_COND, ocr | sdmmc->ocr, rsp, 0, 0, 0);
		if (SDMMC_OK != hc->send_cmd(sdmmc)) {
			return SDMMC_ERROR;
		}
	...
	if ((ocr && OCR_S18R) && (rsp[0] & OCR_S18A)) {
		if (sd_voltage_switch(sdmmc)) {
			if (sdmmc->verbose) {
				ser_putstr("Can't switch voltage to 1.8v\n");
			}
			return SDMMC_ERROR;
		} else {
			hc->signal_voltage(sdmmc, SIGNAL_VOLTAGE_1_8);
		}
	}
...
static int sdmmc_set_relative_addr(sdmmc_t *sdmmc)
{
	...
	if (SDMMC_OK != sdmmc_get_state(sdmmc)) {
		ser_putstr("Get State failed during sdmmc_set_relative_addr\n");
		return SDMMC_ERROR;
	}
...
static int sdmmc_select_card(sdmmc_t *sdmmc)
{
	...
	if (SDMMC_OK != sdmmc_get_state(sdmmc)) {
		ser_putstr("Get State failed during sdmmc_select_card\n");
		return SDMMC_ERROR;
	}
...
static int mmc_switch(sdmmc_t *sdmmc, int mode, int index, int value, int cmdset)
{
	...
	do {
		if (SDMMC_OK != sdmmc_get_state(sdmmc)) {
			ser_putstr("Get State failed switching to mode 0x");ser_puthex(mode);ser_putstr("\n");
			return SDMMC_ERROR;
		}
...
static int mmc_init_hs200(sdmmc_t *sdmmc)
{
	...
	if ((status = hc->tuning(sdmmc, MMC_SEND_TUNING_BLOCK))) {
		ser_putstr("hc->tuning failure\n");
		return( status );
	}
...
static int mmc_init_bus(sdmmc_t *sdmmc)
{
	...
	/* NOTE: Since rcar gen3 does not have HS400 mode support
	 * in the IPL yet, the flag check for SDMMC_CAP_UHS is replaced
	 * with a check for eMMC spec v4+ due to the ECSD being
	 * introduced in spec v4.
	 */
	if (!(sdmmc->caps & SDMMC_CAP_SKIP_IDENT) && (sdmmc->csd.spec_vers >= CSD_SPEC_VER_4)) {
...
static int sdmmc_init_card(sdmmc_t *sdmmc)
{
	...
	if (SDMMC_OK != sdmmc_get_state(sdmmc)) {
		ser_putstr("Get State failed after setting block length\n");
		return SDMMC_ERROR;
	}
