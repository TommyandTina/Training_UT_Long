...
struct startup_header {
	...
	_Uint32t		zero[1];			/*   Zeros */
	_Uint64t		addr_off;			/*S  Offset to add to startup_vaddr, image_paddr, */
										/*   ram_paddr, and imagefs_paddr members         */
...
struct bootargs_entry {
	...
	_Int8t					argc;
	_Int8t					envc;
	/* If the actual startup header address cannot be represented in 32 */
	/* bits, then this member is set to SHDR_ADDR_64 and a _Uint64 with */
	/* the REAL shdr address immediately follows the last byte of the   */
	/* variable-length <args>; it will be included in the advertised    */
	/* <size_*> and is located at  <bootargs_addr> + <size> - 8 .       */
	_Uint32t				shdr_addr;
#define SHDR_ADDR_64  (~((_Uint32t)0U))
...
/* __SRCVERSION("startup.h $Rev: 892924 $"); */

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/public/sys/startup.h $ $Rev: 892924 $")
#endif
