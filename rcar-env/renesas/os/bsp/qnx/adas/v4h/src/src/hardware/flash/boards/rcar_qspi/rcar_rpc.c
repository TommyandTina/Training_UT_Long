...
#define HYPER_SMENR_4BIT_SIZES (RCAR_RPC_SMENR_CDB(4) | RCAR_RPC_SMENR_OCDB(4) | \
                                RCAR_RPC_SMENR_ADB(4) | RCAR_RPC_SMENR_OPDB(4) | \
                                RCAR_RPC_SMENR_SPIDB(4))
...
static int rpc_hyper_write(void *hdl, uint32_t addr, uint8_t *data, int dlen)
{
    ...
    while ((dlen - pos) >= 256) {
        if (rpc_hyper_write_buffer(hdl, addr + pos, data + pos, 256) < 0) {
            return (-1);
        }
        pos += 256;
    }
