...
int ravb_mii_op_ioctl(ravb_dev_t *ravb, struct ifdrv *ifd)
{
    ...
    /* Check if correct name was passed */
    if (strncmp (ifd->ifd_name, "ravb", 4)) {
        return EINVAL;
    }
    if (atoi(&ifd->ifd_name[4]) >= AVB_CHAN_NUM) {
        return EINVAL;
    }
        ...
        switch(mdi_op.mii_op.op) {
            ...
            case WRITE:
                if ((ravb->options.emu_phy != -1) &&
                    (ravb->options.emu_phy == ravb->options.cfg.device_index)) {
                    /* Update the emulated PHY register upon request */
                    emu_phy_write(ravb, mdi_op.mii_op.addr, mdi_op.mii_op.reg, mdi_op.mii_op.data);
                } else {
                ...
                break;
            case CL45_READ:
                mdi_op.cl45_op.phy_data = ravb_cl45_read (ravb, mdi_op.cl45_op.phy_id, mdi_op.cl45_op.phy_dev, mdi_op.cl45_op.phy_reg);
                ...
                break;
            case CL45_WRITE:
                ravb_cl45_write (ravb, mdi_op.cl45_op.phy_id, mdi_op.cl45_op.phy_dev, mdi_op.cl45_op.phy_reg, mdi_op.cl45_op.phy_data);
                mdi_op.cl45_op.err = 0;
                if (cfg->verbose > 5) {
