...
int rifs_load_ifs2(void)
{
	...
	if(!(rifs_flag & RIFS_FLAG_IFS2_SRC)) {
		...
		ifs2_paddr_src = full_imagefs_paddr + shdr->stored_size - shdr->startup_size;
	}

	if(debug_flag > RIFS_DEBUG_LEVEL)
	{
		kprintf("ifs2_paddr_src: 0x%P\n", ifs2_paddr_src);
		kprintf("ifs2_paddr_src (auto): 0x%P\n", (paddr_t)(full_imagefs_paddr + shdr->stored_size - shdr->startup_size));
	}
...
int rifs_restore_ifs(paddr_t ifs_paddr)
{
	...
	if(check_ifs_signature(ifs_hdr) == 0 && check_rifs_signature(rifs_info) == 0)
	{
		...
		for(i = 0; i < rifs_info->numboot; i++)
		{
			...
			paddr_dst = full_image_paddr + shdr->startup_size + rifs_info->elfinfo[i].offset;
			...
			else
			{
				...
				paddr_src = full_imagefs_paddr + rifs_info->elfinfo[i].offset;
				if(debug_flag > RIFS_DEBUG_LEVEL)
				{
	...
	if((phdr = rifs_readelf(addr, &off, &size)))
	{
		...
		if(shdr->flags1 & STARTUP_HDR_FLAGS1_COMPRESS_MASK)
		{
			...
			copy_memory(rifs_info->elfinfo[numboot].data,
					full_image_paddr + shdr->startup_size + rifs_info->elfinfo[numboot].offset,
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/restore_ifs.c $ $Rev: 892924 $")
#endif
