...
alloc_syspage_memory() {
	...
	if(num_mdriver) memset((void*)old_mdriver, 0, sp->old_mdriver.entry_size);
	...
	if(num_intrinfo) memset((void*)old_intrinfo, 0, sp->old_intrinfo.entry_size);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/syspage_memory.c $ $Rev: 951543 $")
#endif
