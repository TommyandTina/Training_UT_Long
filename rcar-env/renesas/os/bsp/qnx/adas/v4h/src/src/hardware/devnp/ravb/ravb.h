...
/* Definition for GEN4 */
#define AVB0_REG_BASE                   0xE6800000
#define AVB1_REG_BASE                   0xE6810000
#define AVB2_REG_BASE                   0xE6820000
#define AVB3_REG_BASE                   0xE6830000
#define AVB4_REG_BASE                   0xE6840000
#define AVB5_REG_BASE                   0xE6850000

#define AVB_CHAN_NUM                    3

/* Definition for IRQ V3U */
#define RCAR_V3U_AVB_DMAC_IRQ           (278 + 32)  // RCAR_V3U_INTCSYS_RAVB0_22
#define RCAR_V3U_AVB_EMAC_IRQ           (280 + 32)  // RCAR_V3U_INTCSYS_RAVB0_24

/* Definition for IRQ V4H */
#define RCAR_V4H_AVB_DMAC_IRQ           (389)       // RCAR_V4H_INTCSYS_RAVB0_22 (SPI_357)
#define RCAR_V4H_AVB_EMAC_IRQ           (391)	    // RCAR_V4H_INTCSYS_RAVB0_24 (SPI_359)
/*
 * io-pkt defaults to 8192 clusters of 2048 bytes each.
 * On an ravb we have 4 queues of Tx and 18 queues of Rx.
...
#define NUM_TX_QUEUES                   4           // Queues are 0 (BE), 1(PTP), 3(Class A) and 2(Class B) according to page 50-283 of the HW Users Manual
#define NUM_RX_QUEUES                   2           // Queues are 0 (BE), 1(PTP), 2-17 (Stream)

#define DBAT_ENTRY_NUM                  (NUM_TX_QUEUES + NUM_RX_QUEUES)
#define TX_QUEUE_OFFSET                 0
#define RX_QUEUE_OFFSET                 NUM_TX_QUEUES

#define RAVB_QUIESCE_PULSE              _PULSE_CODE_MINAVAIL
#define RAVB_RX_PULSE                   (RAVB_QUIESCE_PULSE + 1)
#define RAVB_RX_THREAD_NAME             "ravb Rx"
...
#define APSR                0x008C      // AVB-DMAC Product Specific Register
    ...
    #define APSR_RDM            (1 << 13)       // Rx clock internal Delay Mode (0: normal, 1: delayed)
    #define APSR_TDM            (1 << 14)       // Tx clock internal Delay Mode (0: normal, 1: delayed)
    #define APSR_MIISELECT      (1 << 24)       // MII clock select (0: RGMII clock, 1: MII clock)
    #define APSR_RSEL           (1 << 30)       // Reference Clock Selection For TX (0: Use AVBn_TXCREFCLK, 1: Use TSN0_TXCREFCLK)
#define RCR                 0x0090      // Receive Configuration Register
    ...
    #define RCR_ESF             (3 << 2)        // Stream Filtering Select
...
#define TGC                 0x0300      // Transmit Configuration Register
    ...
    #define TGC_TBD_DEF         (2 << 20 | 2 << 16 | 2 << 12 | 2 << 8)
...
#define TSR                 0x0308      // Transmit Status Register
    ...
    #define TSR_TFFL            (7 << 8)        // Timestamp FIFO Fill Level
...
#define EIS                 0x035C      // Error Interrupt Status Register
    ...
    #define EIS_RESERVED        ((0x7FFF << 17) | (0xF << 11))
...
#define RIS0                0x0364      // Receive Interrupt Status Register
    ...
    #define RIS0_RESERVED       (0x3FFF << 18)
#define RIC1                0x0368
...
#define RIS2                0x0374
    ...
    #define RIS2_RESERVED       (0x1FFF << 18)
...
#define GCCR                0x0390      // gPTP Configuration Control Register
    ...
    #define GCCR_TCSS_PTP       (0 << 8)        // gPTP timer value
    #define GCCR_TCSS_APTP      (1 << 8)        // Adjusted gPTP timer value
    #define GCCR_TCSS_AVTP      (2 << 8)        // AVTP presentation time
    ...
    #define GCCR_TCR_NOP        (0 << 0)        // Timer control is not requested
    #define GCCR_TCR_RESET      (1 << 0)        // gPTP/AVTP presentation time reset
    #define GCCR_TCR_STREAM     (2 << 0)        // Continuous capture of AVTP to GCTt.CTV
    #define GCCR_TCR_CAPTURE    (3 << 0)        // Captures the value set in the TCSS bit
...
#define GIS                 0x03B0      // gPTP Interrupt Status Register
    ...
    #define GIS_PTMF0           (1 << 2)        // Presentation Time Match Interrupt Flag
...
#define GIE                 0x03CC      // Gptp Interrupt Enable register
    ...
    #define GIE_PTMS0           (1 << 2)        // Presentation Time Match Interrupt Set
...
#define TROCR               0x0700      // Transmit retry over counter register
...
/* PTP */
#define RAVB_RX_TIMESTAMP_BUF_SZ    64
#define RAVB_TX_TIMESTAMP_BUF_SZ    16
...
#define CS_MACHINE_LEN (32 + 1)
    char                soc_name[CS_MACHINE_LEN];
} attach_args_t;

#define EVP_ARRAY_SIZE 3

#if defined(VARIANT_ipmmu)
#define IOMMU_NODE_MAX  (NUM_TX_DESC * NUM_TX_QUEUES + NUM_RX_QUEUES * NUM_RX_DESC + NUM_TX_QUEUES + NUM_RX_QUEUES + 512)
typedef struct iommu_node {
    uint64_t paddr;
    uint32_t iova;
    uint32_t size;
    struct iommu_node *right_node;
    struct iommu_node *left_node;
} iommu_node_t;
...
typedef struct {
    ...
    uint32_t            ris0;
    RAVB_DESC           *desc_bat;
    off64_t             desc_bat_addr;
    RAVB_TX_DESC        *tx_bd[NUM_TX_QUEUES];
    RAVB_RX_DESC        *rx_bd[NUM_RX_QUEUES];
...
#if defined(VARIANT_ipmmu)
    int                 ipmmu_handle;
    int                 ipmmu_node_count;
    iommu_node_t         *ipmmu_tree_root;
#endif /* defined(VARIANT_ipmmu) */
} ravb_dev_t;
...
int ravb_output(struct ifnet *ifp, struct mbuf *m, struct sockaddr *dst, struct rtentry *rt);
...
int ravb_process_interrupt_avb_rx (void *arg, struct nw_work_thread *wtp);
int ravb_dmac_enable_interrupt_rx(void * arg);
int ravb_dmac_enable_interrupt_avb_rx (void *arg);
...
const struct sigevent * ravb_avb_isr (void *arg, int iid);
...
void ravb_ptp_stop(ravb_dev_t *ravb);
void ravb_ptp_cal(ravb_dev_t *ravb);
...
void ravb_mii_write(void *handle, uint8_t phy_id, uint8_t location, uint16_t data);
uint16_t ravb_mii_read(void *handle, uint8_t phy_id, uint8_t location);
void ravb_mdi_write(void *handle, uint8_t phy_id, uint8_t location, uint16_t data);
uint16_t ravb_mdi_read(void *handle, uint8_t phy_id, uint8_t location);
void ravb_cl45_write (void *hdl, uint8_t phy_idx, uint16_t device, uint16_t reg, uint16_t val);
uint16_t ravb_cl45_read (void *hdl, uint8_t phy_idx, uint16_t device, uint16_t reg);
int     ravb_phy_funcs (ravb_dev_t *ravb, struct ifdrv *ifd);
int     ravb_cl45_phy_funcs (ravb_dev_t *ravb, struct ifdrv *ifd);
...
#if defined(VARIANT_ipmmu)
iommu_node_t *iommu_get_node(ravb_dev_t *ravb, uint64_t phys, uint32_t size);
void iommu_cleanup(ravb_dev_t *ravb);
#endif /* defined(VARIANT_ipmmu) */

#define TX_INT_FLAGS (TIC_FTE0 | TIC_FTE1 | TIC_TFUE | TIC_TFWE)
#define TX_INT_TIE_FLAGS (TIE_FTS0 | TIE_FTS1 | TIE_TFUS | TIE_TFWS)
#define TX_INT_TID_FLAGS (TID_FTD0 | TID_FTD1 | TID_TFUD | TID_TFWD)
#define FRX_INT_FLAGS (RIC0_FRE0 | RIC0_FRE1)
#define FRX_INT_RIE0_FLAGS (RIE0_FRS0 | RIE0_FRS1)
#define FRX_INT_RID0_FLAGS (RID0_FRD0 | RID0_FRD1)

static inline void unmask_tx_int(ravb_dev_t *ravb) {
    out32(ravb->base + TIE, TX_INT_TIE_FLAGS);
}

static inline void unmask_rx_int(ravb_dev_t *ravb) {
    out32(ravb->base + RIE0, FRX_INT_RIE0_FLAGS);
}

static inline void mask_tx_int(ravb_dev_t *ravb) {
    out32(ravb->base + TID, TX_INT_TID_FLAGS);
}

static inline void mask_rx_int(ravb_dev_t *ravb) {
    out32(ravb->base + RID0, FRX_INT_RID0_FLAGS);
}

static inline void unmask_fte0_int(ravb_dev_t *ravb) {
    out32(ravb->base + TIE,  TIE_FTS0);
}

static inline void mask_fte0_int(ravb_dev_t *ravb) {
    out32(ravb->base + TID, TID_FTD0);
}

static inline void mask_fte1_int(ravb_dev_t *ravb) {
    out32(ravb->base + TID, TID_FTD1);
}

static inline void unmask_tx1_int(ravb_dev_t *ravb) {
    out32(ravb->base + TIE,  TIE_FTS1 | TIE_TFUS | TIE_TFWS);
}
