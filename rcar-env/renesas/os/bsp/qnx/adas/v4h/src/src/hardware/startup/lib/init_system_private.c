...
init_system_private() {
	...
	if(lsp.mdriver.size > 0) {
		...
		// startup memory.
		alloc_ram(full_ram_paddr, shdr->startup_size, 1);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/init_system_private.c $ $Rev: 973831 $")
#endif
