...
#ifndef _RCAR_SDHI_INCLUDED
#define _RCAR_SDHI_INCLUDED
...
#define MMC_CMD1        0x00000001                     /* SEND_OP_COND */
#define MMC_CMD2        (SDH_CMD_RSPR2  | 0x00000002)  /* ALL_SEND_CID */
...
#define MMC_CMD8        0x00000008                     /* SEND_EXT_CSD */
#define MMC_CMD9        (SDH_CMD_RSPR2  | 0x00000009)  /* SEND_CSD */
#define MMC_CMD10       0x0000000A
#define MMC_CMD11       0x0000040B                     /* Voltage Switch - Not in the response busystate */
#define MMC_CMD12       0x0000000C
#define MMC_CMD13       (SDH_CMD_RSPR1  | 0x0000000D)  /* SEND_STATUS */
#define MMC_CMD14       0x00001C0E
#define MMC_CMD15       0x0000000F
#define MMC_CMD16       0x00000010                     /* SET_BLOCKLEN */
#define MMC_CMD17       (SDH_CMD_ADTC | 0x00000011)    /* READ_SINGLE_BLOCK */
#define MMC_CMD18       0x00003C12                     /* READ_MULTI_BLOCK */
...
#define MMC_ACMD6       0x00000046
#define MMC_ACMD41      0x00000069

/* Stop register */
#define SDH_STOP_STP            (1 <<  0)
#define SDH_STOP_SEC            (1 <<  8)
#define SDH_STOP_DEFAULT        (0)
...
/* SDIF_MODE */
#define SDIF_MODE_DEFAULT       (0)
#define SDIF_MODE_HS400             (1 << 0)

#define RCAR_SDHI_SCC_TMPPORT2_HS400OSEL    (1 << 4)
#define RCAR_SDHI_SCC_TMPPORT2_HS400EN      (1 << 31)

/* CC_EXT_MODE */
#define BUF_ACC_DMAWEN          (1 << 1)
#define BUF_ACC_DMAWDIS         (0 << 1)
...
/* MMC_HOST_MODE */
#define MMC_HOST_MODE_BUF_WIDTH_32  (1 << 0)
#define MMC_HOST_MODE_BUF_WIDTH_64  (0 << 0)
#define MMC_HOST_MODE_BUS_WIDTH_32  (1 << 8)
...
#define SDH_CMD_ACMD            (1 <<  6)   /* ACMD */

/*-------------------------------------------*/
/* R-Car Gen3 SDMMC Board Specific Functions */
/*-------------------------------------------*/
typedef struct rcarsdmmc_bs {
    void (*init)(sdmmc_t *sdmmc);
    int (*signal_voltage)(sdmmc_t *sdmmc, int signal_voltage);
} rcar_sdmmc_bs_t;

extern void rcar_sdmmc_init_hc(sdmmc_t *sdmmc, unsigned base, unsigned clock, int verbose);
extern void rcar_sdmmc_init_bs(rcar_sdmmc_bs_t *bs);
extern int rcar_sdmmc_fini(sdmmc_t *sdmmc);

static inline uint32_t sdhi_read(uintptr_t base, int reg)
{
    return in32(base + reg);
}

static inline void sdhi_write(uintptr_t base, int reg, uint32_t val)
{
    out32(base + reg, val);
}

#endif /* #ifndef _RCAR_GEN3_SDHI_INCLUDED */

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/ipl/lib/private/rcar_gen3_sdhi.h $ $Rev: 880154 $")
#endif
