...
/* MMC_HOST_MODE */
#define HOST_MODE_BUSWIDTH_32BIT  (1 << 8)
#define HOST_MODE_WMODE_N64BIT    (1 << 0)
...
#define SDHI_TMOUT                  1000  // FIXME : is it enough?

#define DMA_DESC_MAX                256

#ifdef VARIANT_ipmmu
#define NUM_IPMMU_OBJECTS           32
struct ipmmu_obj {
    uint64_t paddr;
    uint32_t iova;
    uint32_t len;
    TAILQ_ENTRY(ipmmu_obj) ipmmu_tailq;
};
#endif /* VARIANT_ipmmu */
...
static inline uint64_t sdmmc_read64(uintptr_t base, int reg)
{
