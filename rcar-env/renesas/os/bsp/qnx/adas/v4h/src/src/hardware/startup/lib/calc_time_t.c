...
calc_time_t(const struct tm *tm) {
	...
	if ((tm->tm_mon >= 0) && (tm->tm_mon < NUM_ELTS(month_sec))) {
		sec += month_sec[tm->tm_mon];
	}
	else {
		crash("%s: Month returned by RTC is out of bounds\n", __FUNCTION__);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.x_dev/trunk/hardware/startup/lib/calc_time_t.c $ $Rev: 756950 $")
#endif
