...
int suspend_already = 0;
int suspend_resume_running = 1;
#endif
...
static void rcar_sdmmc_debug(sdio_hc_t *hc, char *title)
{
    ...
    if (sdmmc->adjust_hs400_enable && hc->timing == TIMING_HS400) {
...
static struct ipmmu_obj * rcar_ipmmu_remap_obj(rcar_sdmmc_t *sdmmc, struct ipmmu_obj *obj, uint64_t paddr, uint32_t len)
{
    ...
    /* Unmap address
     * TODO: What is correct behavior if unmap fails?
...
static void rcar_sdmmc_flush_sd_buf(sdio_hc_t *hc)
{
    rcar_sdmmc_t    *sdmmc;
...
static int rcar_sdmmc_xfer_setup(sdio_hc_t *hc, sdio_cmd_t *cmd)
{
    ...
    if (cmd->blks > 1 || (cmd->flags & SCF_MULTIBLK)) {
...
static int rcar_sdmmc_bus_width(sdio_hc_t *hc, int width)
{
    ...
    sdio_slogf(_SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: width = %d", __func__, width);
...
static int rcar_sdmmc_clk(sdio_hc_t *hc, int clk)
{
    ...
    sdio_slogf(_SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s : clk = %d", __func__, clk);
     ...
     * 4 TAP: SRCFC=0, FC=1
     */
    if (hc->timing == TIMING_HS400 && sdmmc->hs400_use_4tap && clock == 200000000) {
        clock = 400000000;
    }
    ...
    clkctl &= 0xFFFFFF00;
    clkctl |= clock_div(pclock, &clock, hc->timing);
...
static int rcar_sdmmc_select_tuning(sdio_hc_t *hc)
{
    ...
    int      ntap;      /* Temporary number of 'OK' TAP */
...
static int rcar_sdmmc_tune(sdio_hc_t *hc, int op)
{
    ...
    for (i = 0; i < sdmmc->tap_num * 2; i++) {
        ...
        if ((cmd->status == CS_CMD_CMP)) {
            ...
            delay(4);
            //_sdio_stop_transmission( &hc->device, 0 );
        }
...
static void rcar_sdmmc_resume(sdio_hc_t *hc)
{
    rcar_sdmmc_t *sdmmc = (rcar_sdmmc_t *)hc->cs_hdl;
    int timing;
    ...
    /* Remove standby mode */
    cpg_hwfuncs.mstp_enable(CPG_MSTP_GEN4_SDHI0);

    /* Start clock supply */
    cpg_hwfuncs.clk_enable(CPG_CLK_SDHI, sdmmc->chan_idx);

    /* Start clock */
    sdmmc_write(sdmmc->vbase, MMC_SD_CLK_CTRL, sdmmc_read(sdmmc->vbase, MMC_SD_CLK_CTRL) | SDH_CLKCTRL_SCLKEN);

    /*
     * In changing state from PM_SUSPEND to PM_ACTIVE, if a _sdio_reset() is called in _sdio_pwrmgnt
     * does not need following code segment
     */
    ...
    rcar_sdmmc_bus_width(hc, hc->bus_width);
    timing = hc->timing;

    if ((timing == TIMING_SDR104) ||
        (timing == TIMING_HS200)  ||
        (timing == TIMING_HS400)) {
        sdmmc->tuning_status = RCAR_NEED_RETUNING;
        if (hc->flags & HC_FLAG_DEV_MMC) {
            ...
            rcar_sdmmc_tune(hc, SD_SEND_TUNING_BLOCK);
        }
        ...
        rcar_sdmmc_timing(hc, hc->timing);
    }

    /* Enable the card insertion and removal interrupts */
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO1_MASK,  sdmmc_read(sdmmc->vbase, MMC_SD_INFO1_MASK) & ~(SDH_INFO1_INST | SDH_INFO1_RMVL));
}

static void rcar_sdmmc_suspend(sdio_hc_t *hc)
{
    rcar_sdmmc_t *sdmmc = (rcar_sdmmc_t *)hc->cs_hdl;
    ...
    /* Stop clock supply */
    cpg_hwfuncs.clk_disable(CPG_CLK_SDHI, sdmmc->chan_idx);

    /* Put into standby mode */
    cpg_hwfuncs.mstp_disable(CPG_MSTP_GEN4_SDHI0);
}

static int rcar_sdmmc_pm(sdio_hc_t *hc, int action)
{
    switch (action) {
        case PM_IDLE:
            break;
...
static void rcar_sdmmc_parse_hs400_support(sdio_hc_t *hc)
{
    ...
    if ((prr_reg = mmap_device_io(4, RCAR_PRODUCT_REGISTER)) != (uintptr_t)MAP_FAILED) {
        prr_val = in32(prr_reg);
        switch RCAR_PRODUCT_ID(prr_val) {
            ...
            case PRODUCT_ID_RCAR_V3H:
                switch RCAR_PRODUCT_REV(prr_val) {
                    case PRODUCT_V3H_REV_2_1:
                        /* FIXME : How for V3H 2.1 ?) ? */
                        sdmmc->hs400_manual_correction = 0;
                        sdmmc->hs400_ignore_dat_correction = 0;
                        break;
                    default:
                        /* V3H early rev.2.1 doesn't support HS400 speed */
                        hc->caps &= ~HC_CAP_HS400;
                        break;
                }
            ...
            case PRODUCT_ID_RCAR_V4H:
                /* FIXME : How about V3U, S4, V4H (No document for V3U, S4, V4H yet) ? */
                sdmmc->hs400_manual_correction = 0;
                sdmmc->hs400_ignore_dat_correction = 0;
                break;
            ...
            case PRODUCT_ID_RCAR_M3W:
                switch RCAR_PRODUCT_REV(prr_val) {
                    ...
                    case PRODUCT_M3_REV_1_3: // ES1.3
                        ...
                        if (RCAR_SDHI2_BASE == sdmmc->pbase) { // MMC0
                            sdmmc->adjust_hs400_calib_table = m3w_v13_hs400_calib_code[0];
                        } else if (RCAR_SDHI3_BASE == sdmmc->pbase) { // MMC1
            ...
            case PRODUCT_ID_RCAR_E3:
                ...
                sdmmc->adjust_hs400_enable = 1;
                if (RCAR_SDHI3_BASE == sdmmc->pbase) {// MMC1
...
int rcar_sdmmc_init(sdio_hc_t *hc)
{
    ...
    if (cpg_mgr_getfuncs(&cpg_hwfuncs, sizeof(cpg_mgr_funcs_t))) {
        sdio_slogf(_SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 0,
            "%s : Unable to get cpg hw funcs", __func__);
         return (ENODEV);
    } else {
        cpg_hwfuncs.init(hc->cfg.verbosity);
    }
    ...
    SIGEV_PULSE_INIT(&event, hc->hc_coid, hc->priority, HC_EV_INTR, NULL);
    ...
    { /* Suspend/resume testing */
        pthread_attr_t attr;
        void*suspend_resume_thread( void*  arg );

        pthread_attr_init( &attr );
        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED );
        pthread_create( NULL, &attr, &suspend_resume_thread, hc );
    }
#endif
...
void * suspend_resume_thread( void* arg )
{
    int idle_cnt = 0;
    sdio_hc_t *hc = arg;
    while(suspend_resume_running) {
        if (!suspend_already) {
            idle_cnt++;
        }
        if (idle_cnt == (60 * 1000) && !suspend_already) { // Suspend after 60s idle
            _sdio_pwrmgnt(&hc->device, PM_SUSPEND);
            suspend_already = 1;
            idle_cnt = 0;
        }
        delay(1);
