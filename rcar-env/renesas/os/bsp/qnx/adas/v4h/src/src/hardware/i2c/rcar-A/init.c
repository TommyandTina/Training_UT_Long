...
static int rcar_fastmodeplus_support_check(rcar_i2c_dev_t *dev)
{
    ...
    if (dev->product_id == PRODUCT_ID_RCAR_V3U ||
        dev->product_id == PRODUCT_ID_RCAR_V4H) {
        return 1;
    } else {
        return 0;
    }
...
static int rcar_i2c_change_io_voltage_18v(rcar_i2c_dev_t *dev)
{
    ...
    if (dev->poc.vbase == (uintptr_t)MAP_FAILED) {
        rcar_i2c_slogf(dev, VERBOSE_QUIET, "[chan %d]%s: mmap_device_io failed!", dev->chan_idx, __FUNCTION__);
        return ENOMEM;
    }
...
void * rcar_i2c_init(int argc, char *argv[])
{
    ...
    if ((!dev)) {
        rcar_i2c_slogf(dev, VERBOSE_QUIET, "%s: fail calloc of dev", __FUNCTION__);
        return NULL;
    }
    ...
    if (rcar_i2c_parse_options(dev, argc, argv) == -1) {
    ...
    rcar_i2c_set_bus_speed(dev, dev->bus_speed, NULL);
